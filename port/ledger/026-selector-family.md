...
covers only what is specific to the selectors. It also carries the general 0.4.1 upgrade-infrastructure
findings — CI splitting, the client vitest project's port-binding fix, `ThemeConfig.extends`, and the
`git checkout -- '*'` incident — which `port/ledger/_inbox.md` staged under a second, differently-scoped
"Batch 11" heading ("upstream 0.4.1 port findings"). None of that content is specific to `Selector` or
to the input family; it is filed here rather than with `012-batch-11.md` (Markdown/Table core, the
batch that name otherwise belongs to) because it is 0.4.1-era work, and here rather than in
`025-input-family.md` because its own heading names "0.4.1" specifically. This is a judgement call —
see the task report.
...
### General 0.4.1 upgrade-infrastructure findings
