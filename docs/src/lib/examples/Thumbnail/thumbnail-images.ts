/**
 * The four demo scenes from upstream's `Thumbnail` showcase blocks, verbatim.
 *
 * They are inline data URIs for a reason upstream writes down: `useImageMode`
 * *fetches* the image to sample it, so a cross-origin URL without CORS headers
 * can never be read and the remove button's contrast adaptation silently stops
 * working. Same-origin data URIs keep the example self-contained. The four span
 * a luminance ramp — night forest through snowy peaks — so the adaptation is
