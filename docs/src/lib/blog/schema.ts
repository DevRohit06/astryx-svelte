 ...
 * blog post as produced by the build-time generator
 * (`src/lib/generated/blog-registry.js`) and consumed by the blog pages.
 *
 * The runtime discovery/validation rules live in `./posts.mjs`, ported verbatim
 * from upstream. This file is the type-level mirror of that contract, and is
 * upstream's `schema.ts` unchanged but for this note and the registry path
 * above — the six post types, their labels and every field are its own.
 *
 * @input  none (type declarations)
 * @output BlogPost and related types
 * @position Imported by the generated registry and the blog pages/components
 */
