 ...
 *
 * **Ported verbatim from upstream's `apps/docsite/src/lib/blog/posts.mjs`** —
 * the only edits are this header and prettier's formatting. It needed no
 * translation at all, which is the point worth recording: it is plain Node with
 * no React in it, so the parser, the validator, the reading-time estimate and
 * the discovery order are upstream's own code rather than a re-implementation
 * that could drift from it. Upstream's authoring contract (its
 * `src/content/blog/README.md`) therefore describes this port exactly.
 *
 * Authoring model: a blog post is a single Markdown file with YAML frontmatter
 * under `src/content/blog/posts/<slug>.md`. No bespoke wiring is required —
 * files are discovered automatically and sorted latest-first.
 *
 * Why a hand-written frontmatter parser instead of a YAML dependency, in
 * upstream's words and true here too: the frontmatter schema is small and fixed
 * (scalars, string sequences, and a sequence of `{title, href}` maps for
 * `relatedDocs`). Keeping the parser local avoids adding a dependency to the
 * lockfile of a public repo for a build-only concern.
 *
 * The one thing this port does *not* carry over is upstream's test suite
 * (`src/__tests__/blog.test.ts`), because `docs` has no test runner. The
 * validation still runs on every build — `generate-content.mjs` calls
 * `discoverPosts`, and a malformed post fails the build with a slug-prefixed
 ...
 * @position Imported by scripts/generate-content.mjs
 */
...
export function validatePostFrontmatter(slug, data) {
	...
	if (data.dek != null && (typeof data.dek !== 'string' || data.dek.trim() === '')) {
...
export function estimateReadingTime(body) {
	const words = body
		...
		// Upstream's character class, kept byte-identical. The `\-` is redundant
		// (a trailing hyphen in a class is already literal) and this repo's eslint
		// says so, but the point of a verbatim port is that the reading-time
		// number cannot drift from upstream's — so the rule yields, not the regex.
