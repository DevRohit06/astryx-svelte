 ...
 * frontmatter so profile metadata is defined once.
 *
 * The **mechanism** is upstream's, ported unchanged — `AuthorProfile`,
 * `ResolvedAuthor` and `resolveAuthor`'s GitHub-derivation and unknown-key
 * fallback are its code. The **entries** are necessarily not: upstream's
 * registry lists Meta's design-system team, and copying their names into a port
 * they did not write would attribute this repo's posts to them.
 *
 ...
 * For non-GitHub authors (e.g. a project identity), provide `avatar`/`href`
 * explicitly.
 *
 ...
 * @position Imported by the blog pages and the byline component
 */
...
export interface ResolvedAuthor extends Omit<AuthorProfile, 'avatar' | 'href'> {
	/** The registry key, also the stable list key. */
	key: string;
 ...
 * typo in frontmatter never crashes the page — the build-time validation is
 * what surfaces the problem.
 */
export function resolveAuthor(key: string): ResolvedAuthor {
	...
	const avatar =
		profile.avatar ?? (profile.github ? `https://github.com/${profile.github}.png` : '');
	const href = profile.href ?? (profile.github ? `https://github.com/${profile.github}` : null);
