 ...
 *
 * ## Not ported: `Markdown/parser.perf.test.ts`
 *
 * Upstream's fourth Markdown suite is a **performance** suite — 11 runtime cases
 * (1 reporter, 5 full-parse budgets, 2 streaming full-reparse budgets, 2 streaming
 * incremental budgets, 1 incremental-vs-full speedup ratio) asserting wall-clock
 * millisecond budgets. It is deliberately **not** ported, for four reasons:
 *
 * 1. This repo has no perf-suite precedent — `CodeBlock`, the closest analogue
 *    (a pure tokenizer with an upstream perf story), ported only its behavioural
 *    suite, and `src/tests/` contains no wall-clock budget anywhere.
 * 2. Both vitest projects run with `fileParallelism: false` specifically so that
 *    node workers do not compete with the single Chromium for cores. A timing
 *    budget measured under that arrangement reports the machine and the scheduler,
 *    not the port — the exact flake class the config was written to retire.
 * 3. Upstream's own `it('reports input sizes')` asserts nothing at all; under this
 *    repo's `expect.requireAssertions: true` it cannot be ported verbatim, and
 *    restating a pure `console.log` reporter as an assertion invents coverage.
 * 4. `parser.ts` is a verbatim transcription, so its algorithmic complexity is
 *    upstream's by construction. The property the perf suite actually guards —
 *    that incremental parsing reuses settled blocks rather than re-parsing them —
 *    is asserted directly and deterministically by
 *    `markdown-incremental.test.ts`'s "reuses cached settled blocks when settled
 *    portion is unchanged", which checks array identity instead of a stopwatch.
 */
...
describe('parseMarkdown', () => {
	...
	it('treats a zero-padded value-1 marker as interrupting a paragraph', () => {
		...
		expect(parseMarkdown('Intro\n01. first').map((b) => b.type)).toEqual(['paragraph', 'list']);
		expect(parseMarkdown('Intro\n001) first').map((b) => b.type)).toEqual(['paragraph', 'list']);
...
describe('citation parsing', () => {
	...
	describe('bracket notation [id]', () => {
		...
		it('does NOT treat [text](url) as citation', () => {
			const result = parseInline('[click here](https://example.com)', sourceIds);
			expect(result).toEqual([
				{
					type: 'link',
		...
		it('handles citation next to link', () => {
			const result = parseInline('See [link](https://example.com)[abc1].', sourceIds);
	...
	describe('block-level threading', () => {
		...
		it('detects citations inside list items', () => {
			...
			if (blocks[0].type === 'list') {
				...
				if (firstItem.type === 'paragraph') {
					expect(firstItem.children.some((n) => n.type === 'citation')).toBe(true);
				}
			}
		});
		...
		it('detects citations inside table cells', () => {
			...
			if (blocks[0].type === 'table') {
				expect(blocks[0].rows[0][0].children.some((n) => n.type === 'citation')).toBe(true);
			}
		});
	});
	...
	describe('autolink (gfm)', () => {
		it('is opt-in: bare URLs stay literal by default', () => {
			...
			expect(result).toEqual([{ type: 'text', content: 'see https://example.com for info' }]);
		});
		...
		it('does not nest links: skips URLs inside an existing link', () => {
			const result = parseInline('[my site https://other.com](https://example.com)', {
				autolink: 'gfm'
			});
			expect(result).toHaveLength(1);
			expect(result[0].type).toBe('link');
		...
		it('handles multiple autolinks in one block', () => {
			const result = parseInline('see https://a.com or https://b.com or me@x.com', {
				autolink: 'gfm'
			});
			const linkHrefs = result
				.filter((n) => n.type === 'link')
				.map((n) => (n.type === 'link' ? n.href : ''));
			expect(linkHrefs).toEqual(['https://a.com', 'https://b.com', 'mailto:me@x.com']);
		});
...
describe('link reference definitions', () => {
	...
	it('resolves a full reference `[text][label]` and drops the definition', () => {
		...
		expect(links).toEqual([{ type: 'link', href: 'https://example.com/docs', text: 'the docs' }]);
	});

	it('resolves a collapsed reference `[text][]`', () => {
		const { links } = paragraphLinks('See [the docs][].\n\n[the docs]: https://example.com/docs');
		expect(links).toEqual([{ type: 'link', href: 'https://example.com/docs', text: 'the docs' }]);
	});
	...
	it('matches labels case-insensitively with collapsed whitespace', () => {
		const { links } = paragraphLinks('See [The   Docs][DOCS].\n\n[docs]: https://example.com/d');
		expect(links).toEqual([{ type: 'link', href: 'https://example.com/d', text: 'The   Docs' }]);
	});

	it('supports an angle-bracket destination and a title', () => {
		const { links } = paragraphLinks('See [x].\n\n[x]: <https://example.com/x> "the title"');
		expect(links).toEqual([{ type: 'link', href: 'https://example.com/x', text: 'x' }]);
	});
	...
	it('does not treat a whitespace-only second label as collapsed', () => {
		...
		expect(blocks[0].type).toBe('paragraph');
		if (blocks[0].type === 'paragraph') {
			expect(collectLinks(blocks[0].children)).toEqual([{ type: 'link', href: '/f', text: 'foo' }]);
	...
	it('leaves footnote markers and definitions untouched', () => {
		const blocks = parseMarkdown('A claim.[^1]\n\n[^1]: https://example.com/note');
