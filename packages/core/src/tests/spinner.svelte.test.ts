...
describe('Spinner ring', () => {
	...
	it('sizes the frame from CSS, not from the size constant', async () => {
		const screen = await render(Spinner, { props: { 'data-testid': 'spinner' } });
		const root = screen.getByTestId('spinner').element();
		const svg = ringOf(root);
		// No viewBox, and the circles centre on the box rather than on a centre in
		// user units — together that is what lets CSS size the frame from
		// `--_spinner-box-size`, the same composed var the span is sized from, so a
		// themed diameter moves both. The px attributes remain as the no-stylesheet
		// fallback and CSS outranks them whenever one is present.
		expect(svg.getAttribute('viewBox')).toBeNull();
		const { track, arc } = circlesOf(root);
		for (const c of [track, arc]) {
			expect(c.getAttribute('cx')).toBe('50%');
	...
	it('starts the arc at twelve o’clock', async () => {
		...
		expect(
			circlesOf(screen.getByTestId('spinner').element()).arc.getAttribute('transform')
		).toBeNull();
	});
