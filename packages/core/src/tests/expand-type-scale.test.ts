...
/**
 * Astryx's `theme/expandTypeScale.test.ts`, ported case for case — **30 upstream
 * cases at the 0.5.0 pin, 30 here**, in upstream's order and under upstream's titles.
 * Nothing dropped, nothing added.
 *
 * A **server** project file (`*.test.ts`), as upstream's is: both functions are
 * pure computation over a config object.
 *
 * ## What porting it cost
 *
 * The suite could not be ported case for case until the module's signature was
 * upstream's. `expandTypeScale` here took a `TypographyConfig` — the whole
 * `{scale, body, heading, code}` block — and did two jobs upstream gives to
 * `defineTheme`: building `--font-family-*` from the role declarations, and
 * mapping named weights (`'bold'`) onto `var(--font-weight-bold)`. So the
 * published `TypeScaleConfig` was shape drift under a shared name: a consumer
 * typing against it got `{base, ratio}` and handed the function an object it
 * would not accept.
 *
 * Both jobs moved to `define-theme.ts`, where upstream has them, and the move
 * exposed a real defect on the way — upstream's `heading` family inherits
 * `body`'s when it declares none, and this port's did not, so a theme naming
 * only `body.family` kept the base theme's heading face. See
 * `port/ledger/030-theme-conformance.md`.
 *
 * `theme.test.ts` carried three cases over this module. Two are upstream's under
 * different titles and are covered below (`anchors --font-size-base to the base
 * size`, `applies heading weight overrides`); the third asserted the joined
 * family stack, which is `defineTheme`'s output now and is asserted there.
 */
describe('expandTypeScale', () => {
	...
	describe('Layer 2: semantic tokens', () => {
		...
		it('weight tokens are var() references (unchanged)', () => {
			expect(tokens['--text-heading-1-weight']).toBe('var(--font-weight-semibold)');
			...
			expect(tokens['--text-supporting-weight']).toBe('var(--font-weight-normal)');
		});
		...
		it('assigns normal weight to display types by default', () => {
			expect(tokens['--text-display-1-weight']).toBe('var(--font-weight-normal)');
			expect(tokens['--text-display-2-weight']).toBe('var(--font-weight-normal)');
			expect(tokens['--text-display-3-weight']).toBe('var(--font-weight-normal)');
		});
		...
		it('uses tiered target ratio based on font size', () => {
			...
			expect(Math.round(14 * parseFloat(tokens['--text-heading-4-leading']))).toBe(20);
			// 20-31px target 1.4: 20px → 28px line
			expect(Math.round(20 * parseFloat(tokens['--text-heading-2-leading']))).toBe(28);
			// 20-31px target 1.4: 24px → 32px line
			expect(Math.round(24 * parseFloat(tokens['--text-heading-1-leading']))).toBe(32);
		});
	});
	...
	describe('does not touch named leading tokens', () => {
		...
		it('does not emit --leading-* tokens', () => {
			const leadingKeys = Object.keys(tokens).filter((k) => k.startsWith('--leading-'));
	...
	describe('weight overrides', () => {
		it('applies heading weight overrides', () => {
			...
			expect(tokens['--text-heading-2-weight']).toBe('var(--font-weight-semibold)');
		});
		...
		it('all scales produce 4px-grid-aligned line heights', () => {
			...
			for (const config of scales) {
				...
				for (let level = 1; level <= 6; level++) {
					...
					const fontSize = Math.round(config.base * Math.pow(config.ratio, step));
...
describe('generateTypeScaleComponents', () => {
	...
	it('heading overrides reference semantic tokens', () => {
		const components = generateTypeScaleComponents({ base: 14, ratio: 1.2 });
		expect(components.heading['level:1'].fontSize).toBe('var(--text-heading-1-size)');
		expect(components.heading['level:1'].lineHeight).toBe('var(--text-heading-1-leading)');
	});

	it('text overrides reference semantic tokens', () => {
		const components = generateTypeScaleComponents({ base: 14, ratio: 1.2 });
		expect(components.text['type:body'].fontSize).toBe('var(--text-body-size)');
		expect(components.text['type:body'].lineHeight).toBe('var(--text-body-leading)');
	});
	...
	it('heading display-type rules reference the display semantic tokens', () => {
		...
		// type-keyed rules — see the default-weight-by-type styles for how weight
		// is applied instead.
	...
	it('emits heading type:display-N rules after level:N rules, so type wins at equal specificity', () => {
		...
		// same element simultaneously when `type` is set (see heading.svelte).
		...
		const firstTypeIndex = Math.min(
			...['display-1', 'display-2', 'display-3'].map((type) => keys.indexOf(`type:${type}`))
		);
