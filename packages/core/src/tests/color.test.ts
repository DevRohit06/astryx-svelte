...
describe('formatColor', () => {
	...
	it('formats translucent colors as rgba()', () => {
		expect(formatColor({ r: 0, g: 100, b: 224, a: 0.2 })).toBe('rgba(0, 100, 224, 0.2)');
	});
...
describe('toGLFloats', () => {
	...
	it('converts RGBA channels to 0-1 floats, dropping alpha', () => {
		expect(toGLFloats({ r: 0, g: 100, b: 224, a: 0.5 })).toEqual([0, 100 / 255, 224 / 255]);
	});
	...
	it('clamps hand-constructed out-of-range channels', () => {
		expect(toGLFloats({ r: 300, g: -5, b: 127.5, a: 1 })).toEqual([1, 0, 127.5 / 255]);
	});
	...
	it.each([
		...
		['rgb() string → fallback on the hex path', 'rgb(255, 0, 0)', [0.5, 0.5, 0.5]],
...
describe('alpha application (conformance for charts/lab `colors.alpha`)', () => {
	...
	it('opacity argument wins over rgba() input alpha', () => {
		expect(applyAlpha('rgba(0, 100, 224, 0.9)', 0.2)).toBe('rgba(0, 100, 224, 0.2)');
	});
