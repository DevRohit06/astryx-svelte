...
// through `$lib/…` rather than a relative path out of `src/tests/`. The
// precedent for reading a `.ts` module as text is
// `input-clear-button.svelte.test.ts`.
import SOURCE from '$lib/components/number-input/number-parser.ts?raw';

/**
 * Astryx's `NumberInput/numberParser.docblock.test.ts`, ported case for case.
 * Upstream at the **0.5.2** pin declares **3** blocks producing **34** cases —
 * the 8-row `CLAUSES` `it.each`, the 25-row `NAMED` `it.each`, and one `it` for
 * the generated sweep — and all 3 are here. Nothing is dropped.
 *
 * The file docblock of `number-parser.ts` states the rule the parser follows.
 ...
 *
 * This is *not* the "coverage beyond upstream" case `CLAUDE.md` sets a high bar
 * for: it is upstream's own suite, and the paragraph it grades is upstream's
 * paragraph, transcribed with only the file references renamed to this port's
...
const ALPHABET: ReadonlySet<string> = new Set(
	JSON.parse(`"${/const SEPARATOR_CHARS = "([^"]*)"/.exec(SOURCE)?.[1] ?? ''}"`)
);
...
function readSymbols(locale: string) {
	...
	return {
		group: (parts.find((p) => p.type === 'group')?.value ?? ',').normalize('NFKC'),
		...
		for (const separator of ALPHABET) {
			for (const a of runs) {
				for (const b of runs) {
					...
					for (const c of runs) {
						yield [`${a}${separator}${b}${separator}${c}`, locale] as [string, string];
						for (const d of runs) {
							yield [`${a}${separator}${b}${separator}${c}${separator}${d}`, locale] as [
								string,
								string
							];
						}
