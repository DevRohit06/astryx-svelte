...
import {
	...
	DETENT_DEDUP_PX
} from '$lib/components/bottom-sheet/snap-offsets.js';
...
describe('resolveSnapPoints', () => {
	...
	it('keeps the caller order, mixing forms', () => {
		expect(resolveSnapPoints(['120px', 0.5, '90%'], 800)).toEqual([120, 400, 720]);
	});

	it('accepts fractional and unit-cased lengths, and surrounding space', () => {
		expect(resolveSnapPoints(['12.5%', '0.5PX', ' 80px '], 800)).toEqual([100, 0.5, 80]);
	});

	it('drops numbers outside the (0, 1] fraction range', () => {
		// 200 is the px mistake — a bare number is never a length.
		expect(resolveSnapPoints([0, 0.5, 1, 1.2, -0.3, 200], 1000)).toEqual([500, 1000]);
	});

	it('drops lengths it cannot resolve by arithmetic on the viewport', () => {
		expect(
			resolveSnapPoints(['50', '30rem', '50vh', 'calc(50% - 20px)', '-10px', '', '0px'], 800)
		).toEqual([]);
	});
});
...
describe('scrimOpacityForOffset', () => {
	describe('with a peek detent', () => {
		...
		it('is full at or above the mid detent', () => {
			expect(scrimOpacityForOffset(0, offsets, dismissOffset, peekOffset)).toBe(1);
			expect(scrimOpacityForOffset(100, offsets, dismissOffset, peekOffset)).toBe(1);
			expect(scrimOpacityForOffset(60, offsets, dismissOffset, peekOffset)).toBe(1);
		});
		...
		it('fades linearly from the mid detent toward the peek floor', () => {
			...
			// 0.3 floor => 0.65.
			expect(scrimOpacityForOffset(150, offsets, dismissOffset, peekOffset)).toBeCloseTo(0.65);
		});

		it('thins to the peek floor at or below the peek detent (still modal)', () => {
			expect(scrimOpacityForOffset(200, offsets, dismissOffset, peekOffset)).toBeCloseTo(0.3);
			expect(scrimOpacityForOffset(240, offsets, dismissOffset, peekOffset)).toBeCloseTo(0.3);
		});
	});
	...
	describe('with collapsed stops but no peek among them', () => {
		...
		it('fades only across the dismiss overshoot below the shortest stop', () => {
			expect(scrimOpacityForOffset(130, offsets, dismissOffset, null)).toBeCloseTo(0.5);
	...
	describe('with a single detent (no peek)', () => {
		...
		it('fades across the dismiss overshoot toward the threshold', () => {
			expect(scrimOpacityForOffset(80, offsets, dismissOffset, null)).toBeCloseTo(0.5);
			expect(scrimOpacityForOffset(160, offsets, dismissOffset, null)).toBe(0);
		});
	});
});
