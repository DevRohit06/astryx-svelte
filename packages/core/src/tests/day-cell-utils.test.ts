/** PORTS: Calendar/dayCellUtils.test.ts */
...
} from '$lib/components/calendar/day-cell-utils.js';
 ...
 * reactivity and no React in the frame — the module transcribes upstream's
 * line for line, so the suite does too. It is a plain `*.test.ts` and therefore
 * runs in the **server** project.
 *
 * `dayCellUtils` is module-private (upstream's `Calendar/index.ts` re-exports
 * nothing from it), so the import is by path rather than through the barrel —
 * exactly as upstream's own test imports `./dayCellUtils`.
 *
 * No cases are dropped.
 */

function makeInput(overrides: Partial<DayCellStateInput> = {}): DayCellStateInput {
	return {
...
describe('computeDayCellState', () => {
	...
	it('identifies not-today', () => {
		const state = computeDayCellState(makeInput({ today: plainDateFromISO('2024-03-16') }));
	...
	it('identifies selected day in single mode', () => {
		const state = computeDayCellState(makeInput({ selectedDate: plainDateFromISO('2024-03-15') }));
...
describe('isRangeHighlighted', () => {
	...
	it('false outside the range bounds', () => {
		expect(isRangeHighlighted({ ...base, date: plainDateFromISO('2024-03-25') })).toBe(false);
	});
...
describe('isPreviewHighlighted', () => {
	...
	it('false outside the preview bounds', () => {
		expect(isPreviewHighlighted({ ...base, date: plainDateFromISO('2024-03-25') })).toBe(false);
	});

	it('false with no preview span', () => {
		expect(isPreviewHighlighted({ ...base, previewStart: null, previewEnd: null })).toBe(false);
	});
});
...
describe('computeDayNeighborContinuity', () => {
	...
	const week = (
		['2024-03-15', '2024-03-16', '2024-03-17', '2024-03-18', '2024-03-19'] as const
	).map((iso) => ({
	...
	it('breaks continuity where a neighbour is an outside day', () => {
		const weekWithOutside = week.map((d, i) => (i === 3 ? { ...d, isOutside: true } : d));
		const c = computeDayNeighborContinuity({
			...baseInput,
