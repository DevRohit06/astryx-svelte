 ...
 * and CI (typically UTC) run the same assertions. Every exact-string
 * expectation therefore names an explicit zone and uses a `system_*` format,
 * which is built from numeric parts and is both timezone- and
 * locale-independent. Assertions about locale-formatted output stay structural.
 *
 * **Nothing is translated.** `tooltip-entries.ts` warns through
 * `devWarn('Timestamp', …)`, exactly as upstream's `tooltipEntries.ts` does, so
 * the `console.warn` spy sees the same single `Timestamp: …` string on both
 * sides and every assertion here is upstream's verbatim. (An earlier revision of
 * this header said the port "does not have" `devWarn` and used a bare
 * `console.warn` instead. That was true when written and is not any more — the
...
describe('formatTooltipLines', () => {
	...
	it('renders system_date_time in an explicit zone', () => {
		const lines = formatTooltipLines(INSTANT, [{ timezoneID: 'UTC', format: 'system_date_time' }]);
	...
	it.each(['date', 'date_long', 'date_weekday'] as const)(
		...
		(format) => {
			...
			const [tokyo] = formatTooltipLines(INSTANT, [{ timezoneID: 'Asia/Tokyo', format }]);
	...
	it('matches the local-getter output character-for-character for the viewer zone', () => {
		// The zone-less path builds system_* from date.getFullYear()/getHours()/…
		// Naming the viewer's own zone explicitly must produce the identical
		// string, or the two code paths have drifted.
		...
		const implicit = formatTooltipLines(INSTANT, [{ format: 'system_date_time' }]);
	...
	it("does not treat 'local' as an IANA id to be looked up", () => {
		...
		expect(() => formatTooltipLines(INSTANT, [{ timezoneID: 'local' }])).not.toThrow();
	});
	...
	it('omits the zone abbreviation when one unnamed zone is shown twice', () => {
		...
		const localName = new Intl.DateTimeFormat(undefined, { timeZoneName: 'short' })
	...
	it('marks a single explicitly-named zone so it cannot be read as local', () => {
		...
		const [line] = formatTooltipLines(INSTANT, [{ timezoneID: 'Asia/Tokyo', format: 'date_time' }]);
	...
	it('falls back for an empty or whitespace-only zone id', () => {
		...
		try {
			...
			for (const timezoneID of ['', '   ']) {
				const lines = formatTooltipLines(INSTANT, [{ timezoneID, format: 'system_date_time' }]);
				expect(lines[0].value).toBe(local[0].value);
			}
	...
	it('treats a fixed-offset id like EST as fixed, not as US Eastern', () => {
		...
		const fixed = formatTooltipLines(summer, [{ timezoneID: 'EST', format: 'system_date_time' }]);
	...
	it('spans the full range either side of the date line', () => {
		...
		expect(lines.map((l) => l.value)).toEqual(['2026-02-20 07:00:00', '2026-02-19 06:00:00']);
	});
	...
	it('renders two alias spellings of one zone identically to a single one', () => {
		...
		const solo = formatTooltipLines(INSTANT, [{ timezoneID: 'UTC', format: 'date_time' }]);
	...
	it('treats an explicitly undefined format as absent', () => {
		const explicit = formatTooltipLines(INSTANT, [{ timezoneID: 'UTC', format: undefined }]);
	...
	it('preserves entry order', () => {
		...
		expect(lines.map((l) => l.value)).toEqual(['2026-02-20', '2026-02-19', '2026-02-19']);
	});

	it('returns one line per entry, including duplicates', () => {
		const lines = formatTooltipLines(INSTANT, [{ timezoneID: 'UTC' }, { timezoneID: 'UTC' }]);
		expect(lines).toHaveLength(2);
		expect(lines[0].value).toBe(lines[1].value);
	});
