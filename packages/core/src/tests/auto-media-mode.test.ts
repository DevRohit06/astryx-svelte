...
/**
 * Astryx's `hooks/useAutoMediaMode.test.ts`, ported case for case — **all 10
 * cases at the 0.5.0 pin**, in upstream's order and under upstream's titles.
 * Nothing dropped, nothing added.
 *
 * A **server** project file (`*.test.ts`), as upstream's is: `pickMediaMode` is
 * pure contrast arithmetic over parsed colours, and the DOM half of the hook —
 * the backdrop walk and the hidden probe — is the caller's. Upstream covers
 * that half separately in `theme/MediaTheme.dom.test.tsx`, which is a client
 * suite here and is not ported yet.
 */
...
describe('pickMediaMode', () => {
	it('picks dark for a dark surface under a light page', () => {
		expect(pickMediaMode(rgb('#0A1317'), ambientLight, onDark, onLight)).toBe('dark');
	});

	it('picks light for a light surface under a dark page', () => {
		expect(pickMediaMode(rgb('#FFFFFF'), ambientDark, onDark, onLight)).toBe('light');
	});
	...
	it('answers off for a surface a theme merely calls inverted', () => {
		...
		expect(pickMediaMode(rgb('#E4E6EB'), ambientLight, onDark, onLight)).toBe('off');
	});
	...
	it('answers off for a chromatic surface its ambient text already reads on', () => {
		...
		expect(pickMediaMode(rgb('#E3193B'), ambientDark, onDark, onLight)).toBe('off');
	});
	...
	it('picks dark for a saturated surface under a light page', () => {
		...
		expect(pickMediaMode(rgb('#AA071E'), ambientLight, onDark, onLight)).toBe('dark');
	});

	it('respects a theme that overrides its on-colors', () => {
		// The decision follows the theme's own values, not an assumption that
		// on-dark is white.
		expect(pickMediaMode(rgb('#0A1317'), ambientLight, rgb('#101010'), rgb('#F0F0F0'))).toBe(
			'light'
		);
	});
	...
	it('composites a translucent on-color before judging it', () => {
		...
		expect(
			pickMediaMode(rgb('#767676'), rgb('#8A8A8A'), rgb('rgba(255,255,255,0.10)'), rgb('#171717'))
		).toBe('light');
	});
	...
	it('treats the bar as a floor, not a target', () => {
		...
		expect(pickMediaMode(rgb('#8A8A8A'), rgb('#FFFFFF'), onDark, onLight)).toBe('off');
	});

	it('gives up when an on-color is unreadable', () => {
		expect(pickMediaMode(rgb('#0A1317'), ambientLight, null, onLight)).toBeNull();
		expect(pickMediaMode(rgb('#0A1317'), ambientLight, onDark, null)).toBeNull();
	});
});
