 ...
 * Chromium — the rule `use-typeahead` and the `resolve` suite already follow.
 *
 * The module itself is not exported from the hooks barrel (upstream keeps it out
 * of its own), so this imports it from the module directly, as upstream's suite
 * does.
 */
...
describe('computeOverflow — single line, no limits (parity with legacy loop)', () => {
	it('shows all items when they fit', () => {
		// 3×50, gap 10 → 170 ≤ 200
		expect(compute({ widths: [50, 50, 50], gap: 10, availableWidth: 200 })).toEqual({
			visibleCount: 3,
			rows: 1
		});
	});
	...
	it('works without an indicator', () => {
		...
		expect(compute({ widths: [50, 50, 50], gap: 10, availableWidth: 130 })).toEqual({
			visibleCount: 2,
			rows: 1
		});
	});
...
describe('computeOverflow — degenerate inputs', () => {
	...
	it('single item that does not fit, no floor', () => {
		expect(compute({ widths: [200], indicatorWidth: 30, availableWidth: 100 })).toEqual({
			visibleCount: 0,
			rows: 0
		});
	});
