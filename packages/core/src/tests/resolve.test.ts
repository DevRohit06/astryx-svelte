...
describe('resolve — basic lookup', () => {
	test('falls back to shipped en catalog when locale is en and no messages passed', () => {
		...
		const out = resolve('@astryx.pagination.next', undefined, 'en', {}, undefined);
		expect(out).toBe('Go to next page');
	});

	test('returns the key itself and warns for unknown key', () => {
		const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
		const out = resolve('@astryx.does.not.exist', undefined, 'en', {}, undefined);
...
describe('resolve — provider messages', () => {
	...
	test('uses provider messages for the current locale', () => {
		const out = resolve('@astryx.pagination.next', undefined, 'fr', messages, undefined);
	...
	test('falls back to en for keys not in the provider catalog, silently', () => {
		...
		const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
		const out = resolve('@astryx.pagination.previous', undefined, 'fr', messages, undefined);
...
describe('resolve — locale-chain fallback (regional → language)', () => {
	...
	test('exact regional match wins', () => {
		const out = resolve('@astryx.pagination.next', undefined, 'pt-BR', messages, undefined);
	...
	test('falls back to base language when regional entry missing', () => {
		...
		const out = resolve('@astryx.pagination.previous', undefined, 'pt-BR', messages, undefined);
	...
	test('falls back to en when neither regional nor base has the key', () => {
		...
		const warn = vi.spyOn(console, 'warn').mockImplementation(() => {});
		const out = resolve('@astryx.pagination.label', undefined, 'pt-BR', messages, undefined);
...
describe('resolve — overrides', () => {
	test('per-locale override wins over shipped catalog', () => {
		...
		const out = resolve('@astryx.pagination.next', undefined, 'fr', messages, overrides);
	...
	test('overrides respect the locale chain (pt-BR → pt)', () => {
		...
		const out = resolve('@astryx.pagination.next', undefined, 'pt-BR', {}, overrides);
	...
	test('override for a different locale does not apply', () => {
		...
		const out = resolve('@astryx.pagination.next', undefined, 'fr', {}, overrides);
...
describe('resolve — ICU MessageFormat', () => {
	test('interpolates simple named placeholders', () => {
		const out = resolve('@astryx.pagination.goToPage', { page: 5 }, 'en', {}, undefined);
	...
	test('handles plural forms with ICU {n, plural, one {} other {}} pattern', () => {
		...
		expect(resolve('test.items', { count: 0 }, 'en', messages, undefined)).toBe('no items');
		expect(resolve('test.items', { count: 1 }, 'en', messages, undefined)).toBe('1 item');
		expect(resolve('test.items', { count: 5 }, 'en', messages, undefined)).toBe('5 items');
	});
});
