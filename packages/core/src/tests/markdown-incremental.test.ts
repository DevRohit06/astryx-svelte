...
import {
	...
	type BlockNode,
	type InlineNode
} from '$lib/components/markdown/parser.js';
 ...
 *
 * - `suppresses table separator without data rows` asserts *nothing* upstream;
 *   its body is three comments talking itself out of an assertion. Under this
 *   repo's `expect.requireAssertions: true` it cannot be ported as-is, so it is
 *   **restated** to check the property its own last comment names ("we don't
 *   show partial pipe syntax"), which is also what its title claims.
 *
...
describe('trimStreamingArtifacts', () => {
	...
	it('auto-closes unclosed bold after closed bold', () => {
		expect(trimStreamingArtifacts('Hello **done** and **open')).toBe('Hello **done** and **open**');
	});
	...
	it('preserves closed strikethrough', () => {
		expect(trimStreamingArtifacts('Hello ~~struck~~ more')).toBe('Hello ~~struck~~ more');
	});
	...
	it('preserves closed links', () => {
		expect(trimStreamingArtifacts('Hello [text](url)')).toBe('Hello [text](url)');
	});
...
describe('streaming structural suppression', () => {
	...
	it('suppresses table separator without data rows', () => {
		...
		//
		// RESTATED: upstream's body ends here, with no `expect` at all — under
		// `expect.requireAssertions: true` that is a failing case, and the
		// assertion it describes in prose is the one worth having. Both readings
		// its comments allow are permitted below (suppressed entirely, or a table
		// with zero data rows); what is *not* permitted is the raw pipe syntax
		// leaking into a paragraph, which is the property the title names.
		const table = blocks.find((b) => b.type === 'table');
		expect(table === undefined || (table.type === 'table' && table.rows.length === 0)).toBe(true);
		const literalText = blocks
			.map((b) =>
				b.type === 'paragraph' || b.type === 'heading'
					? b.children.map((n) => (n.type === 'text' ? n.content : '')).join('')
					: ''
			)
			.join('');
	...
	describe('with autolink option', () => {
		it('produces same final result as full parse with autolink: gfm', () => {
			...
			if (para.type === 'paragraph') {
				...
				expect(linkHrefs).toEqual(['https://example.com', 'mailto:user@example.com']);
			} else {
		...
		it('invalidates the cache when the autolink option toggles', () => {
			...
			const noLinks = off.every(
				(b) => b.type !== 'paragraph' || b.children.every((n: InlineNode) => n.type !== 'link')
			);
			...
			const hasLink = on.some(
				(b) => b.type === 'paragraph' && b.children.some((n: InlineNode) => n.type === 'link')
			);
...
describe('parseMarkdownIncremental link reference definitions', () => {
	...
	it('re-resolves an already-settled reference once its definition arrives', () => {
		...
		const after = parseMarkdownIncremental('See [d] here.\n\n[d]: /docs\n', state);
