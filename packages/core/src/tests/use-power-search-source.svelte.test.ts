...
/**
 * Astryx's `PowerSearch/usePowerSearchSource.test.ts`, ported case for case —
 * **31 upstream cases, 31 here**, in upstream's order and under upstream's
...
async function createSource(config: PowerSearchConfig): Promise<SearchSource<PowerSearchItem>> {
	const screen = await render(PowerSearchSourceProbe, { props: { config } });
