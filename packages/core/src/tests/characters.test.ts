 ...
 *
 * The fallback block imports the module a second time under a stubbed `Intl`,
 * which needs `vi.resetModules()` — the segmenter is constructed once at module
 * scope, so a cached module would still hold the real one.
 */

// Multi-code-unit fixtures: an emoji surrogate pair (2 units), a flag sequence
// (4 units), a ZWJ family (11 units), and a combining mark (2 units).
const EMOJI = '\u{1F600}'; // 😀
const FLAG = '\u{1F1F9}\u{1F1F7}'; // 🇹🇷
const FAMILY = '\u{1F468}‍\u{1F469}‍\u{1F467}‍\u{1F466}'; // 👨‍👩‍👧‍👦
// é as base + combining mark, spelled as an escape so NFC-normalising tools
// cannot silently precompose the fixture into single-code-unit é.
const E_ACUTE = 'é';
...
describe('truncateCharacters', () => {
	...
	it('degrades to just the ellipsis when max is smaller than the ellipsis', () => {
		// Documented degenerate behaviour: no consumer passes max below the
...
describe('code-point fallback (no Intl.Segmenter)', () => {
	...
	it('keeps surrogate pairs intact while counting by code points', async () => {
		...
		const fallback = await import('$lib/utils/characters.js');
		...
		expect(fallback.truncateCharacters(EMOJI.repeat(4), 3)).toBe(`${EMOJI.repeat(2)}…`);
