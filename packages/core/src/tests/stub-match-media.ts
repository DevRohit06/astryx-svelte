 ...
 * `MobileNav`'s close delay is capped at 0 under reduced motion, so a suite
 * using the blanket stub runs against an immediate close while its test names
 * claim it is exercising the real delay. The failure mode is invisible: every
 * test still passes.
 *
 ...
 * value meant for `reduce` — the same answer-without-reading mistake this helper
 * exists to prevent. Core only queries `reduce` today (`MobileNav`, `Carousel`);
 * handling both is what keeps that true if one changes.
 */
function answerReducedMotion(query: string, reduceMotion: boolean): boolean | null {
