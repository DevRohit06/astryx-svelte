 ...
 * nothing restated.
 *
 * (The count is worth stating precisely because it is easy to undercount: the
 * cases are spread over 34 nested `describe` blocks, several of which hold two,
 * and 10 of the 51 sit in the top-level `createPowerSearchConfig` block rather
 * than under `applyFilters`.)
 *
 * ## No fixture, and the server project
 *
 * Upstream's suite imports `createPowerSearchConfig` — the **pure builder** —
 * and never touches the `usePowerSearchConfig` hook, so there is no hook to
 * render and no probe to write. `createPowerSearchConfig` is published upstream
 * for exactly this reason and is transcribed verbatim here (operator order,
 * i18n keys, default operators, the whole match table), so the suite has
 * nothing to translate.
 *
 * That makes this a plain `*.test.ts` in the **server** project: 51 pure calls
 * in node, rather than 51 pure calls behind a Chromium boot that no assertion
 * here would use. The module is a `.svelte.ts` only because
 * `usePowerSearchConfig`'s `$derived` lives in the same file; importing it into
 * a node test is the `optimistic.test.ts` precedent.
 *
 * ## The one divergence in the module under test, and why no case pins it
 *
 * `matchesFilter` dispatches with `Object.hasOwn(stringOpHandlers, operator)`
 * where upstream writes `operator in stringOpHandlers` — a deliberate fix to an
 * upstream prototype-chain bug (a filter whose operator is `'toString'` would
 * resolve `Object.prototype.toString` and call it as a comparator). Upstream's
 ...
 * module header and port/debts.md's Known debts.
 */
...
describe('createPowerSearchConfig', () => {
	...
	it('generates date operators', () => {
		...
		expect(field.operators.map((o) => o.key)).toEqual(['before', 'after', 'between']);
	...
	it('generates enum operators with values', () => {
		...
		expect(field.operators.map((o) => o.key)).toEqual(['is', 'is_not', 'is_any_of', 'is_none_of']);
	...
	it('generates enum_list operators with values', () => {
		...
		expect(field.operators.map((o) => o.key)).toEqual(['is_any_of', 'is_none_of']);
...
describe('applyFilters', () => {
	...
	describe('string: is', () => {
		...
		it('does not match partial strings', () => {
			const result = applyFilters([filter('name', 'is', { type: 'string', value: 'bob' })], data);
	...
	describe('number: equals', () => {
		it('matches exact number', () => {
			const result = applyFilters([filter('age', 'equals', { type: 'float', value: 30 })], data);
			expect(result).toHaveLength(1);
			expect(result[0].name).toBe('Alice Johnson');
		});
	});
	...
	describe('number: greater_than', () => {
		it('matches numbers greater than value', () => {
			...
			expect(result.map((r) => r.name).sort()).toEqual(['Alice Johnson', 'Charlie Brown']);
		});
	});

	describe('number: less_than', () => {
		it('matches numbers less than value', () => {
			const result = applyFilters([filter('age', 'less_than', { type: 'float', value: 28 })], data);
			expect(result).toHaveLength(1);
			expect(result[0].name).toBe('Bob Smith');
		});
	});
	...
	describe('number: less_than_or_equal', () => {
		it('matches numbers <= value', () => {
			...
			expect(result).toHaveLength(2);
			expect(result.map((r) => r.name).sort()).toEqual(['Bob Smith', 'Diana Prince']);
		});
	});
	...
	describe('date: between', () => {
		it('matches dates within absolute range', () => {
			...
			expect(result).toHaveLength(2);
			expect(result.map((r) => r.name).sort()).toEqual(['Bob Smith', 'Charlie Brown']);
		});
	...
	describe('boolean: is_true', () => {
		it('matches truthy values', () => {
			const result = applyFilters([filter('active', 'is_true', { type: 'empty' })], data);
			expect(result).toHaveLength(3);
	...
	describe('boolean: is_false', () => {
		it('matches falsy values', () => {
			const result = applyFilters([filter('active', 'is_false', { type: 'empty' })], data);
			expect(result).toHaveLength(1);
			expect(result[0].name).toBe('Bob Smith');
		});
	});
	...
	describe('enum: is', () => {
		it('matches exact enum value', () => {
			const result = applyFilters([filter('role', 'is', { type: 'enum', value: 'admin' })], data);
			expect(result).toHaveLength(2);
			expect(result.map((r) => r.name).sort()).toEqual(['Alice Johnson', 'Diana Prince']);
		});
	});
