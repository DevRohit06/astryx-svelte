 ...
 *
 * A **server** project file (`*.test.ts`), as upstream's is: every case is a
 * pure function over a `defineTheme` result, asserting on generated CSS text
 * with no DOM anywhere in it.
 *
 * ## The one translation, which is not a case
 *
 * Upstream imports `{defineTheme, generateThemeCSS, generateThemeRules}` from
 * `./index`; this imports the same three names from `$lib/theme/index.js`, the
 * same barrel. Until batch 034 the port published neither `generateThemeRules`
 * (a flat rule list) nor `generateThemeCSS` (`{prose, component}`, two `@scope`
 * blocks with no `@layer` wrappers) — it had `generateThemeRulesSplit` and a
 * `generateThemeCss` returning one layer-wrapped document — and that divergence,
 * not any assertion here, is what blocked this suite. `generateThemeCss`
 * (lowercase `ss`) still exists for the theme build scripts and is deliberately
 * off the public barrel, because upstream publishes no such function; this file
 * never touches it.
 *
 * Two structural facts the suite depends on, both of which hold here even though
 * this port reaches them from the other side. Upstream generates one flat list
 * and *derives* the prose/component split from it by testing each rule for a
 * leading `:where(`; this port generates the two groups directly and
 * concatenates them. So `generateThemeCSS returns prose and component blocks
 * with @scope` — which requires every string from `generateThemeRules` to appear
 * verbatim inside `prose + component` — only passes because the blocks are
 * joined without re-indenting, and `orders size overrides after the themed type
 * font-size rules` only passes because the concatenation preserves the order
 * upstream's single pass produces.
 *
 * ## Not covered elsewhere
 *
 * `theme.test.ts` has a `generateThemeCss` describe, but it exercises this
 * port's own layered document under four titles upstream does not have; no case
 * below duplicates one of them. `theme-tokens.test.ts` also has a `focus outline
 * tokens` describe — upstream's `theme/tokens.test.ts` does too, with three
 * different titles — and it asserts on `resolvedTokens` rather than on emitted
 * CSS, so the case below is upstream's own and stands alone.
 */
...
describe('generateThemeRules', () => {
	...
	it('applies pseudo-class suffixes to both compat selector prefixes', () => {
		...
		expect(pseudoRules.some((rule) => rule.includes('.astryx-button:hover'))).toBe(true);
	});
...
describe('derived var expansion', () => {
	...
	it('replaces paddingInline with the var for textarea (no raw property)', () => {
		...
		expect(rule).toContain('--_textarea-inline-padding: var(--eps-input-padding-x)');
