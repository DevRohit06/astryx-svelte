...
describe('plainDateFromISO / plainDateToISO roundtrip', () => {
	it.each(['2026-01-01', '2026-06-15', '2026-12-31', '2000-02-29', '1999-11-30'])(
		'roundtrips %s',
		(iso) => {
...
describe('plainDateAddMonths', () => {
	...
	// These four replaced two cases that pinned the *overflow*: `Date#setMonth`
	// pushed Jan 31 + 1 month out to Mar 3, so month arithmetic from an
	// end-of-month date skipped February entirely. 0.2.0 clamps instead.
	it('clamps to the last day when the target month is shorter', () => {
		// Jan 31 + 1 month lands in February (clamped), not overflowed into March —
		// matching Temporal.PlainDate.add and date-fns addMonths.
...
describe('plainDateToInstant / plainDateFromInstant', () => {
	...
	it('supports explicit time components', () => {
		expect(
			plainDateToInstant({ year: 2026, month: 5, day: 13 }, 'America/Los_Angeles', 9, 30)
...
describe('plainDateIsBefore / plainDateIsAfter', () => {
	it('returns false for equal dates (isBefore)', () => {
		expect(
			plainDateIsBefore({ year: 2026, month: 1, day: 15 }, { year: 2026, month: 1, day: 15 })
		).toBe(false);
	});

	it('returns false for equal dates (isAfter)', () => {
		expect(
			plainDateIsAfter({ year: 2026, month: 1, day: 15 }, { year: 2026, month: 1, day: 15 })
		).toBe(false);
	});

	it('returns true when first is earlier (isBefore)', () => {
		expect(
			plainDateIsBefore({ year: 2026, month: 1, day: 14 }, { year: 2026, month: 1, day: 15 })
		).toBe(true);
	});

	it('returns true when first is later (isAfter)', () => {
		expect(
			plainDateIsAfter({ year: 2026, month: 1, day: 16 }, { year: 2026, month: 1, day: 15 })
		).toBe(true);
	});

	it('compares by year first', () => {
		expect(
			plainDateIsBefore({ year: 2025, month: 12, day: 31 }, { year: 2026, month: 1, day: 1 })
		).toBe(true);
	});

	it('compares by month when years equal', () => {
		expect(
			plainDateIsBefore({ year: 2026, month: 1, day: 31 }, { year: 2026, month: 2, day: 1 })
		).toBe(true);
	});
});
...
describe('plainDateIsEqual', () => {
	it('returns true for same date', () => {
		expect(
			plainDateIsEqual({ year: 2026, month: 1, day: 15 }, { year: 2026, month: 1, day: 15 })
		).toBe(true);
	});

	it('returns false for different day', () => {
		expect(
			plainDateIsEqual({ year: 2026, month: 1, day: 15 }, { year: 2026, month: 1, day: 16 })
		).toBe(false);
	});

	it('returns false for different month', () => {
		expect(
			plainDateIsEqual({ year: 2026, month: 1, day: 15 }, { year: 2026, month: 2, day: 15 })
		).toBe(false);
	});
});
...
describe('plainDateIsInRange', () => {
	...
	it('returns true for date within range', () => {
		expect(plainDateIsInRange({ year: 2026, month: 1, day: 15 }, [start, end])).toBe(true);
	});
	...
	it('returns false for date before range', () => {
		expect(plainDateIsInRange({ year: 2026, month: 1, day: 9 }, [start, end])).toBe(false);
	});

	it('returns false for date after range', () => {
		expect(plainDateIsInRange({ year: 2026, month: 1, day: 21 }, [start, end])).toBe(false);
	});
});
...
describe('plainDateSetStartOfWeek / plainDateSetEndOfWeekExclusive', () => {
	...
	it('returns the exclusive end of the week', () => {
		expect(plainDateSetEndOfWeekExclusive({ year: 2026, month: 5, day: 13 }, 0)).toEqual({
			year: 2026,
			month: 5,
			day: 17
		});
