...
import {
	...
	parseLocaleNumber
} from '$lib/components/number-input/number-parser.js';

/**
 * Astryx's `NumberInput/numberParser.test.ts`, ported case for case. Upstream
 * at the **0.5.2** pin declares **4** blocks producing **105** cases — the
 * 95-row `it.each` corpus, one `it`, the 8-row `formatEditableNumber` `it.each`,
 * and one more `it` — and all 4 are here. Nothing is dropped.
 *
 * The suite is a pure-module one on both sides: `numberParser.ts` imports only
 * a type, so this is a transcription rather than a translation and the corpus
 * carries over row for row. It runs in the **server** project, which is why the
 * filename has no `.svelte.` segment.
 *
 * Every locale is passed explicitly, so nothing here depends on the ambient
 * default the client project pins in `vite.config.ts`.
 */
...
describe('parseLocaleNumber', () => {
	...
	it('reads back the grouping every locale writes', () => {
		for (const locale of ['en-US', 'de-DE', 'fr-FR', 'de-CH', 'en-IN', 'ar-SA']) {
...
describe('formatEditableNumber', () => {
	...
	it('round trips through the parser in every locale', () => {
		for (const locale of ['en-US', 'de-DE', 'fr-FR', 'de-CH', 'en-IN', 'ar-SA', 'ja-JP', 'hi-IN']) {
			for (const value of [
				0, 1, -1, 0.1, 1.5, -0.5, 1234.56, -1234567.125, 123456789, 3.141592653589793, 1e21, 1.5e-7,
				-9007199254740991
			]) {
				expect(parseLocaleNumber(formatEditableNumber(value, locale), locale)).toBe(value);
			}
		}
	});
});
