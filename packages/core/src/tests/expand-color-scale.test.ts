 ...
 * `hct.ts` and `contrast.ts` end-to-end across ten accent configs in both modes,
 * which is the only in-repo check on that math — no oracle covers it, because
 * all five ported themes declare their colour tokens literally and nothing this
 * repo builds passes through the generator.
 *
...
const ACCENT_TOKENS = ['--color-accent', '--color-accent-muted', '--color-on-accent'] as const;
...
describe('expandColorScale — neutral-only themes (#2279)', () => {
	...
	it('seeds the neutral palettes from the light half of the default accent', () => {
		...
		const seeded = expandColorScale({ accent: DEFAULT_ACCENT, neutralStyle: 'warm' });
...
/**
 * Upstream's three CSS cases below call `generateThemeRules(theme)`, which
 * returns the theme's own scope rules and generates the on-media rules
 * *separately* (its source says so). This port publishes no rules-array
 * equivalent — only `generateThemeCss`, which appends the on-media layer — so
 * strip that layer back off to recover upstream's subject.
 *
 * Not cosmetic: `defaultOnDarkTokens` and `defaultOnLightTokens` both set
 * `--color-accent`, so the neutral-only case's `not.toContain('--color-accent:')`
 * would fail against the full stylesheet for a reason that has nothing to do
 * with the colour expander.
 */
function themeRules(theme: DefinedTheme): string {
	...
	// `generateThemeCss` embeds it via its `indent()` helper — one tab on every
	// non-empty line. Reconstruct that exactly, so the removal is not a guess.
	const embedded = onMedia
		.split('\n')
		.map((line) => (line.length > 0 ? '\t' + line : line))
		.join('\n');
	return css.replace(embedded, '');
}
...
describe('expandColorScale + defineTheme integration', () => {
	it('explicit token overrides win over generated values', () => {
		...
		// Upstream reads `theme.tokens`; this port splits the field, keeping the
		// resolved map on `resolvedTokens` and the raw input on `tokens`.
		expect(theme.resolvedTokens['--color-accent']).toBe('red');
	});
	...
	it('a neutral-only theme leaves the accent tokens at their defaults (#2279)', () => {
		...
		for (const key of ACCENT_TOKENS) {
			expect(theme.resolvedTokens).not.toHaveProperty(key);
		}
		// Neutrals are still themed.
		expect(theme.resolvedTokens['--color-background-surface']).toMatch(/^light-dark\(#/);
	...
	it('an explicit accent token still wins on a neutral-only theme (#2279)', () => {
		...
		expect(resolveThemeTokens(theme, { mode: 'light' })['--color-text-accent']).toBe('#AA0000');

		expect(themeRules(theme)).toContain('--color-text-accent: var(--color-accent);');
	});
});
...
function resolveToken(tokens: Record<string, string>, name: string, mode: Mode): string {
...
describe('expandColorScale WCAG contrast guarantees', () => {
	...
	describe.each(configs)('config %o', (config) => {
		...
		it.each(modes)(
			...
			(_label, mode) => {
				const surface = resolveToken(tokens, '--color-background-surface', mode);
				const body = resolveToken(tokens, '--color-background-body', mode);
				const card = resolveToken(tokens, '--color-background-card', mode);
				const popover = resolveToken(tokens, '--color-background-popover', mode);
				// Alpha tokens paint over the surface — composite before measuring.
				const muted = resolveComposited(tokens, '--color-background-muted', surface, mode);
				const neutral = resolveComposited(tokens, '--color-neutral', surface, mode);

				const textPrimary = resolveToken(tokens, '--color-text-primary', mode);
				const textSecondary = resolveToken(tokens, '--color-text-secondary', mode);
		...
		it.each(modes)('non-text UI tokens meet 3:1 against the surface (%s)', (_label, mode) => {
			const surface = resolveToken(tokens, '--color-background-surface', mode);
			...
			expect(
				contrastRatio(resolveToken(tokens, '--color-icon-primary', mode), surface)
			).toBeGreaterThanOrEqual(3);
			expect(
				contrastRatio(resolveToken(tokens, '--color-icon-secondary', mode), surface)
			).toBeGreaterThanOrEqual(3);
			// Form-control boundaries (CheckboxInput, Selector borders).
			expect(
				contrastRatio(resolveToken(tokens, '--color-border-emphasized', mode), surface)
			).toBeGreaterThanOrEqual(3);
		});
	});
