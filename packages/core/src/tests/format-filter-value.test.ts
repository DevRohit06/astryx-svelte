 ...
 * port transcribes it line for line, so the suite transcribes too. It is a
 * plain `*.test.ts` and therefore runs in the **server** project; a
 * `.svelte.test.ts` would boot Chromium to call one pure function.
 *
 ...
 *
 * `_config` is unread by the function on both sides, so the stub is upstream's
 * `{} as never` — here it stands in for `InternalConfig` rather than upstream's
 * `InternalConfig`, which is the only type-level difference in the file.
 */
...
describe('formatFilterValue', () => {
	...
	describe('string', () => {
		it('returns the string unchanged when within maxLength', () => {
			expect(fmt({ type: 'string' }, { type: 'string', value: 'hello' })).toBe('hello');
		});

		it('truncates with an ellipsis when longer than maxLength', () => {
			const out = fmt({ type: 'string' }, { type: 'string', value: 'abcdefghij' }, 5);
			expect(out).toBe('abcd' + ELLIPSIS);
			expect(out.length).toBe(5);
		});

		it('keeps a string exactly at the boundary intact', () => {
			expect(fmt({ type: 'string' }, { type: 'string', value: 'abcde' }, 5)).toBe('abcde');
		});
	});
	...
	describe('integer / float', () => {
		...
		it('appends integer units only when the operator is an integer', () => {
			expect(fmt({ type: 'integer', units: 'ms' }, { type: 'integer', value: 42 })).toBe('42 ms');
		...
		it('appends float units only when the operator is a float', () => {
			expect(fmt({ type: 'float', units: 'kg' }, { type: 'float', value: 3.5 })).toBe('3.5 kg');
	...
	describe('enum', () => {
		...
		it('truncates a long enum label', () => {
			expect(fmt(enumOp, { type: 'enum', value: 'us' }, 5)).toBe('Unit' + ELLIPSIS);
		});
	});
	...
	describe('string_list', () => {
		...
		it('joins multiple items when they fit', () => {
			expect(fmt(op, { type: 'string_list', value: ['a', 'b', 'c'] })).toBe('a, b, c');
		});
		it('summarizes as a count when the join exceeds maxLength', () => {
			expect(fmt(op, { type: 'string_list', value: ['alpha', 'bravo', 'charlie'] }, 8)).toBe(
				'3 items'
			);
		});
	});
	...
	describe('enum_list', () => {
		...
		it('joins mapped labels when they fit', () => {
			expect(fmt(enumOp, { type: 'enum_list', value: ['r', 'g'] }, 40)).toBe('Red, Green');
		});
		it('summarizes labels as a count when too long', () => {
			expect(fmt(enumOp, { type: 'enum_list', value: ['r', 'g'] }, 5)).toBe('2 items');
		});
		it('uses raw values with a non-enum operator', () => {
			expect(fmt({ type: 'string' }, { type: 'enum_list', value: ['x', 'y'] }, 40)).toBe('2 items');
			expect(fmt({ type: 'string' }, { type: 'enum_list', value: ['solo'] }, 40)).toBe('solo');
		});
	});
	...
	describe('entity_list', () => {
		...
		it('returns a single entity label', () => {
			expect(fmt(op, { type: 'entity_list', value: [{ id: '1', label: 'Ada' }] })).toBe('Ada');
		});
	...
	describe('dates and time', () => {
		...
		it('returns a relative date value verbatim', () => {
			expect(fmt({ type: 'date_relative' }, { type: 'date_relative', value: '7d' })).toBe('7d');
		});
	...
	describe('nested', () => {
		it('uses the singular form for a single nested filter', () => {
			expect(fmt({ type: 'nested' }, { type: 'nested', value: [{} as never] })).toBe('1 filter');
		});
