 ...
 * arithmetic over a config object.
 *
 * `theme.test.ts` previously carried two cases over this module — one pinning
 * neutral's published `theme.css` output, one on the omitted slow band. The
 * second is upstream's `does not emit slow tokens when slow is omitted` and
 * moves here; the first has no upstream counterpart (it asserts against a
 * shipped theme's real CSS rather than against the function's contract) and
 * stays where it is.
 */
describe('expandMotionScale', () => {
	...
	it('produces exactly 6 duration tokens without slow', () => {
		const tokens = expandMotionScale({ fast: 175, medium: 410, ratio: 0.75 });
		const durationKeys = Object.keys(tokens).filter((k) => k.startsWith('--duration-'));
