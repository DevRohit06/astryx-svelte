...
describe('parseDateInput', () => {
	...
	describe('edge cases', () => {
		...
		it('never returns an out-of-range date for partial input', () => {
			...
			// Upstream guards the expect behind `if (result !== null)`. Every one of
			// these inputs is currently rejected, so that branch never runs — and
			// this repo's vitest sets `requireAssertions`, which reads a test with
			// no assertion as a failure. Same check, stated unconditionally.
			for (const input of ['0', '1', '01', '00', '9', '99']) {
				const result = parseDateInput(input);
				expect(result === null || result.year >= 1).toBe(true);
			}
		});
	});
});
