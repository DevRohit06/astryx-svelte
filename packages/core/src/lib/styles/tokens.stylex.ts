 ...
 *
 * Ported from Astryx's `packages/core/src/theme/tokens.stylex.ts`, preserving its
 * group structure and names so theme tooling (`defineTheme`, `createTheme`) can
 * consume these vars the same way upstream does.
 *
 * - `*Defaults` are plain objects of default values, consumed by `defineVars`
 *   and by themes.
 * - `*Vars` are the CSS custom properties themes override via `createTheme`.
 *
 * Keys deliberately start with `--`: StyleX treats such keys as literal custom
 * property names rather than generating hashed ones, which is what keeps the
 * public token surface readable (`--color-accent`, not `--x1abc2de`).
 *
...
export const colorDefaults = {
	...
	// luminance (ProgressBar tracks, Slider rails, Switch off-state). Defaults to
	// the same low-contrast surface as --color-skeleton; themes can override per
	// their neutral ramp if 1-D channels need more weight.
	'--color-track': 'light-dark(#CCD3DB, #5A5E66)',
	'--color-shadow': 'light-dark(rgba(5, 54, 89, 0.1), rgba(0, 0, 0, 0.3))',
	// Hover tint: black in light mode, white in dark — pair with color-mix().
...
// offset. Components read these through utils/focus-outline.stylex.ts; the
...
export const radiusDefaults = {
	...
	// cards in the same view. Tracks --radius-page by default but stays a distinct
	// token so chat rounding can be themed independently.
	'--radius-chat': '28px',
	'--radius-full': '9999px'
} as const;
...
// Shadow
// =============================================================================
// Outer elevation ascends low → med → high. Inset shadows are input state rings.
...
// Motion — duration
// =============================================================================
// Three bands: fast (micro-interactions), medium (entrance/exit), slow
// (continuous). min/max variants derive from base × ratio (default ≈ 0.75).
...
// The deprecated `--transition-fast` / `--transition-normal` shorthand pair was
// REMOVED at upstream 0.3.0's "remove long-deprecated compatibility APIs", along
// with `transitionDefaults` / `transitionVars` / `TransitionVarName`. The
// published 0.3.0 dist carries no `transition` token at all. Nothing here ever
// consumed them and they reached no barrel, so removal is clean — and it takes
// the core token count from 186 to 184, which is what `liquid-glass`'s
// `check-theme.mjs` reads out of the built `dist/`. Use `durationVars` +
// `easeVars`, as the deprecation said.
...
// =============================================================================
// Typography — raw size scale
// =============================================================================
// Geometric scale: round(14 × 1.2^step) / 16, base = 14px, ratio = 1.2.

export const textSizeDefaults = {
	'--font-size-4xs': '0.375rem', // step -5:  6px
	'--font-size-3xs': '0.4375rem', // step -4:  7px
	'--font-size-2xs': '0.5rem', // step -3:  8px
	'--font-size-xs': '0.625rem', // step -2: 10px
	'--font-size-sm': '0.75rem', // step -1: 12px
	'--font-size-base': '0.875rem', // step  0: 14px — base anchor
	'--font-size-lg': '1.0625rem', // step +1: 17px
	'--font-size-xl': '1.25rem', // step +2: 20px
	'--font-size-2xl': '1.5rem', // step +3: 24px
	'--font-size-3xl': '1.8125rem', // step +4: 29px
	'--font-size-4xl': '2.1875rem', // step +5: 35px
	'--font-size-5xl': '2.625rem' // step +6: 42px
} as const;
...
// =============================================================================
// The source of truth for heading and text sizing. Components reference these so
// a theme overriding `typography.scale` moves the raw and semantic tokens
// together. Sizes are var() references to the raw scale; line heights are
// precomputed and snapped to a 4px grid.
//
// Step mapping: h6 = -2, h5 = -1, h4 = 0 (base), h3 = +1, h2 = +2, h1 = +3.

export const typeScaleDefaults = {
	'--text-heading-1-size': 'var(--font-size-2xl)', // 24px → 32px
	'--text-heading-1-weight': 'var(--font-weight-semibold)',
	'--text-heading-1-leading': '1.3333',
	'--text-heading-2-size': 'var(--font-size-xl)', // 20px → 28px
	'--text-heading-2-weight': 'var(--font-weight-semibold)',
	'--text-heading-2-leading': '1.4',
	'--text-heading-3-size': 'var(--font-size-lg)', // 17px → 24px
	'--text-heading-3-weight': 'var(--font-weight-semibold)',
	'--text-heading-3-leading': '1.4118',
	'--text-heading-4-size': 'var(--font-size-base)', // 14px → 20px
	'--text-heading-4-weight': 'var(--font-weight-semibold)',
	'--text-heading-4-leading': '1.4286',
	'--text-heading-5-size': 'var(--font-size-sm)', // 12px → 20px
	'--text-heading-5-weight': 'var(--font-weight-semibold)',
	'--text-heading-5-leading': '1.6667',
	'--text-heading-6-size': 'var(--font-size-xs)', // 10px → 16px
	'--text-heading-6-weight': 'var(--font-weight-semibold)',
	'--text-heading-6-leading': '1.6',

	// body / label / code at base, large at +1, supporting at -1
	...
	// Display continues the geometric progression above h1. Leading is tighter
	// (~1.2) than headings (~1.3) since large text reads better tight, and weight
	// stays normal to balance the visual presence of the size.
