...
import type { Catalog, Locale, MessagesByLocale, Overrides } from './types.js';
import enSource from '../locales/en.json' with { type: 'json' };
import { warnOnce, __resetDevWarnings } from '../utils/dev-warning.js';

/**
 * The shared lookup + ICU formatting core, ported verbatim from Astryx's
 * `i18n/resolve.ts`. There is no React in it, so nothing needed translating.
 *
 * Lookup order:
...
export function resolve(
	...
	// IntlMessageFormat.format returns string | (string | elements) — we only
	// ever pass string values so it will be a string; assert for the type system.
