/**
 * Scrollbar-gutter compensation for a scroll lock, ported from Astryx's
 * `hooks/scrollbarGutter.ts`.
 *
 ...
 *
 * ## Two things about this module in the Svelte port
 *
 * A plain `.ts` module rather than a `.svelte.ts` one: it holds no reactive
 * state and owns no effect. The caller's effect decides when the hold starts,
 * settles and is released, exactly as upstream's `useEffect` does — which is
 * why this is the one piece of the scroll lock that ports across with no
 * translation at all.
 *
 * **It is internal.** Upstream names it in neither `hooks/index.ts` nor the
 * root barrel: `useScrollLock` and `MobileNav` are its only callers. It is
 * therefore absent from this port's barrels too, per the parity rule.
 */
