...
/**
 * A fixed pool of scoped markers, each paired with its reveal style blocks
 * (reveal / conceal, each with a layout-preserved variant) compiled against
 * that marker. Ported from Astryx's `hooks/containerReveal.pool.stylex.ts`.
 *
 * WHY A POOL: `stylex.when.ancestor(':hover', marker)` welds the marker's class
 * into the compiled selector (`.marker:hover *`), so a marker and the styles
 * referencing it are inseparable **at compile time** — there is no runtime knob
 * that re-points a compiled block at a different marker. A single shared marker
 * would therefore leak hover/focus across NESTED reveal containers (an ancestor
 * container's `:hover` matches a descendant container's revealed content).
 * Pre-declaring N markers, each with its own copy of the blocks, lets
 * `useContainerReveal` hand every concurrently mounted container a distinct
 * marker (the free-list lives there), isolating nested containers without the
 * caller authoring any StyleX.
 *
 * Internal to `useContainerReveal` and **barrel-absent**, as upstream keeps it.
 *
 * Upstream GENERATES this file from `scripts/gen-container-reveal-pool.mjs`;
 * the repetition below is that generator's output transcribed. The blocks are
 * identical slot to slot apart from the marker each one names, so if the shape
 * changes it changes 6 times.
 */

export const POOL_SIZE = 6;

export const m0: ReturnType<typeof stylex.defineMarker> = stylex.defineMarker();
...
export const m1: ReturnType<typeof stylex.defineMarker> = stylex.defineMarker();
...
export const m2: ReturnType<typeof stylex.defineMarker> = stylex.defineMarker();
...
export const m3: ReturnType<typeof stylex.defineMarker> = stylex.defineMarker();
...
export const m4: ReturnType<typeof stylex.defineMarker> = stylex.defineMarker();
...
export const m5: ReturnType<typeof stylex.defineMarker> = stylex.defineMarker();
...
// The compiled StyleX style objects carry `when.ancestor` conditionals that the
// public `StyleXStyles` type does not model, so the slot shape is inferred from
// the generated blocks rather than annotated.
export const POOL = [
	{
		marker: m0,
		reveal: s0.reveal,
