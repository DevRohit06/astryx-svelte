...
/**
 * Imperative screen-reader announcements, ported from Astryx's
 * `hooks/useAnnounce.ts`.
 *
 * There is nothing React-shaped left once `useCallback` is dropped — the whole
 * hook is a module-level singleton plus a function — so this is a transcription
 * and the file needs no `.svelte.ts` suffix. `useAnnounce()` is kept as the
 * entry point for parity with upstream's API rather than because it has to be
 * called during component init; it does not.
 *
 * The one edit is the attribute literal, which goes through `dataAttr()` so the
...
const CONTAINER_ATTR = dataAttr('live-region');
 ...
 * Long enough for screen readers to pick the message up and finish reading it;
 * clearing afterwards keeps stale status text out of the accessibility tree for
 * users who browse the DOM later. Each announce resets the timer.
 */
...
function scheduleClear(politeness: AnnouncePoliteness, target: HTMLElement): void {
...
function announceMessage(message: string, politeness: AnnouncePoliteness): void {
	...
	// indefinitely. Scheduled per announce, so a newer announcement always resets
	// the countdown (the delay comfortably outlasts the rAF re-set).
