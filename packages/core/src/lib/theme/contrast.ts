/**
 * Ported from Astryx's `src/theme/contrast.ts`.
 *
 * Dependency-free WCAG 2.x contrast math. Backs the contrast guarantees for
 * generated color tokens (WCAG 1.4.3 text contrast >= 4.5:1, WCAG 1.4.11
 * non-text contrast >= 3:1).
 *
 * Semi-transparent foregrounds are composited over their backdrop in
 * gamma-encoded sRGB space (matching CSS alpha compositing) before the ratio is
 * measured — a translucent token has no contrast of its own, only against what
 * it renders on.
 *
 * The one piece of the color-scale chain that is genuinely new in upstream's
 * 0.3.0 (`expandColorScale.ts` and `hct.ts` both predate v0.2.0). Module-private
 * to the theme layer, as upstream's is: neither `theme/index.ts` nor the root
 * barrel publishes it.
 */

import { parseColor, type RGBA } from '../utils/color.js';
...
/**
 * Composite a (possibly translucent) foreground over an opaque backdrop using
 * standard source-over alpha blending in gamma-encoded sRGB space, matching how
 * CSS paints translucent tokens. Returns an opaque color.
 */
 ...
 * before calling, since its rendered color is unknowable here.
 *
 * @example
 * ```ts
