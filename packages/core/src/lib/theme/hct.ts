/**
 * Ported from Astryx's `src/theme/hct.ts`.
 *
 * Minimal HCT (Hue, Chroma, Tone) color space implementation, itself ported
 * upstream from Google's material-color-utilities (Apache-2.0). Zero runtime
 * dependencies — all the math is self-contained.
 *
 * HCT combines:
 ...
 *
 * Module-private to the theme layer, exactly as upstream's is: neither
 * `theme/index.ts` nor the root barrel publishes it. `expand-color-scale.ts` is
 * the only consumer.
 */

import { formatHex, parseHex } from '../utils/color.js';
...
/**
 * Generate a tonal palette: hex colors at standard tones for a given hue and
 * chroma.
 */
