/**
 * Ported from Astryx's `src/theme/expandColorScale.ts`.
 *
 * Generates color token overrides from a single accent color using the HCT
 * perceptual color model. Only produces tokens that meaningfully derive from the
 * accent — status colors, categorical hues, and fixed tokens (on-dark/on-light)
 * fall through to the token defaults.
 *
 * `accent` is optional: a neutral-only config still gets the full neutral ramp
 * (seeded from the default accent's hue) while the accent tokens themselves fall
 * through to the token defaults, same as the tokens above.
 *
 * WCAG contrast guarantees (asserted in `tests/expand-color-scale.test.ts`):
 * - Text tones are guaranteed >= 4.5:1 against their surfaces by tone spacing
 *   alone — HCT tone is CIE L*, which fixes relative luminance regardless of
 *   hue/chroma, so the fixed tone assignments hold for any accent/neutralStyle
 *   (WCAG 1.4.3).
 * - `--color-border-emphasized` (form-control boundaries) is tone-bumped until
 *   it reaches >= 3:1 against the generated surface (WCAG 1.4.11).
 * - `--color-border`, `--color-skeleton`, and `--color-track` are intentionally
 *   decorative/redundant cues and are NOT held to 3:1 — see the test file for
 *   the rationale.
 *
 * Upstream's `defineTheme` accepts this config as its `color` field. Ours does
 * not yet: `define-theme.ts` records the field as deliberately omitted, so this
...
import { hctToHex, hexToHct, hexWithAlpha, tonalPalette } from './hct.js';
...
export interface ColorScaleConfig {
	/**
	 * Seed accent colour. Everything derives from this.
	 *
	 * Either a single hex (#RRGGBB) used for both colour schemes, or a
	 * `[light, dark]` hex tuple: the light scheme's palettes derive from the
	 * light seed and the dark scheme's palettes from the dark seed.
	 *
	 * Optional. When omitted, the neutral palettes are seeded from the default
	 * accent's hue and the accent tokens (`--color-accent`,
	 * `--color-accent-muted`, `--color-on-accent`) are not generated — they fall
	 * through to the token defaults.
	 */
	accent?: string | [light: string, dark: string];

	/**
	 * Neutral tone warmth. Controls how much of the seed's hue bleeds into
 ...
 * `colorDefaults['--color-accent']` (a test guards the two against drift). Only
 * its hue reaches the output: the accent tokens stay ungenerated, so they keep
 * their default values rather than this seed's derivation.
 */
 ...
 *
 * Used for tokens whose preferred tone is not guaranteed by tone spacing alone
 * (e.g. `--color-border-emphasized`). Because HCT tone is CIE L*, each step moves
 * luminance monotonically, so the loop always terminates — at worst at pure
 * black/white (21:1 against anything mid-range).
 */
 ...
 *
 * Only generates tokens that meaningfully derive from the accent color. Tokens
 * that are convention-bound (status colors, categorical hues,
 * `--color-on-dark`/`on-light`) are NOT generated — they fall through to the
 * token defaults.
 *
 * Without an `accent`, the accent tokens join that fall-through set: the neutrals
 * are seeded from the default accent's hue, and `--color-accent`,
 * `--color-accent-muted` and `--color-on-accent` keep their default values.
 *
 * @example
 * ```ts
...
export function expandColorScale(config: ColorScaleConfig): ColorScaleTokens {
	...
	// Normalise to per-scheme seeds. A string accent (or an absent one) uses the
	// same seed for both halves, which keeps single-seed output identical to the
	// pre-tuple implementation.
	...
	// Palette naming: *L palettes feed the light (first) half of each `ld()` pair
	// below, *D palettes the dark (second) half. With a single seed the D palettes
	// alias the L ones.
	...
	return {
		...
		// to the token defaults, whose `--color-accent` is NOT what the default
		// seed derives: defaulting the seed instead of omitting the tokens would
		...
		...(accent != null
			? {
					...
					// Derived accent tokens reference `--color-accent` instead of
					// baking its resolved hex, so a scoped override of the base token
					// re-accents the whole subtree at runtime. `--color-on-accent`
					// stays baked: it is a contrast computation against the accent,
