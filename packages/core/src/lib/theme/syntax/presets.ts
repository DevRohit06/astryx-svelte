/**
 * Ported from Astryx's `theme/syntax/presets.ts` — 12 community syntax theme
 * presets.
 *
 * Colour values are sourced from each theme's official VS Code extension. All
 * the original themes are MIT licensed and only the colour values are extracted;
 * see `THIRD_PARTY_LICENSES.md` in this directory for full attribution.
 */

import { defineSyntaxTheme, type SyntaxThemeDefinition } from './define-syntax-theme.js';
