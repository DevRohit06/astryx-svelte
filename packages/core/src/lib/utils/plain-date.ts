...
/**
 * Immutable date records and pure date arithmetic — upstream's replacement for
 * scattered `Date` usage across the Calendar hooks.
 */
...
export function plainDateAddMonths(pd: PlainDate, n: number): PlainDate {
	// Pure month arithmetic with day clamping (Temporal.PlainDate.add semantics):
	// Jan 31 + 1 month is Feb 28/29, not Mar 2/3. `Date#setMonth` would overflow
	// the excess days into the following month instead, so month arithmetic from
	// an end-of-month date skipped February entirely.
...
export function plainDateFromInstant(instant: number, timezoneID: string): PlainDate {
	const parts = getTimeZoneParts(instant, timezoneID);
...
export function plainDateIsInRange(pd: PlainDate, range: [PlainDate, PlainDate]): boolean {
...
export function plainDateSetEndOfWeekExclusive(pd: PlainDate, weekStartsOn: number): PlainDate {
...
export function plainDateFormat(pd: PlainDate, options: Intl.DateTimeFormatOptions): string {
 ...
 * `formatInstant` switch and DateInput's display path, so the two never drift
 * for the members they share.
 */
 ...
 * its own `formatInstant` switch (its input is a `Date`, not a `PlainDate`).
 */
export function formatSharedDate(pd: PlainDate, format: SharedDateFormat): string {
