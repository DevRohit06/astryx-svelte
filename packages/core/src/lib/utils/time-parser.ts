/**
 * Time parsing and formatting. Used by the time inputs.
 */
 ...
 */
export function formatISOTime(time: ParsedTime, includeSeconds: boolean = false): ISOTimeString {
 ...
 * - positive if a > b
 */
export function compareTime(a: ISOTimeString | undefined, b: ISOTimeString | undefined): number {
	if (!a && !b) {
		return 0;
	}
	if (!a) {
...
export function adjustTime(
	...
	return formatISOTime({ hour: newHour, minute: newMinute, second: parsed.second }, includeSeconds);
}
