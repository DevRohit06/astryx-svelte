/**
 * @file snap-offsets.ts
 * @input Pure geometry helpers — no runtime framework, no DOM.
 * @output Resolves snap points and converts them to translateY offsets.
 * @position Internal to BottomSheet; consumed by `use-sheet-gestures` and
 *   `bottom-sheet-panel`, tested by `snap-offsets.test.ts`.
 *
 ...
 */
function parseSnapPoint(point: BottomSheetSnapPoint, viewportPx: number): number | null {
