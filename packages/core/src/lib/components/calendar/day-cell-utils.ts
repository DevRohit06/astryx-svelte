...
import { plainDateIsEqual, plainDateIsInRange } from '../../utils/plain-date.js';

/**
 * Ported from Astryx's `Calendar/dayCellUtils.ts`.
 *
 * Every function here is pure and transcribes verbatim — no reactivity, no DOM.
 * That is deliberate upstream: the neighbour logic is broken out of the render
 * path precisely so it can be unit-tested in isolation, and the ported suite
 * keeps that property.
 *
 * None of this is published API. Upstream's `Calendar/index.ts` re-exports
 * `isSameDay`/`isDateInRange`/`getWeekNumber` from `utils.ts` and nothing at all
 * from `dayCellUtils.ts`, so these stay module-private here too.
 */
 ...
 *
 * Outside (adjacent-month) days never receive selection, range, preview, or
 * today state: in the two-month layout the same date renders in both panes, and
 * highlighting the spillover copy would duplicate the selection onto the wrong
 * month's pane (#2715). `isToday` is guarded for the same reason and its own
 * (#4021) — it drives `aria-current="date"`, so an unguarded copy puts that on
 * two cells at once whenever `hasOutsideDays` or the two-month view is on.
 */
...
export function computeDayNeighborContinuity(input: {
	...
	const { week, dayIndex, mode, rangeStart, rangeEnd, previewStart, previewEnd, isDisabled } =
		input;
