/**
 * Ported from Astryx's `NumberInput/numberParser.ts`, new at 0.5.1.
 *
 ...
 * the way utils/date-parser.ts settles an ambiguous date by locale preference.
 ...
 *
 * That paragraph is executable: number-parser.docblock.test.ts implements it as
 ...
 * shape utils/date-parser.ts uses for its `[-/.]` date separators.
 *
 * Module-private, as upstream's is: `NumberInput/index.ts` publishes only the
 * component and its props types, so neither export here reaches the barrel.
 */
...
function readUnder(
	...
	if (group !== '' && intText.includes(group)) {
		...
		if (chunks.length - 1 < minGroups || !chunks.every((chunk) => /^\d+$/.test(chunk))) {
			return null;
		}
...
function toPlainNumber(core: string, locale: Locale | undefined): string | null {
	if (!NUMBER_BODY.test(core)) {
		return null;
	}

	const { group, decimal, primaryGroupSize, secondaryGroupSize } = getLocaleNumberSymbols(locale);
	const inLocale = readUnder(core, group, decimal, primaryGroupSize, secondaryGroupSize, 1);
	if (inLocale != null) {
		return inLocale;
	}

	// The machine format, which is what the field read before this parser
	// existed. It comes after the locale rather than before it — date-parser.ts
...
export function parseLocaleNumber(text: string, locale?: Locale): number | null {
	const normalized = toAsciiDigits(text.normalize('NFKC').replace(INVISIBLES, ''));
	if (normalized == null) {
		return null;
	}
