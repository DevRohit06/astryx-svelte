/**
 * The markdown parser, ported from Astryx's `Markdown/parser.ts`.
 *
 * Pure TypeScript with no React anywhere in it, so this is a transcription
 * rather than a translation: same functions, same order, same regexes, same
 * comments. The one change is `export function parseInline(…)` and friends
 * keeping their overload pairs, which carry the legacy `ReadonlySet<string>`
 * second argument alongside the `ParseOptions` bag.
 */
...
function resolveOptions(arg: ReadonlySet<string> | ParseOptions | undefined): ResolvedOptions {
	if (arg == null) {
		return EMPTY_OPTS;
	}
...
export function parseInline(text: string, sourceIds?: ReadonlySet<string>): InlineNode[];
export function parseInline(text: string, options: ParseOptions): InlineNode[];
export function parseInline(text: string, arg?: ReadonlySet<string> | ParseOptions): InlineNode[] {
...
function transformAutolinks(nodes: InlineNode[]): InlineNode[] {
	...
	for (const node of nodes) {
		...
		} else if (node.type === 'bold' || node.type === 'italic' || node.type === 'strikethrough') {
...
function parseTable(
	...
	const headers: TableCellNode[] = splitTableRow(lines[lineIndex]).map((cell) => ({
...
function parseList(
	...
	const orderedStart = ordered ? lines[startIndex].match(/^ *(\d+)([.)]) /) : null;
	...
	while (index < lines.length && itemPattern.test(lines[index])) {
		...
		if (lookahead > index && lookahead < lines.length && itemPattern.test(lines[lookahead])) {
			loose = true;
			index = lookahead;
		}
	}
...
export function parseMarkdown(input: string, sourceIds?: ReadonlySet<string>): BlockNode[];
export function parseMarkdown(input: string, options: ParseOptions): BlockNode[];
...
function parseMarkdownImpl(input: string, baseOpts: ResolvedOptions): BlockNode[] {
	...
	const opts: ResolvedOptions = linkDefs != null ? { ...baseOpts, linkDefs } : baseOpts;
 ...
 */
function mergeSettledBlocks(prev: BlockNode[], delta: BlockNode[]): BlockNode[] {
...
export function parseMarkdownIncremental(
	...
	const parseOpts: ResolvedOptions = linkDefs.size > 0 ? { ...opts, linkDefs } : opts;
	...
	const unsettledBlocks = unsettledText ? parseMarkdownImpl(unsettledText, parseOpts) : [];
