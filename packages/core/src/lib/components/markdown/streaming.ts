/**
 * Boundary tracking and segment splitting for the streaming fade-in animation,
 * ported from Astryx's `Markdown/streaming.ts`.
 *
 * Pure logic, extracted upstream for testability and transcribed here verbatim.
 * `TextSegment.key` keeps its job: upstream calls it a React reconciliation key,
 * and it is the `{#each … (key)}` key here — a span whose key survives keeps its
 * DOM node, which is what lets the `@starting-style` fade run once instead of
 * restarting on every tick.
 */

/**
 * A segment of text produced by splitting at boundary positions.
 * Each segment is either "settled" (fully visible) or "fading" (has an animation).
 */
export interface TextSegment {
	/** Stable key for keyed rendering. Never changes for a segment's lifetime. */
	key: string;
 ...
 * @param nodeKey - The key for this text node (usually its index)
