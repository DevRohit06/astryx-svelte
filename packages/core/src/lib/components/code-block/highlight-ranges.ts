/**
 * Ported verbatim from Astryx's `CodeBlock/highlightRanges.ts`.
 *
 * Pure DOM, no framework, so this is a transcription rather than a translation.
 * It paints syntax colour with the CSS Custom Highlight API — `Range` objects
 * registered in `CSS.highlights`, styled by `::highlight()` — which keeps the
 * code text as bare text nodes instead of thousands of `<span>`s.
 *
 * `CodeBlock` uses only `applyHighlightRangesChunked`. `Batch` and `Flat` exist
 * for upstream's lab `CodeEditor`, which this port does not ship; they are kept
 * because upstream's `index.ts` publishes them.
 */

import type { SyntaxToken, TokenLine } from './tokenizer.js';
import { ensureHighlightStyles, TOKEN_TYPES } from './highlight-styles.js';
...
export function applyHighlightRangesChunked(
	...
	for (let i = 0; i < codeEl.children.length; i++) {
		...
		if (lineDivs.length > 0 && child.tagName === 'DIV' && !child.hasAttribute('data-line')) {
	...
	function applyChunk(wrapper: Element, index: number) {
		...
		const ranges = applyRangesToContainer(wrapper, tokenLines, chunkLineOffsets[index], resolve);
