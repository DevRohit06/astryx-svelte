/**
 * Ported verbatim from Astryx's `CodeBlock/tokenizer.ts`.
 *
 * A pure module — no DOM, no framework — so the port is a transcription rather
 * than a translation. The language table covers 24 aliases across 10 groups;
 * anything else tokenizes to `[]`, which renders as unhighlighted plain text.
 *
...
function buildLanguagePatterns(lang: string): {
	...
	switch (lang) {
		...
		/**
		 * **No upstream counterpart.** Upstream's table stops at `tsx`/`jsx`
		 * because its examples are TSX; every example in this port is a `.svelte`
		 ...
		 * the two sets are merged and ordered by specificity: comments and
		 * strings first (they can contain anything), then the constructs unique
		 * to Svelte, then tags, then the JS fallbacks. The result is a
		 * highlighter, not a parser: `a < b` in a script block still reads as a
		 * tag open, which is the same approximation the `html` group already
		 * makes.
		 */
		case 'svelte':
			return {
				...
				patterns: [
					...
					{ type: 'string', regex: /"(?:[^"\\]|\\.)*"/ },
					{ type: 'string', regex: /'(?:[^'\\]|\\.)*'/ },
					// Template blocks — `{#if}`, `{:else}`, `{/each}`, `{@render}`,
					// `{@const}`. Matched before the tag and punctuation rules so the
					// brace and marker read as one keyword.
					{ type: 'keyword', regex: /\{[#@:][a-zA-Z]+|\{\/[a-zA-Z]+/ },
					// Runes, including the `.raw` / `.by` / `.id` members. Also covers
					// `$store` references, which are Svelte-specific in the same way.
					{ type: 'constant', regex: /\$[a-zA-Z_$][\w$]*(?:\.[a-zA-Z_$][\w$]*)?/ },
					{ type: 'tag', regex: /<\/[a-zA-Z][\w.-]*\s*>/ },
					{ type: 'tag', regex: /<[a-zA-Z][\w.-]*/ },
					{ type: 'tag', regex: /\/?>/ },
					// Directives — `bind:value`, `onclick`, `use:action`,
					// `transition:fade|local`, `class:active`. Before the generic
					// attribute rule, which stops at the colon.
					{
						type: 'attribute',
						regex: /\b(?:bind|use|in|out|transition|animate|class|style|let|on):[\w|-]+/
					},
					// The `html` group spells this `(?=\s*=)`, which is safe there because
					// it has no script block. Here that would colour the `value` in
					// `let value = $state('')` as an attribute. A real markup attribute
					// never has a space before `=`, and its value always opens with a
					// quote or a brace — requiring that keeps assignments out.
					{ type: 'attribute', regex: /\b[a-zA-Z_:][\w:.-]*(?==["'{])/ },
					{ type: 'number', regex: /\b0[xX][0-9a-fA-F_]+\b/ },
					{ type: 'number', regex: /\b\d[\d_]*\.?[\d_]*(?:[eE][+-]?\d+)?\b/ },
					{ type: 'keyword', regex: JS_KEYWORDS },
					{ type: 'function', regex: /\b[a-zA-Z_$][\w$]*(?=\s*\()/ },
...
export function flatTokensToLines(
	...
	for (const token of tokens) {
		while (lineIdx < lineStarts.length - 1 && token.start >= lineStarts[lineIdx + 1]) {
			lineIdx++;
		}
