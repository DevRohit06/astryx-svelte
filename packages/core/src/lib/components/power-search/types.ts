...
/**
 * Ported from Astryx's `PowerSearch/types.ts`. **Complete as of batch 14** —
 * every declaration upstream's file carries is here.
 *
 * ## Why this file landed a batch ahead of its component
 *
 * It shipped with batch 13, when `PowerSearch` itself was unported, because
 * `useTableFiltering` imports upstream's *types* and nothing else:
 * `UseTableFilteringConfig` carries a `searchConfig: PowerSearchConfig`, and
 * `toSearchFilters` returns `PowerSearchFilter[]`. Both are public, so both type
 * names had to exist and be nameable, exactly as `TableColumn.filter` forced
 * `TableFilterFieldRef` to be nameable before the filtering plugin landed.
 *
 * It was therefore deliberately *partial*: the transitive closure of what the
 * filtering plugin's public surface referenced, and no further. Batch 14 closed
 * the remaining nine, and the split is worth keeping because it is the shape of
 * the deferral rather than an accident:
 *
 * - **Seven describe the component's own API** — `PartialFilter`,
 *   `PowerSearchChangeType`, `PowerSearchHandle`, `PowerSearchTokenProps`,
 *   `PowerSearchEditorProps`, `PowerSearchComponentOverride` and
 *   `PowerSearchComponents`. All seven are on upstream's barrel and are now on
 *   ours.
 * - **Two are module-public-and-unpublished on both sides** —
 *   `PowerSearchAuxData` and `PowerSearchItem`. `usePowerSearchSource` returns
 *   `SearchSource<PowerSearchItem>`, so the name has to exist for it to say so;
 *   they stay **off** `src/lib/index.ts`, because upstream's barrel omits them.
 *   They are the reason that file enumerates this module's names instead of
 *   re-exporting it with `export type *`: a wildcard would have published them
 *   the moment they landed, with no diff to review. That is exactly what the
 *   enumeration bought, and it is why the enumeration stays.
 *
 * ## Translations
 *
 * - **`ReactNode` → `Snippet`** for `EnumItem.icon` and `PowerSearchField.icon`.
 *   Both are caller-authored markup that upstream renders verbatim, so there is
 *   no string branch to discriminate — the `SearchableItem.element` shape, not
 *   the `SelectorOptionData.icon` one, which is `ReactNode | IconType` upstream
 *   and so gained an `IconName` branch.
 *
 *   Batch 14 checked both against how they are actually consumed, which is what
 *   the batch-13 note said was still owed. They come out differently:
 *   **`PowerSearchField.icon` is confirmed** — `PowerSearch.tsx` reads it into a
 *   `ReactNode` local and hands it to `TypeaheadItem.icon`, which is `ReactNode`
 *   upstream and `Snippet` here, rendered raw on both sides. **`EnumItem.icon`
 *   is unverifiable**: no upstream module reads it at all. Every consumer of an
 *   `EnumItem` — `enumItemsToSearchableItems`, `formatEnumLabel`, the token
 *   renderer's `getEnumLabel` — takes `value` and `label` and nothing else. The
 *   `Snippet` choice is consistent with its sibling rather than validated by a
 *   call site, and that is the strongest thing that can be said for it.
 * - **A `ReactNode`-returning render prop → `Snippet<[…]>`.**
 *   `EntityListOperatorValue.renderItem` takes the item it renders, so it is
 *   `Snippet<[SearchableItem]>`; `Tokenizer.renderItem` already has that shape
 *   here.
 * - **`ComponentType<P>` → `Component<P>`** for `CustomOperatorValue.Editor`.
 *   It is a component *constructor* on both sides, not an element. (Upstream
 *   spells this one `React.ComponentType` and the rest `ComponentType`; the
 *   two are the same type.)
 *
 * Everything else transcribes verbatim, `readonly` modifiers included.
 */
...
export interface EntityListOperatorValue {
	...
	readonly renderItem?: Snippet<[SearchableItem]>;
}
 ...
 *
 * `timezoneID` is part of the shape on both sides and is never supplied:
 * `PowerSearch` passes it to neither the default editor nor an override. It
 * reaches only the token text, through `formatFilterValue`.
 */
...
export interface PowerSearchComponentOverride {
	...
	readonly Token?: Component<PowerSearchTokenProps>;
	/** Replaces the full editor popover content for filters of this type. */
	readonly Editor?: Component<PowerSearchEditorProps>;
}
