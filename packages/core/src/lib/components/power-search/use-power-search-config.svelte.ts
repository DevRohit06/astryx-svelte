...
/**
 * Ported from Astryx's `PowerSearch/usePowerSearchConfig.ts`.
 *
 * A field-definition shorthand: seven simple `type`s expand into the full
 * operator sets a `PowerSearchConfig` needs, and the same call hands back an
 * `applyFilters` that evaluates the resulting filters against plain data. Both
 * halves are published.
 *
 * **`createPowerSearchConfig` is the whole implementation and transcribes
 * verbatim** — operator order, i18n keys, default operators, the match table and
 * every guard. `usePowerSearchConfig` is upstream's `useMemo` over it and
 * nothing else: no context, no state, no effect.
 *
 * ## What changes
 *
 * - **`usePowerSearchConfig` takes getters**, as every hook in this port does,
 *   and returns getters. Upstream's memo keys on the *reference* identity of
 *   `definitions`/`configName`, so a caller passing a new array gets a new
 *   config; a `$derived` over a getter is that statement in Svelte. Every
 *   upstream call site hoists `fieldDefs` to module scope, so in practice the
 *   derived recomputes never — which is exactly upstream's behaviour too.
 * - **Do not destructure the result.** `const {config, applyFilters} = …`
 *   snapshots both and stops tracking, the `useTheme()` hazard already recorded
 *   for `useThemeHookUsage`. `createPowerSearchConfig` is the plain-object
 *   escape hatch for callers who want to, and it is published upstream for
 *   exactly that reason.
 * - **`FieldType` and `FieldTypeToJS` stay unexported.** Upstream's `.d.ts`
 *   emits `export {}` to keep them unnameable, so a consumer can write
 *   `FieldDefinition` and not the type union inside it.
 *
 * ## One upstream bug, fixed rather than replicated
 *
 * `matchesFilter`'s dispatch is `if (operator in stringOpHandlers)`, and `in`
 * walks the prototype chain — so a filter whose `operator` is `'toString'`,
 * `'valueOf'` or `'propertyIsEnumerable'` resolves an `Object.prototype` method
 * and *calls* it as a comparator. It is unreachable through the typed API and
 ...
 * the divergence is recorded in port/debts.md's Known debts.
 */
...
function matchesFilter(row: Record<string, unknown>, filter: PowerSearchFilter): boolean {
	...
	switch (filterValue.type) {
		...
		case 'string': {
			...
			// `Object.hasOwn`, where upstream writes `operator in stringOpHandlers`
			// — see the header note on the prototype-chain bug.
			if (Object.hasOwn(stringOpHandlers, operator)) {
				return stringOpHandlers[operator as keyof typeof stringOpHandlers](s, target);
			}
			return true;
		}
		...
		case 'float': {
			...
			if (Object.hasOwn(numberOpHandlers, operator)) {
				return numberOpHandlers[operator as keyof typeof numberOpHandlers](n, target);
			}
			return true;
		}
...
/**
 * Hook that re-derives {@link createPowerSearchConfig} when its definitions change.
 *
 * **Read the members off the result; do not destructure it.** Both are getters,
 * so `const {config} = usePowerSearchConfig(…)` snapshots the config and stops
 * tracking. Callers who want a plain object should call
 * `createPowerSearchConfig` directly — that is what it is published for.
 *
 * @example
 * ```
 * const defs = [
 ...
 * ] as const;
 *
 * const search = usePowerSearchConfig(() => defs);
 * const filtered = search.applyFilters(filters, data);
 * ```
 */
export function usePowerSearchConfig<const D extends ReadonlyArray<FieldDefinition>>(
	...
	const built = $derived(createPowerSearchConfig(definitions(), configName?.()));
