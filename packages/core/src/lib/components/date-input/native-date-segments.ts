/**
 * Ported from Astryx's `DateInput/nativeDateSegments.ts`. There is no React in
 * it — it touches the DOM and nothing else — so the transcription is verbatim.
 *
...
function probeEngine(): EngineKind {
	...
	try {
		...
		if (pseudoIsRealBox(input, style, '::-webkit-datetime-edit', base)) {
			...
			kind = 'segmented';
		} else if (pseudoIsRealBox(input, style, '::-webkit-date-and-time-value', base)) {
