...
/**
 * Ported from Astryx's `DateInput/monthGeometry.ts`.
 *
 ...
 * to be wrong, and it is testable without a layout engine. `src/tests/
 * month-geometry.test.ts` is that test, and it runs in the **server** project
 ...
 */
export function scrollOffsetForRow(row: number, paneSize: number, isRTL: boolean = false): number {
 ...
 */
export function paneWindow(centerRow: number, rowCount: number, overscan: number): PaneWindow {
	return {
