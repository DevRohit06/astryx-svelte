...
export default {
	...
	usage: {
		...
		bestPractices: [
			{
				guidance: true,
				description: 'Use presets for common ranges like "Last 7 days" to speed up selection.'
			},
			{
				guidance: true,
				description: 'Use min/max to constrain selectable dates to valid ranges.'
			},
			{
				guidance: true,
				description: 'Keep hasClear enabled (default) so users can reset the filter.'
			},
			{
				guidance: true,
				description:
			...
			{
				guidance: false,
				description: 'Use DateRangeInput when only a single date is needed; use DateInput instead.'
			},
			{
				guidance: false,
				description: 'Hide the label without surrounding context that makes the purpose obvious.'
			},
			{
				guidance: false,
				description:
		...
		anatomy: [
			{
				name: 'Label',
				required: true,
				description: 'Text above the trigger describing what date range is expected.'
			},
			{
				name: 'Trigger button',
				required: true,
			...
			},
			{
				...
				required: true,
				description: 'A dual-month calendar grid with range selection and hover preview.'
			},
			{
				name: 'Preset sidebar',
				required: false,
	...
	props: [
		...
		},
		{
			...
			type: 'DateRange | null',
			description:
				'Selected date range or null. Import the `DateRange` type from `@astryx-svelte/core`; it is `{start: ISODateString, end: ISODateString}`. Do NOT redeclare your own DateRange type; use the exported one so TypeScript structurally matches.',
			required: true
		},
		{
			name: 'onChange',
			type: '(value: DateRange | null) => void',
			description: 'Callback when the range changes. Called with null on clear.',
			required: true
		},
		{
			name: 'changeAction',
		...
		},
		{
			...
			type: 'ISODateString',
			description:
				'Minimum selectable date. `ISODateString` is a template literal type (`\\`${number}${number}${number}${number}-${number}${number}-${number}${number}\\``). Pass a string literal like `"2026-01-28"`, not a runtime string variable. Import it from `@astryx-svelte/core` or use `as ISODateString` if computing the value dynamically.'
		},
		{
			name: 'max',
			type: 'ISODateString',
