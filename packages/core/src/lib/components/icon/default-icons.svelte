<!--
	Built-in fallback icons, ported from Astryx's `src/Icon/defaultIcons.tsx`.

	Deliberately minimal inline SVGs that give basic visual completeness with no
	icon-library dependency. Themes are expected to override them with something
	better (lucide, heroicons, Material Symbols).

	This file is a snippet library, not a component — its default export renders
	nothing. Top-level snippets compile to module-scope functions, so the module
	script can collect them into the registry record directly.

	All icons use a 24×24 viewBox, `currentColor`, and `1em` sizing so the span's
	font-size drives them. Status icons (success, error, warning, info) are solid
	rather than stroked, which reads better at small sizes.
-->
...
<!-- ☰ — hamburger menu. Heavier stroke than the rest, as upstream. -->
{#snippet menu()}
	<svg
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
...
<!-- ↕ — arrows up-down (sortable indicator) -->
{#snippet arrowsUpDown()}
	<svg
		xmlns="http://www.w3.org/2000/svg"
		viewBox="0 0 24 24"
		fill="none"
