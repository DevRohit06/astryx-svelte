...
/**
 * The shared layer dismissal stack, ported from Astryx's `Layer/layerStack.ts`
 * (upstream 0.5.0, #4881): registration, top-most ordering, and the single
 * document-level Escape listener.
 *
 * Almost none of this is framework code — it is module state, a comparator and
 * one DOM listener — so it transcribes rather than translates. Upstream's
 * structure and names are kept deliberately; the only edits are to the comments
 * that name React mechanics, and each of those hazards exists here too (see
 * `compareEntries`).
 *
 * Internal to the Layer system: `use-layer-dismissal.svelte.ts` is what overlays
 * actually call, and nothing here reaches the package barrel — upstream does not
 * export it from `Layer/index.ts` either.
 *
 * ## Why one stack
 *
...
export interface LayerStackEntry {
	...
	token: object;
	/**
	 * Nesting depth from the component tree (see `layer-depth-context.ts`), NOT
	 * the DOM. Context flows through portals and is known during initialisation,
	 * so it is correct for layers that portal out and for an inner + outer layer
	 * that open in the same flush — the case where effect order lies, because
	 ...
	 * break ties between layers the component tree reports at the same depth — a
	 * raw `useFocusTrap` cannot wrap its content in a depth provider (it renders
	 ...
	 *
	 * Registration alone is not proof: a layer whose open state lives in
	 * component state can lag a frame behind the DOM, so gating registration on
	 * that state drops presses that arrive in the gap. Such layers register for
	 * their whole lifetime and answer this from the DOM instead. Omit it to mean
	 * "registered means present", which is true for layers that register on open.
	 */
	isPresent?: () => boolean;
	dismiss: () => void;
}
...
// depth or behavior changes — a Dialog whose `purpose` flips while open, a focus
// trap that moves. Counting registrations instead would hand the layer a fresh,
 ...
 *    an inner-and-outer pair that mount in the SAME flush come out right;
 *    registration order alone gets it backwards, because child effects run
 *    before parent effects. Upstream's reason is React's commit order; Svelte's
 *    effect ordering produces the same hazard, which is why the port of
 *    `useFocusTrap` already had to reach for containment for the same reason.
