/**
 * Tooltip line formatting for Timestamp, ported from Astryx's
 * `src/Timestamp/tooltipEntries.ts`.
 *
...
function resolveTimezoneID(timezoneID: string | undefined): string | undefined {
	...
	// `value` is handled in `timestamp.svelte`.
	try {
		...
		if (!warnedTimezoneIDs.has(timezoneID)) {
			...
			devWarn(
				'Timestamp',
				`unknown time zone ${JSON.stringify(timezoneID)} in tooltipEntries. ` +
					"Falling back to the viewer's time zone."
			);
		}
		return undefined;
	}
