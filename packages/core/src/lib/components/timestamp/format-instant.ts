/**
 * Absolute rendering for Timestamp, ported from Astryx's
 * `src/Timestamp/formatInstant.ts`.
 *
 ...
 * from how many zones are on screen (see `tooltip-entries.ts`). Only the
...
export interface FormatInstantOptions {
	 ...
	 * took the identifier from a consumer, not here (`tooltip-entries.ts`
	 * resolves and warns in `resolveTimezoneID`, then passes only what
	 * survived).
	 */
	timeZone?: string;
	/**
