<script lang="ts">
	...
	let taReadOnly = $state('Generated by the importer; edit the source file to change it.');
...
It preserves whitespace and line breaks.
  ...
  Indentation is maintained.
	...
	let niFormatted = $state<number | null>(1234.56);
	let niSteppers = $state<number | null>(2);
	let niWheelEnabled = $state<number | null>(2);
	let niWheelDisabled = $state<number | null>(2);
	let niReadOnly = $state<number | null>(1024);
	let niVariantAttached = $state<number | null>(-5);
	let niVariantDetached = $state<number | null>(-5);
	const niCurrency = new Intl.NumberFormat('en-US', { style: 'currency', currency: 'USD' });
	...
	// SearchableWithSections — filtering keeps group headers and drops empty
	// groups, which is what the 0.4.1 search panel exists to show.
	const SELECTOR_SEARCH_SECTIONS: SelectorOptionType[] = [
		{
			type: 'section',
			title: 'Citrus',
			options: [
	...
	const SELECTOR_PLACEMENT_OPTIONS = ['Apple', 'Banana', 'Cherry', 'Date'];
	...
	let selSearchSections = $state<string | undefined>(undefined);
	// Placements — the three-placement block #5003 added alongside the standard
	// menu clearance, so each one is visibly clear of the trigger.
	let selPlacementBelow = $state<string | undefined>('Banana');
	let selPlacementStart = $state<string | undefined>('Banana');
	let selPlacementEnd = $state<string | undefined>('Banana');
	// RadioSelectionIndicator / DefaultSelectionIndicator / StartIndicatorPosition
	// — the 0.4.1 indicator stories. `check` is the indicator every
	// single-selection mark draws, so replacing it once in the theme reaches this
	// Selector and every other component that marks "this one is chosen".
	const radioSelectionTheme = defineTheme({
		name: 'radio-selection-demo',
		indicators: { check: RadioIndicator }
	});
	let selRadioIndicator = $state<string | undefined>('Banana');
	let selDefaultIndicator = $state<string | undefined>('Banana');
	let selStartIndicator = $state<string | undefined>('Banana');
	...
	// EndIndicatorPosition — the checkbox on the trailing edge of each row.
	let msEndIndicator = $state<string[]>(['Name', 'Email']);
	...
	let diMondayFirst = $state<ISODateString | undefined>(undefined);
	...
	let tiReadOnly = $state('ACCT-4417-9920');
					...
					<section id="progressbar" data-section="progressbar">
						...
						<VStack gap={4} maxWidth={420}>
							...
							<!--
								...
								`height` on it with `defineTheme`, and a taller height overhangs the bar
								symmetrically. Upstream shows the effect without a full theme by injecting
								a raw `<style>` block for `.astryx-progressbar-mark` in the shape
								`astryx theme build` emits — inside `@layer astryx-theme`, with the size
								arriving as the derived vars rather than as `width`/`height` — so the demo
								exercises the real theming path. The counterpart here is a `:global` rule
								in this page's own stylesheet, since Svelte scopes styles to the component
			...
			-->
					...
					<section id="numberinput" data-section="numberinput">
						...
						<div class="field-column">
							...
							<NumberInput
								...
								min={0}
								step={0.01}
								formatValue={(v: number) => niCurrency.format(v)}
								value={niFormatted}
								onChange={(v: number | null) => (niFormatted = v)}
							/>
							<NumberInput
								label="Quantity"
								min={0}
								...
								hasNumberSteppers
								value={niSteppers}
								onChange={(v: number | null) => (niSteppers = v)}
							/>
							<NumberInput
								label="Allocated storage (GB)"
								isReadOnly
								value={niReadOnly}
								onChange={(v: number | null) => (niReadOnly = v)}
							/>
							<NumberInput
								label="Attached (default)"
								status={{ type: 'error', message: 'Must be a positive number' }}
								value={niVariantAttached}
								onChange={(v: number | null) => (niVariantAttached = v)}
							/>
							<NumberInput
								label="Detached"
								status={{ type: 'error', message: 'Must be a positive number' }}
								statusVariant="detached"
								value={niVariantDetached}
								onChange={(v: number | null) => (niVariantDetached = v)}
							/>
						</div>
						<div
							style="width: 360px; height: 260px; overflow-y: auto; padding: 16px; border: 1px solid var(--color-divider);"
						>
							<div style="height: 140px;">Scroll down to the focused examples.</div>
							<div class="field-column">
								<NumberInput
									label="Wheel stepping enabled (default)"
									description="Focus, then use the wheel to change the value"
									value={niWheelEnabled}
									onChange={(v: number | null) => (niWheelEnabled = v)}
								/>
								<NumberInput
									label="Wheel stepping disabled"
									description="Focused wheel gestures continue scrolling"
									isWheelEnabled={false}
									value={niWheelDisabled}
									onChange={(v: number | null) => (niWheelDisabled = v)}
								/>
							</div>
							<div style="height: 180px;"></div>
						</div>
						<p class="note">
							The control is a <strong>text-backed spinbutton</strong> —
							<code>type="text"</code> with <code>role="spinbutton"</code> and
							<code>aria-valuemin</code>/<code>valuemax</code>/<code>valuenow</code>/<code
								>valuetext</code
							>, not <code>type="number"</code>. That is what lets <code>formatValue</code> show
							<code>$1,234.56</code>
							at rest and the raw number on focus, and what makes the stepping arithmetic the component's
							own. Typing and stepping treat <code>min</code>/<code>max</code>
							differently:
							<code>onChange</code>
							fires only for typed values that pass <code>min</code>/<code>max</code>/<code
								>isIntegerOnly</code
							>, and an out-of-range entry dims, announces itself and reverts on blur — where a step
							<em>clamps</em> and then no-ops at the boundary, which is also what greys out one
							stepper at a time. <code>step</code> is never a validation constraint; it drives the
							arrow keys, the wheel and the steppers, and falls back to <code>1</code> when unset.
						</p>
					</section>
					...
					<section id="selector" data-section="selector">
						...
						<div class="field-column">
							...
							/>
							<!--
								SearchableWithSections — filtering keeps group headers and drops
								empty groups.
							-->
							<Selector
								label="Fruit"
								hasSearch
								options={SELECTOR_SEARCH_SECTIONS}
								value={selSearchSections}
								onChange={(v: string) => (selSearchSections = v)}
								placeholder="Select a fruit..."
							/>
						</div>
						<!--
							RadioSelectionIndicator — swap the single-selection indicator for a
							radio. `check` is the indicator every single-selection mark draws, so
							replacing it once in the theme reaches this Selector — and any other
							component that marks "this one is chosen" — without touching a call
							site.

							Note what the default check could never do: an unselected row draws an
							*empty circle*. The mark is rendered in every state and told which
							state to draw, so an indicator that has an unselected form can show it.
						-->
						...
						<div class="field-column">
							<Theme theme={radioSelectionTheme} mode="light">
								<Selector
									...
									options={['Apple', 'Banana', 'Cherry']}
									value={selRadioIndicator}
									onChange={(v: string) => (selRadioIndicator = v)}
									isDefaultOpen
								/>
							</Theme>
							<!--
								DefaultSelectionIndicator — the same Selector with no theme, for
								comparison: a checkmark on the selected row, and nothing at all on
								the others.
							-->
							<Selector
								label="Single selection drawn as a check (default)"
								options={['Apple', 'Banana', 'Cherry']}
								value={selDefaultIndicator}
								onChange={(v: string) => (selDefaultIndicator = v)}
								isDefaultOpen
							/>
							<!--
								StartIndicatorPosition — `indicatorPosition="start"` moves the mark
								to the leading edge, the way a native menu marks its chosen row.

								The column is reserved on every row, not just the chosen one, so the
								labels stay on one line — the default check draws nothing when
								unchecked, and without the column only the chosen label would be
								indented.
							-->
							<Selector
								label="Mark at the start"
								options={['Apple', 'Banana', 'Cherry']}
								value={selStartIndicator}
								onChange={(v: string) => (selStartIndicator = v)}
								indicatorPosition="start"
								isDefaultOpen
							/>
						</div>
						<!--
							Placements — `below`/`start`/`end`, each clearing the trigger by the
							standard menu gap (#5003). An explicit `placement` opts out of the
							selected-item overlay, which is the mode that has no gap by design.
						-->
						<h3>Placements</h3>
						<div class="field-column">
							<Selector
								label="placement=below"
								options={SELECTOR_PLACEMENT_OPTIONS}
								value={selPlacementBelow}
								onChange={(v: string) => (selPlacementBelow = v)}
								placement="below"
							/>
							<Selector
								label="placement=start"
								options={SELECTOR_PLACEMENT_OPTIONS}
								value={selPlacementStart}
								onChange={(v: string) => (selPlacementStart = v)}
								placement="start"
							/>
							<Selector
								label="placement=end"
								options={SELECTOR_PLACEMENT_OPTIONS}
								value={selPlacementEnd}
								onChange={(v: string) => (selPlacementEnd = v)}
								placement="end"
							/>
						</div>
					...
					<section id="multiselector" data-section="multiselector">
						...
						<!--
							EndIndicatorPosition — `indicatorPosition="end"` moves the checkbox to
							the trailing edge of each row. The default is `start`, where the
							checkbox leads the label as it does in CheckboxList.

							No `hasSelectAll`: its divider was an unallowed listbox child and
							failed the a11y audit as soon as a story opened the popup (#4994).
						-->
						...
						<div class="field-column">
							<MultiSelector
								...
								value={msEndIndicator}
								onChange={(v) => (msEndIndicator = v)}
...
<style>
	...
	/* `ProgressBar`'s ThemedMarks story, in the shape `astryx theme build` emits:
	   the `astryx-theme` layer, and the size arriving as the derived vars the
	   registry expands `width`/`height` into rather than as the properties
	   themselves — which is what lets the override land even where a consumer's
	   StyleX is unlayered. `:global` because the mark is ProgressBar's element,
	   not this page's. */
	@layer astryx-theme {
