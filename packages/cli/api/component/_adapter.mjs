...
/**
 * Locate `@astryx-svelte/core`, throwing the stable not-found error when absent.
 * @param {string} cwd
 * @returns {string}
 */
 ...
 * @param {string} packageName - e.g. '@acme/widgets'
 * @param {string} cwd
 ...
 * @param {string} dirName - bare component name
 * @param {import('../../foundation/integrations/integrations.mjs').LoadedIntegration[]} loadedIntegrations
 ...
 * @param {string} dirName - bare component name
 * @param {{coreDir: string, cwd: string, name: string}} ctx - `name` is the caller's original input, used in error text
 ...
 *
 * Ported whole and, against this port's core, unreachable: every export has its
 * own doc, so **0 of 209 carry a `components` array** for a member to be found
 * in. It stays because the shape is authored, not generated — an integration or
 * a future multi-component core doc re-arms it — and because deleting it would
 * silently change what `component Code` does the day one appears.
 *
 * The one edit is that upstream also matches a member literally named
 * `XDS<Name>`; that prefix is residue of upstream's own rename and no doc in
 * this port has ever carried it, so only the bare name is matched.
 * @param {LoadedComponentDoc} docs
 * @param {string} dirName
 * @param {string} coreDir
...
export function scopeSubComponent(docs, dirName, coreDir) {
	...
	const matchingComponent =
		isParentDoc && docs.components ? docs.components.find((c) => c.name === dirName) : null;
