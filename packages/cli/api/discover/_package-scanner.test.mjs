 ...
 *
 * Ported case for case: 6/6, verbatim apart from the fixture directory
 * prefixes, which follow this repo's `.astryx-*` transient-fixture convention.
 */
...
import { scanDirectory, scanAllPackages, findComponentInPackages } from './_package-scanner.mjs';
 ...
 * @param {string} root
 * @param {string} name
 * @param {{astryx?: unknown, docs?: Record<string, string>}} [opts]
 */
...
describe('scanDirectory — adversarial node_modules', () => {
	it('skips a package with a non-string astryx.docs instead of crashing', () => {
		tmp = fs.mkdtempSync(path.join(os.tmpdir(), '.astryx-ps-nonstr-'));
		writePkg(tmp, 'good', {
			astryx: { docs: 'docs' },
	...
	it('skips a docs dir that escapes its own package (../.. traversal)', () => {
		tmp = fs.mkdtempSync(path.join(os.tmpdir(), '.astryx-ps-escape-'));
	...
	it('coerces a non-string package name to a string', () => {
		tmp = fs.mkdtempSync(path.join(os.tmpdir(), '.astryx-ps-name-'));
	...
	it('discovers a well-formed package with nested docs (skips node_modules)', () => {
		tmp = fs.mkdtempSync(path.join(os.tmpdir(), '.astryx-ps-ok-'));
		writePkg(tmp, 'ds', {
			astryx: { docs: 'docs' },
			docs: {
...
describe('scanAllPackages + findComponentInPackages', () => {
	it('dedups by name and finds a component case-insensitively', () => {
		tmp = fs.mkdtempSync(path.join(os.tmpdir(), '.astryx-ps-all-'));
		writePkg(tmp, 'ds', {
			astryx: { docs: 'docs' },
