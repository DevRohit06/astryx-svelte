 ...
 *
 * Verbatim from upstream. A discoverable package is one whose `package.json`
 * declares an `astryx.docs` directory, and what it is scanned for is
 * `*.doc.mjs` — both framework-neutral. The `astryx` field keeps upstream's
 * spelling deliberately (see foundation/fs/paths.mjs), so a third-party
 * authoring contract is not forked.
 */
