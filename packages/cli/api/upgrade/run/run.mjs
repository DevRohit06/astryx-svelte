 ...
 * write in the pipeline — codemod output, the agent-docs block, the post-codemod
 * hooks' commands — is gated on the same flag. That is the whole safety model of
 * the command: a user who runs it wrong sees a preview.
 */
...
import { statusUpToDate, statusNoCodemods, statusConfigFixable } from '../status/status.mjs';
 ...
 *
 * @param {import('../upgrade.type.mjs').UpgradeOptions} [options]
...
export async function run(options = {}, { cwd = process.cwd() } = {}) {
	...
	if (!installed) {
		const msg = `Could not find installed @astryx-svelte/core. Install the target version first, then rerun \`${getCliInvocation()} upgrade --from <old-version>\`.`;
		logger.error(msg);
		logger.log('Aborted\n');
	...
	// independent of codemods, so refresh on every path.
	...
	if (!ready) {
		const msg = 'svelte is required to run codemods but could not be installed.';
		logger.log('Aborted\n');
	...
	const mergedFilesChanged =
		(coreResult?.totalFilesChanged ?? 0) + (integrationResult?.totalFilesChanged ?? 0);
	const mergedTransformsApplied =
		(coreResult?.totalTransformsApplied ?? 0) + (integrationResult?.totalTransformsApplied ?? 0);
