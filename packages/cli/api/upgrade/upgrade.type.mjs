 ...
 * ---------------------------------------------------------------------------
 * astryx-svelte --json upgrade --list                 -> upgrade.list
 * astryx-svelte --json upgrade [--apply]              -> upgrade.run
 * astryx-svelte --json upgrade (status short-circuit) -> upgrade.status
 * (version detection failure)                         -> CLIError
 */

/**
 * astryx-svelte --json upgrade --list
...
/**
 * astryx-svelte --json upgrade [--apply]
...
/**
 * astryx-svelte --json upgrade — short-circuit status results.
 *
 ...
 * - `config_fixable`: DRY-RUN ONLY. The consumer's astryx-svelte.config
 *   currently fails strict validation, but a pending core CONFIG codemod (in
 *   the selected range) would repair it. The dry run previews the fix without
 *   writing and reports the exact command to apply it; integrations are skipped
 *   for the preview (they will be processed on the `--apply` run).
 *
 ...
 * @property {boolean} [installDeps] Auto-install the codemod parser (`svelte`) without prompting.
