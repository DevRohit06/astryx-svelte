 ...
 * caller stays quiet.
 *
 * ## The runner underneath is not upstream's
 *
 * Upstream drives jscodeshift. jscodeshift cannot parse `.svelte`, so this port
 * replaces it with `magic-string` (editing) + `svelte/compiler` (parsing and the
 * re-parse corruption guard). Nothing about *this* file changed as a result —
 * the flag surface, the validation order, the status short-circuits and the
 * receipt shape are all upstream's. See `assets/codemods/runner.mjs` and
 * `assets/codemods/svelte-parser.mjs` for what the substitution can and cannot
 * do.
 */
