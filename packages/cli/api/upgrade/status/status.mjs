 ...
 *
 * The `formatCliCommand` below is one of the port's four call sites — upstream
 * calls it here too. `suggestedCommand` itself stays the bare-bin spelling, so
 * the machine-readable field is stable while the printed line is install-aware;
 * that split is upstream's and is what its own test asserts.
 */
...
/**
 * DRY-RUN only: the consumer's astryx-svelte.config fails strict validation, but
 * a pending core CONFIG codemod previewed a change that would repair it. Preview
...
export function statusConfigFixable({ from, to, configError, configCodemods, agentDocs }) {
	...
	const suggestedCommand = `astryx-svelte upgrade --from ${from} ${codemodFlags} --apply`;
	const guidance =
		'Your astryx-svelte.config currently fails strict validation, but a pending ' +
