/**
 * @file Unit tests for `checkGhCli()` — the only export of `_github.mjs`.
 * Ported case-for-case from upstream's `api/swizzle/_github.test.mjs` — 4
 * cases, 4 here.
 *
 ...
 * helper injection-free, and the one that also keeps it spawnable on Windows,
 * where Node has refused to run a `.cmd` shim without a shell since
 * CVE-2024-27980. `gh` is a real `.exe`, so the static array works unchanged.
 */
