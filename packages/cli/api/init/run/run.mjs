 ...
 * `astryx-svelte init`. It installs the AGENTS.md/CLAUDE.md agent-docs cheat
 * sheet, points at the theme + page-building workflows, and (optionally)
 * scaffolds a starter template — with NO prompts, so it behaves identically for
 * humans, agents, CI, and piped I/O. It performs the side effects and returns an
 * `init.run` receipt. Hard errors (unknown feature/template/agent) throw
 * AstryxError with a stable code. Human output is emitted through the shared
 * `logger` (silent by default) so the CLI keeps its exact plain output and a
...
/**
 * The scaffolded page file. Upstream writes `src/pages/<name>/page.tsx` (Next.js
 * pages router); a SvelteKit route is `src/routes/<name>/+page.svelte`. Both
 * halves change, so both are named here rather than inlined at the call site.
 */
const TEMPLATE_ROUTES_DIR = path.join('src', 'routes');
const TEMPLATE_PAGE_FILE = '+page.svelte';

/**
 * Read the bundled page-template names.
 *
 * Upstream imports `listTemplates` from `api/template/template.mjs` statically.
...
async function loadTemplateNames() {
	try {
		const mod = await import('../../template/template.mjs');
...
/**
 * Build the "Next steps" lines printed at the end of `astryx-svelte init`.
 *
 * Theme guidance must match the runtime recommendation emitted by core's
 * `<Theme>` component (`packages/core/src/lib/theme/theme.svelte`), which names
 * the bare package import plus its `theme.css`. **This port's theme packages
 * publish no `/built` subpath** — their exports are `.`, `./tokens` and
 * `./theme.css` — so upstream's "use the `/built` path" advice would name a
 * specifier that does not resolve. Same property, opposite literal.
 *
 * **Upstream's step 1 — "Ensure the @stylexjs/stylex peer dependency is met" —
 * is dropped, not adapted.** `@stylexjs/stylex` is a plain `dependency` of
 * `@astryx-svelte/core`, never a peer, so there is nothing for a consumer to
 * ensure and `doctor` has nothing to verify. The fact that *does* matter here —
 * a consumer's bundler must run the StyleX compiler over core — is a build-setup
 * instruction, and it lives in the agent-docs SETUP block that this same command
 * installs, where it is regenerated on every run.
 *
 * @param {string} invocation install-aware CLI invocation stem (e.g. `npx astryx-svelte`, `pnpm exec astryx-svelte`, or `npx @astryx-svelte/cli` for one-off runs)
...
export function getNextSteps(invocation) {
	return [
		'',
		'  Next steps:',
		"    1. Import base styles: @import '@astryx-svelte/core/base.css'",
		'       in your global stylesheet — it contains the reset and the layer order',
		"    2. Add a theme: @import '@astryx-svelte/theme-neutral/theme.css'",
		"    3. Import components: import { Button } from '@astryx-svelte/core'",
		'    4. Optionally drive the theme from markup:',
 ...
 * @param {import('../init.type.mjs').InitRunData} data
 */
async function applyTemplate(cwd, { templateName }, invocation, data) {
	const templates = await loadTemplateNames();
	...
	const outputDir = path.resolve(cwd, TEMPLATE_ROUTES_DIR, templateName);
	...
	const destFile = path.join(outputDir, TEMPLATE_PAGE_FILE);
	...
	logger.log(`[ok] Template created at ${rel}/${TEMPLATE_PAGE_FILE}`);
