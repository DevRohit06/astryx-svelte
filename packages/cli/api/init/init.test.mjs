/**
 * @file Direct-API tests for init(). Ported case-for-case from upstream's
 * `api/init/init.test.mjs` — 14 cases, 14 here (12 running + 2 `it.todo`).
 *
 * These call the function straight (no CLI harness) against a throwaway cwd,
 * asserting the receipt, the on-disk effects, cwd honoring, silence-by-default,
 * and the exact lines emitted through the shared logger (the CLI's byte-for-byte
 * source). Command-level behavior (flag parsing, exit codes) is covered by
 * clients/cli/commands/init.behavior.test.mjs.
 *
 * **Two cases are `it.todo`, both blocked on the same deferral: this CLI bundles
 * no page templates yet.** `template` (slice 6a) landed its discovery, but
 * upstream's 1,329 template assets are deferred as transcription, so
 * `listTemplates()` is `[]`, `applyTemplate` short-circuits to `'skipped'`
 * before either error path can run, and both cases would pass vacuously.
 */
...
describe('init() — errors', () => {
	...
	it('the thrown error is an AstryxError with a helpful message', async () => {
		...
		expect(/** @type {AstryxError} */ (caught).message).toMatch(/Unknown features: nope/);
		expect(/** @type {AstryxError} */ (caught).message).toMatch(/agents, theme, template/);
	});
});
...
describe('init() — logger', () => {
	...
	it('emits the install line + full next-steps through the shared logger', async () => {
		...
		const errSpy = vi
			.spyOn(console, 'error')
			.mockImplementation((...a) => void lines.push(`ERR:${a.join(' ')}`));
		...
		// Upstream's glyph is `✓`; the CLI's plain-ASCII dialect writes `[ok]`.
		...
		expect(text).toContain(getNextSteps('npx astryx-svelte')[2].slice(0, 20));
	});
});

describe('init() — write-path safety', () => {
	it.todo(
		'template scaffold refuses to clobber an existing +page.svelte — blocked on the ' +
			'deferred template assets: listTemplates() is [] so applyTemplate returns ' +
			'"skipped" before the ERR_FILE_EXISTS guard can run'
	);
	...
	it.todo(
		'rejects a traversal templateName before any write (ERR_UNKNOWN_TEMPLATE) — blocked ' +
			'on the deferred template assets, same short-circuit as the case above'
	);
});
