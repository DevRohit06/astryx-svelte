 ...
 *
 * One case reads differently here than upstream and says so in place: this
 * port's theme sources carry no Meta copyright line, so "strips the Meta
 * copyright header" asserts the *absence* against a real bundled file rather
 * than against a header that was there a moment ago. Kept, because `theme add`
 * still runs the strip — the bundle is a copy of `packages/themes/*\/src`, and a
 * theme adapted from upstream would arrive with the header attached.
 */
...
describe('themeAdd (api/theme/add)', () => {
	it('copies a theme into src/themes/<slug> and returns a theme.add receipt', async () => {
		...
		for (const f of result.data.files) {
			expect(fs.existsSync(path.join(tmpDir, 'src', 'themes', 'neutral', f))).toBe(true);
		}
	});
	...
	it('strips the Meta copyright header from copied files', async () => {
		...
		expect(fs.readFileSync(first, 'utf-8')).not.toMatch(/Copyright \(c\) Meta Platforms/);
	});
	...
	it('honors an explicit target path (within cwd)', async () => {
		...
		expect(fs.existsSync(path.join(tmpDir, 'themes', 'mine', result.data.files[0]))).toBe(true);
	});
	...
	it('surfaces ERR_WRITE_FAILED (not a raw errno) when the target path is an existing file', async () => {
		...
		await expect(themeAdd('neutral', { cwd: tmpDir, targetPath: 'blocker' })).rejects.toMatchObject(
			{
