 ...
 *
 * Verbatim from upstream apart from the identity strings: the reference docs are
 * data files, and everything framework-shaped about them lives in their content,
 * not in the loader.
 */
...
export async function loadReferenceDocs(docPath, { lang } = {}) {
	...
	return {
		...
		sections: docs.sections.map(
			(/** @type {import('../../authoring/doctypes/types').ReferenceSection} */ section) => {
				...
				return {
					...
					content: section.content.map(
						(
							/** @type {import('../../authoring/doctypes/types').ReferenceContentBlock} */ block,
							/** @type {number} */ bi
						) => {
