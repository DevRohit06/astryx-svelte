 ...
 *
 * Ported case for case from upstream: 8/8, no deferrals. Nothing here reads a
 * topic's *content*, so nothing had to be refixtured for this port's rewritten
 * reference docs.
 */
...
describe('docs() dispatcher routing', () => {
	...
	it(
		'empty topic -> docs.list (falsy topic routes to list)',
		async () => {
	...
	it(
		'topic + section -> docs.detail.section',
		async () => {
			const { data } = await docs();
			let routed = null;
	...
	it(
		'unknown topic -> ERR_UNKNOWN_TOPIC',
		async () => {
	...
	it(
		'known topic + unknown section -> ERR_UNKNOWN_SECTION',
		async () => {
			const { data } = await docs();
			const topic = data[0].topic;
	...
	it(
		'non-string topic -> ERR_UNKNOWN_TOPIC (not a raw TypeError)',
		async () => {
	...
	it(
		'non-string section -> ERR_UNKNOWN_SECTION (not a raw TypeError)',
		async () => {
			const { data } = await docs();
			const topic = data[0].topic;
