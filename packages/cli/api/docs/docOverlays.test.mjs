 ...
 *
 * Ported case for case: 8/8 source cases (the loop contributes two per overlay
 * pair, so six pairs report as seventeen at runtime). The last case's section
 * title is the one thing that moved — upstream's base doc calls it
 * "useTheme Hook" and this port's calls it "useTheme", because the things a
 * Svelte consumer reaches for are utils, not hooks.
 */
...
describe('reference doc overlays (#2182)', () => {
	...
	for (const { topic, variant, basePath, overlayPath } of pairs) {
		it(`${topic} --${variant}: every overlay section anchors to a real base section`, async () => {
			...
			const baseTitles = base.docs.sections.map((/** @type {any} */ s) => s.title);
			const unanchored = (overlay.sections || [])
				.filter((/** @type {any} */ s) => s.section == null || !baseTitles.includes(s.section))
				.map((/** @type {any} */ s) => s.section ?? `(no anchor) "${s.title}"`);
		...
		it(`${topic} --${variant}: no base section is overridden twice`, async () => {
			const overlayMod = await load(overlayPath);
			const overlay = overlayMod.docsDense || overlayMod.docsZh;
			const anchors = (overlay.sections || []).map((/** @type {any} */ s) => s.section);
			const dupes = anchors.filter(
				(/** @type {any} */ a, /** @type {number} */ i) => anchors.indexOf(a) !== i
			);
