 ...
 *   token-refs resolved, matching `astryx-svelte --json docs <topic>`.
...
async function resolveTokenRefs(docsData, topics) {
	...
	for (let si = 0; si < resolved.sections.length; si++) {
		...
		/** @type {import('../../../authoring/doctypes/types').ReferenceContentBlock[]} */
		const newContent = [];
		for (const block of section.content) {
			if (block.type === 'token-ref') {
				...
				const refSection = refDocs.sections.find(
					(/** @type {import('../../../authoring/doctypes/types').ReferenceSection} */ s) =>
				...
				if (refSection.previewType && !section.previewType) {
					resolved.sections[si] = {
						...section,
						previewType: refSection.previewType,
						content: newContent
					};
				}
			} else {
