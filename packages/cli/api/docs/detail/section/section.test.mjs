 ...
 * first section).
 *
 * Ported case for case: 3/3, verbatim — the `tokens` topic still has a section
 * whose title contains "spacing".
 */
...
describe('docs.detail.section leaf', () => {
	it(
		'resolves a named section (case-insensitive) into a docs.detail.section envelope',
		async () => {
	...
	it(
		'throws ERR_UNKNOWN_SECTION for an unknown section',
		async () => {
			let err;
			try {
	...
	it(
		'does not return the first section for an empty section name',
		async () => {
