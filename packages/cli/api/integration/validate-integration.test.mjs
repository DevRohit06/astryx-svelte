 ...
 * matching the convention the other suites use.
 *
 * ## Ported case count
 *
 * Upstream has 13; **13 here, all live as of slice 9**, which landed
 * `checkCodemods` and with it the last deferred case, `flags a broken codemod as
 * invalid_codemod error`. The manifest half was complete from slice 3
 * (presence, uniqueness, schema, roots-inside-package, roots-exist); the
 * contribution half arrived one check per slice — `checkComponents` with slice
 * 4, `checkTemplates` with slice 6, `checkCodemods` with slice 9.
 *
 * The two `checkComponents` cases were deferred alongside them and **landed with
 * slice 4**. One of them, "reports no errors for a valid component", would have
 * passed while deferred —
 * `expect(byCode(issues, 'invalid_component')).toHaveLength(0)` is trivially true
 * when nothing can emit `invalid_component` — which is why it was held back
 * rather than landed green: it would have reported the check as working while
 * the check did not exist. Its sibling below is what makes it non-vacuous.
 */
...
function writePackage(
	...
	fs.writeFileSync(path.join(dir, `astryx-svelte.integration.${manifestExt}`), manifest);
}
...
describe('validate-integration API', () => {
	...
	it('flags multiple manifests as multiple_manifests error', async () => {
		...
		fs.writeFileSync(path.join(pkgDir, 'astryx-svelte.integration.js'), 'export default {};\n');
		const result = await validateLocalIntegration(pkgDir);
	...
	it('reports no errors for a valid component (doc + same-stem source)', async () => {
		...
		fs.writeFileSync(path.join(cDir, 'Widget.doc.mjs'), `export default { name: 'Widget' };\n`);
		// Same-stem source. Upstream writes `Widget.tsx`; an integration here
		// contributes the extension this port swizzles, `.svelte`.
		fs.writeFileSync(path.join(cDir, 'Widget.svelte'), `<div>Widget</div>\n`);
	...
	it('flags a component doc missing its same-stem source as invalid_component', async () => {
		...
		const cDir = path.join(pkgDir, 'components');
		fs.mkdirSync(cDir, { recursive: true });
		// Doc with no sibling Widget.svelte.
		fs.writeFileSync(path.join(cDir, 'Widget.doc.mjs'), `export default { name: 'Widget' };\n`);
