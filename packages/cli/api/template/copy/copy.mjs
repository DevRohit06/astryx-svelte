 ...
 *
 * **One adaptation.** Upstream names a scaffolded page `page.tsx`, the Next.js
 * App Router's route component. A SvelteKit route directory holds
 * `+page.svelte`, so that is the default filename here; the block branch is
 * unchanged, since a block is scaffolded under its own basename either way.
 */
...
import { PAGE_SOURCE_FILE, stripTemplateAssetRefs } from '../_adapter.mjs';
...
export function templateCopy(match, { targetPath, cwd, overwrite = false }) {
	...
	if (isFilePathArg(targetPath)) {
		...
		outputFileName = match.type === 'block' ? path.basename(match.filePath) : PAGE_SOURCE_FILE;
	...
	// (@astryx-svelte/cli/api) and must enforce this itself — same guard the peer
