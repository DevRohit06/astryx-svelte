 ...
 *
 * Exported via `@astryx-svelte/cli/json`:
 *   import { parseResponse, isError, assertResponse } from '@astryx-svelte/cli/json';
 */

/**
 * Parse raw CLI output into a typed result.
 * Consumer types are declared in foundation/response/base.d.ts — this is the
...
export function assertResponse(raw, expectedType) {
	...
	if (/** @type {any} */ (result).type !== expectedType) {
		throw new Error(`Expected type "${expectedType}", got "${/** @type {any} */ (result).type}"`);
	}
	return result;
}
