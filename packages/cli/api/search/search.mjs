 ...
 *
 * Returns the same typed envelope { type, data } that
 * `astryx-svelte --json search` outputs. The CLI command handler is a thin
 * wrapper around this function.
 *
 * `search(query)` is the single "I'm looking for X" entry point across ALL
 * content domains — components, utils, docs topics, and templates (page +
 * block). Today, finding the right thing requires four separate list calls
 * (`component --list`, `util --list`, `docs`, `template --list`) plus manual
 ...
 * component fuzzy resolver in foundation/text/string-utils.mjs:
 *
 ...
 *
 * ## The three places this is not upstream's file
 *
 * 1. `loadModuleDoc` reads `mod.default ?? mod[exportName]`. Upstream reads only
 *    the named export because every doc it ships is the legacy loose form; all
 *    209 docs `packages/core` ships are the *stamped default export* form, so
 *    reading only `docs` would return `undefined` for every component and util
 *    and every keyword signal would silently vanish. Same change
 *    `foundation/discovery/component-loader.mjs` documents.
 * 2. The `template` domain indexes external-package blocks and integration
...
/** Default import specifier for a util whose doc names none. */
const DEFAULT_UTIL_IMPORT = '@astryx-svelte/core/hooks';
 ...
 * @param {string} candidate.name - Primary identifier (component/util name, topic, template name).
...
/**
 * Load a doc module's docs object, swallowing errors.
 *
 * `default ?? named` rather than upstream's named-only read: every doc
 * `packages/core` ships is the stamped default-export form. The reference docs
 * under `assets/docs` are still the legacy `export const docs`, so both forms
 * have to work and the default has to win — see the file header.
...
/**
 * Build util candidates: name + keywords + usage/description from the util's
 * .doc.mjs. The domain stays `hook` — see search.type.mjs.
 * @param {string} coreDir
 * @returns {Promise<Candidate[]>}
 */
async function gatherHooks(coreDir) {
	...
	for (const hookName of names) {
		...
		let keywords = [];
		let description = '';
		let importPath = DEFAULT_UTIL_IMPORT;
		if (docPath) {
 ...
 *
 * Landed with slice 6, upstream's body unchanged. It indexes whatever
 * `discoverTemplates` finds — external-package blocks and integration
 * templates today, plus core's pages and blocks once those assets land — and
 * upstream's own `catch { return [] }` already covers a project with none.
 *
 * @param {string} cwd
 * @returns {Promise<Candidate[]>}
 */
	...
	switch (c.domain) {
		case 'component':
			return {
				...base,
				import: c._import,
				command: `astryx-svelte component ${c.name}`
			};
		case 'hook':
			return {
				...base,
				import: c._import,
				// The verb is `util` here; the domain is not. See search.type.mjs.
				command: `astryx-svelte util ${c.name}`
			};
		case 'doc':
			return {
				...base,
				title: c._title,
				command: `astryx-svelte docs ${c.name}`
			};
		case 'template':
			return {
				...base,
				...
				kind: c._kind,
				command: `astryx-svelte template ${c.name}`
			};
		default:
			return base;
	}
...
/**
 * Unified ranked search across components, utils, docs, and templates.
 *
...
export async function search(query, options = {}) {
	...
	if (!query || !String(query).trim()) {
		throw new AstryxError(
			...
			[{ name: 'astryx-svelte search button', reason: 'example' }],
	...
	if (!coreDir) {
		throw new AstryxError('Could not find @astryx-svelte/core package');
	}
