 ...
 *
 * The command that consumes this is `util`, not `hook` (see
 * `api/util/util.type.mjs`); the file keeps upstream's name because what it
 * renders is a `HookDoc`, the authoring type this port publishes under exactly
 * that name. Renaming the renderer would fork it from the type it renders.
 *
...
function formatReturnsTable(returns) {
	...
	for (const r of returns) {
		lines.push(`| \`${mdCell(r.name)}\` | \`${mdCell(r.type)}\` | ${mdCell(r.description)} |`);
	}
	return lines.join('\n');
}
 ...
 *   # Name, description, anatomy→params, best practices, props→params+returns
 * @param {any} docs
 * @returns {string}
 */
export function formatHookFull(docs) {
	...
	// Returns (util-specific, no component equivalent)
