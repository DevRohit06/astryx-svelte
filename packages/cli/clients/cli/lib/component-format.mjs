 ...
 *
 * Near-verbatim: a `ComponentDoc` is framework-neutral data, so the only edits
 * are the namespace literal (`astryx-` stays — it is core's class prefix, which
 * this port kept) and the fenced-code language, which is `svelte` rather than
 * `tsx` because that is what an example in this port would be written in.
 */
 ...
 *
 * Keep the `astryx-` literal in sync with packages/core/src/lib/internal/naming.ts
 * (NAMESPACE / classPrefix).
 * @param {{className: string, visualProps?: string[], states?: string[]}} target
 * @returns {string}
 */
...
/** @param {{visualProps?: string[], states?: string[]}} target @returns {string[]} */
...
/** @param {any[]} [props] @returns {string} */
 ...
 * @param {{visualProps?: string[]}} target
 * @param {any} docs
 * @returns {string[]}
 */
...
function formatTargetsTable(docs, themeData) {
	...
	for (const target of docs.theming.targets) {
		...
		// e.g. className 'astryx-button' → component key 'button'
...
/**
 * Format full component docs (default mode).
 *
 * @param {any} docs - Component doc object
 * @param {object} [options] - Options
 * @param {any} [options.themeData] - Resolved theme data
 * @param {string|null} [options.importHint] - Import path hint (e.g. '@astryx-svelte/core')
 */
	...
	if (docs.theming) {
		...
		if (docs.theming?.vars?.length) {
			...
			if (docs.theming?.derived?.length) {
				const varsKey = docs.theming.targets?.length
					? targetKey(docs.theming.targets[0])
...
/**
 * Format compact docs for LLM consumption.
...
export function formatCompact(docs, componentName, importHint) {
	const displayName = componentName.startsWith('XDS') ? componentName.slice(3) : componentName;
		...
		for (const d of docs.theming.derived) {
			const target =
				d.expand === 'container'
					...
					: (d.vars || []).map((/** @type {any} */ v) => `\`${mdCell(v)}\``).join(', ');
 ...
 * unions fall through to the terse prop list. The full values are always one
 * `astryx-svelte component <Name>` away.
 */
const SIGNATURE_UNION_MAX_MEMBERS = 8;

/**
 * Format a brief, LLM-optimized summary.
 *
...
export function formatBrief(docs, componentName, importHint, options = {}) {
	...
	// Build signature from union-type props. Upstream excludes `ReactNode`
	// unions; the Svelte counterpart of a renderable child is `Snippet`, which is
	// what this port's own compiler-derived prop types spell.
	/** @type {string[]} */
	...
	for (const prop of props) {
		const values =
			prop.type.includes('|') && !prop.type.includes('Snippet')
				? prop.type
						.replace(/['"]/g, '')
						.split('|')
						.map((/** @type {string} */ v) => v.trim())
	...
	// Signature line
	const sigStr =
		signatureProps.length > 0 ? `${displayName}(${signatureProps.join(', ')})` : displayName;
	...
	// Theme targets (component class, preferred data attrs, props, states) with
	// theme variant merging
	if (docs.theming?.targets?.length) {
		const { themeData = null } = options;
		const targetParts = docs.theming.targets.map((/** @type {any} */ t) => {
			...
			if (themeVars?.length)
				parts.push(`theme: ${themeVars.map((/** @type {any} */ v) => v + '*').join(', ')}`);
			return parts.join(' ');
		});
	...
	// First code example. Upstream looks for the line opening a `<XDS…>` tag;
	// that prefix never existed here, so the markup line is the one opening any
	// tag at all.
	if (examples.length > 0) {
		const code = examples[0].code;
		const codeLine =
			code.split('\n').find((/** @type {string} */ l) => l.trim().startsWith('<')) ||
			code.split('\n')[0];
