/**
 * @file Colocated tests for the component doc formatters.
 *
 * ## Ported case count
 *
 * 7, matching upstream one for one. Three fixture edits, none of which changes
 * what is asserted:
 *
 *   - sub-components are named bare (`RadioListItem`), not `XDSRadioListItem`.
 *     The `XDS` prefix is residue of upstream's own rename and no doc in this
 *     port has ever carried one.
 *   - the "see docs" pointer is built from `getCliInvocation()` rather than
 *     hard-coded, because the invocation this port prints is package-manager
 *     aware (`pnpm exec astryx-svelte`, `npx astryx-svelte`, …) and asserting
 *     one spelling would make the case depend on how the suite was launched.
 *   - a `ReactNode` prop type becomes `Snippet`. The formatter treats the type
 *     as opaque text, so only the fixture's honesty changes.
 */
...
describe('formatFull sub-component rendering', () => {
	...
	// reference, e.g. {name: 'RadioListItem'}, with props/description in their
	// own .doc.mjs. Previously `comp.description` was undefined and got printed
	// as the literal string "undefined", and the props area was blank.
	it('does not print "undefined" for a bare {name} sub-component', () => {
		...
		expect(out).toContain('### RadioListItem');
		expect(out).not.toContain('undefined');
		// Points the reader at the sub-component's own docs instead of a blank.
		expect(out).toContain(`${getCliInvocation()} component RadioListItem`);
	});
...
describe('formatFull theming override keys', () => {
	...
	// `.astryx-astryx-*` at runtime and silently match nothing.
	...
	it('prints override keys without the astryx- prefix (compound Table classes)', () => {
		const docs = {
			...
			description: 'A table.',
			theming: {
				targets: [{ className: 'astryx-base-table' }, { className: 'astryx-table-cell' }]
			}
		};
		const out = formatFull(docs);
...
/** Pipes that actually separate cells — an escaped one is content, not a separator. */
function cellPipes(row) {
	return (row.match(/(?<!\\)\|/g) || []).length;
}

describe('markdown table cells escape their pipes', () => {
	// A prop type is a union, and a union is spelled with the very character GFM
	// uses to separate cells. Backticks do not protect it: a row with more cells
	// than the header has columns gets its excess *discarded*, so an unescaped
	// `gap: 0 | 0.5 | ...` silently eats its own Default and Description columns.
	// See packages/core/src/lib/components/markdown, which splits on unescaped
	// pipes and preserves an escaped one as literal.
