 ...
 * degrade to null (like an unknown slug) rather than crash
 * `astryx-svelte component` with a raw TypeError from specifier.startsWith(...).
 *
 * `resolveTheme` is async here where upstream's is sync — it loads a theme
 * package with `import()` because `require()` resolves none of this port's (see
 * resolve-theme.mjs's header). Every case is `await`ed; the assertions are
