 ...
 *
 * Background: the --json envelope and the `preAction` gate work for the normal
 * command-execution path, but Commander's own error and help paths run BEFORE
 * preAction:
 *
 ...
 * try/catch in `bin/astryx-svelte.mjs` never sees them). The result: a --json
 ...
 *      envelope path (so `astryx-svelte bogus-cmd --json` gets exit 1 + envelope
 ...
 * subcommand (e.g. `astryx-svelte component --help --json`).
 *
 * @param {import('commander').Command} cmd
 * @returns {{apiVersion: number, type: 'help', data: object}}
 */
export function buildHelpEnvelope(cmd) {
	// Reconstruct the fully-qualified command name (e.g. "astryx-svelte theme build").
	const nameParts = [];
 ...
 *   commander.unknownCommand      — `astryx-svelte bogus-cmd`
 *   commander.unknownOption       — `--bogus-flag`
 *   commander.missingArgument     — `astryx-svelte theme build` (no <file>)
