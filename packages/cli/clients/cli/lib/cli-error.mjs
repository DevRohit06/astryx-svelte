 ...
 *
 * This is the CLI half of the seam: everything `api/` throws crosses into
 * presentation here, exactly once. `cliError` reads the stable `code` off the
 * thrown `AstryxError` and puts it on the JSON envelope verbatim, so an API
 * error and a CLI error are indistinguishable to a machine consumer.
 *
 * ## Exit-code policy
 *
 ...
 *
 *   3. **Help-display is success.** `astryx-svelte`, `astryx-svelte --help`,
 *      `astryx-svelte <cmd> --help`, and `astryx-svelte --version` all exit 0.
 *      The user got what they asked for.
 *
 *   4. **Empty / no-args invocations:** if the command has a sensible
 *      default (e.g. `astryx-svelte component` lists all components,
 *      `astryx-svelte docs` lists all topics), exit 0. If the command requires
 *      arguments (e.g. `astryx-svelte theme build`), Commander rejects it with
 *      exit 1.
 *
 ...
 * envelope's `suggestions` field. Canonical definition lives in
 * foundation/response/base.
