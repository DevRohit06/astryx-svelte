/**
 * @file End-to-end tests for the `upgrade` command shell.
 *
 * ## Ported case count
 *
 * Upstream has 15; 15 here, all live. Ten of them — the whole agent-docs block —
 * are verbatim apart from the package name, because the agent-docs refresh is
 * the half of `upgrade` that has nothing to do with codemods and therefore
 * nothing to do with the runner substitution.
 *
 * Two assertions changed, both because the core codemod registry is empty (see
 * `assets/codemods/registry.mjs`) and each named at its case: the 0.0.9 → 0.0.10
 * gate regression now asserts the *absence* of the up-to-date short-circuit
 * rather than the presence of a run receipt, and the `--list` case asserts an
 * empty list.
 *
 * Temp dirs are repo-local rather than under the OS temp dir, matching the rest
 * of this port's CLI suites.
 */
...
beforeEach(() => {
	...
	tmpDir = fs.mkdtempSync(path.join(process.cwd(), '.astryx-upgrade-test-'));
...
function createProgram() {
	...
	// Mirror the global --json flag from clients/cli/index.mjs so
	// program.opts().json resolves the same way in tests.
...
function writeInstalledCore(version, packageName = '@astryx-svelte/core') {
...
describe('upgrade gate (semver comparison)', () => {
	it('does NOT block an upgrade from 0.0.9 to installed 0.0.10 (regression)', async () => {
		...
		// Upstream's assertion is a three-way OR that includes `upgrade.run`; with
		// an empty core registry the honest form is the direct one — the gate must
		// NOT have short-circuited to up_to_date. That is the regression, and this
		// says so without depending on a registered codemod.
		expect(result.error).toBeUndefined();
		expect(result.type).toBe('upgrade.status');
		expect(result.data.status).toBe('no_codemods');
		expect(result.data.status).not.toBe('up_to_date');
	});
	...
	it('blocks when --from >= installed target by semver (e.g. 0.0.10 → 0.0.9)', async () => {
		...
		const program = createProgram();
		await program.parseAsync(['node', 'astryx-svelte', 'upgrade', '--from', '0.0.10']);
...
describe('upgrade argument validation', () => {
	...
	it('detects the installed target version from @astryx-svelte/core', async () => {
		writePkg();
		writeInstalledCore('0.0.15');
		writeSourceFile();
...
// `upgrade` must refresh the managed agent-docs block after a version bump, on
// EVERY path (the block documents the installed library, not the codemod
// outcome), and must NOT write during a dry run.
describe('upgrade agent-docs refresh', () => {
	...
	it('refreshes a stale block with --apply, even when there are no codemods (up-to-date path)', async () => {
		...
		// stale block must still be brought current. This is the exact gap the
		// upstream issue described.
	...
	it('surfaces agentDocs in the human output on the up-to-date path', async () => {
		...
		const program = createProgram();
		await program.parseAsync(['node', 'astryx-svelte', 'upgrade', '--from', '0.0.15', '--apply']);
		const output = stdoutCalls.join('') + logCalls.join('\n');
