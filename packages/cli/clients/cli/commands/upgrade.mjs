/**
 * @file `astryx-svelte upgrade` command — thin wrapper around api/upgrade.
 *
 * `astryx-svelte upgrade` runs codemods that migrate source between Astryx
 * versions and refreshes the managed agent-docs block. All logic lives in
 * api/upgrade; this handler only parses flags, wires the progress logger (term
 * output in human mode, silent in --json), and renders the result / exit code.
 *
 * Consumers bump/install their Astryx packages first, then run:
 *   astryx-svelte upgrade --from <old-version> --path <source-dir> --apply
 *
...
export function registerUpgrade(program) {
	program
		...
		// Upstream's flag installs jscodeshift, its codemod parser. This runner's
		// parser is `svelte`; the flag keeps its name and its meaning.
		.option('--install-deps', 'Auto-install the codemod parser (svelte) without prompting', false)
		.option('--list', 'List available codemods', false)
		.action(
			/**
			 * @param {import('../../../api/upgrade/upgrade.type.mjs').UpgradeOptions} options
			 */
			async (options) => {
				...
				try {
					...
					// envelope + exits 1; human mode already saw the error line + outro
					// via the logger, so we just set the exit code. Anything else is
					// unexpected — let the top-level boundary in bin/astryx-svelte.mjs
					// format it as before.
