/**
 * @file `astryx-svelte doctor` command — diagnose a project + environment and
 * report PASS / WARN / FAIL for each check with an actionable fix.
 *
 ...
 * FAILs (warnings are fine), 1 when any check FAILs — so `astryx-svelte doctor`
 * works as a CI gate.
 */
...
function printHuman(report) {
	...
	emit(
		section('astryx-svelte doctor — diagnosing your setup'),
...
/**
 * Register the `astryx-svelte doctor` command.
 * @param {import('commander').Command} program
 */
export function registerDoctor(program) {
	program
		.command('doctor')
		.description('Diagnose your Astryx setup and report problems with fixes')
		.addHelpText(
			'after',
			'\nExit code:\n' +
