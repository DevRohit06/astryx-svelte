/**
 * @file theme command — Commander wiring for `astryx-svelte theme`
 * (build/list/add).
 *
 * Thin CLI wrapper. The `build` compiler lives in the programmatic API
 * (../../../api/theme/build/build.mjs); `list`/`add` delegate to the
 * ../../../api/theme/list/list.mjs and ../../../api/theme/add/add.mjs leaves.
 * This file only parses options, injects a logger, renders human output, and
 * maps AstryxError → cliError. Watch mode (a human-interactive, long-running
 * loop) stays here because it re-invokes `theme build` as a child process.
 *
 * Usage:
 *   astryx-svelte theme build ./src/themes/ocean.ts
...
/**
 * Path to this CLI's real entry (bin/astryx-svelte.mjs), resolved from this
 * module's location. Used to re-invoke `theme build` as a child process in
 * watch mode.
 *
...
function resolveCliBin() {
	...
	return path.resolve(commandsDir, '../../../bin/astryx-svelte.mjs');
}
 ...
 *   options; `out` and `iconsSpecifier` are forwarded.
...
export function registerTheme(program) {
	const theme = program
		...
		.action((/** @type {unknown} */ options, /** @type {import('commander').Command} */ cmd) => {
			...
			// unknown subcommand (e.g. `astryx-svelte theme bogus`), Commander hands
			// it to us as a positional in cmd.args — exit 1 with a clear error.
			...
			// Bare `astryx-svelte theme` — show the subcommand list. Exit 0 (help is
			// success).
			theme.help();
		});
	...
	theme
		...
		.option('-w, --watch', 'Rebuild automatically when the theme file changes (Ctrl-C to stop)')
		.option(
			'-c, --check',
			'Verify the committed outputs match the source without writing; exit non-zero if stale'
		)
		// Upstream declares this through its self-documenting `defineCommand`
		// surface (#4714), which this port does not have; a plain commander option
		// is the same flag by the same name.
		.option(
			'--icons-specifier <specifier>',
			"Override the icon registry's import specifier in the generated module"
		)
		.action(
			async (
				/** @type {string} */ file,
				...
				try {
					const result = await themeBuild(
						file,
						{ out: options.out, check: options.check, iconsSpecifier: options.iconsSpecifier },
						{ cwd: process.cwd() }
					);
	...
	theme
		...
		.action(
			async (
				...
				// Upstream strips a `.tsx`/`.ts` extension; this port's theme entry is
				// always `<slug>-theme.ts`, and the `.svelte` icon registry beside it is
				// never the entry, so the same expression covers it.
				const entryModule = importSpecifier(outputDir, entry.replace(/\.tsx?$/, ''));
				emit(
