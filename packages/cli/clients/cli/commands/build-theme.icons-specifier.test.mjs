/**
 * @file Regression tests for `astryx-svelte theme build --icons-specifier`.
 *
 * The generated module imports the icon registry rather than inlining it,
 * because the registry holds component references (Svelte components here,
 * React elements upstream — the mechanism is the same either way, since neither
 * survives serialization). `extractIconInfo` lifts that specifier out of the
 * TypeScript source, where an extensionless `./icons` is resolved by the
 * TypeScript resolver — but the artifact is ESM JavaScript, which requires a
 * fully specified path, so Node cannot load it. See upstream #4620.
 *
 ...
 * is an uncompiled registry module that only a bundler can resolve — working.
 *
 * Ported case-for-case from upstream's
 * `clients/cli/commands/build-theme.icons-specifier.test.mjs` (6 cases, 6
 * ported, nothing dropped). Two notes on the translation:
 *
 *   - Upstream declares the flag through its self-documenting `defineCommand`
 *     surface (#4714), which this port does not have; here it is a plain
 *     commander option of the same name. No upstream case in this file asserts
 *     on generated help text or a command doc, so nothing is dropped for it.
 *   - The theme fixture's neighbouring registry module is framework-neutral
 *     (`export const testIcons = {}`) upstream and stays so here: what is under
 *     test is an extensionless specifier that a bundler resolves and Node does
 *     not, which has nothing to do with what the registry holds.
 */
...
function writeThemeWithIcons(dir, name) {
	...
	fs.writeFileSync(path.join(dir, 'icons.mjs'), 'export const testIcons = {};\n');
...
describe('theme build --icons-specifier', () => {
	it('emits the declared specifier', async () => {
		...
		const result = await runCli(
			['theme', 'build', path.relative(project, themeFile), '--icons-specifier', './icons.mjs'],
			project
		);
		expect(result.code).toBe(0);

		const generated = fs.readFileSync(path.join(project, 'declared.js'), 'utf8');
		expect(iconImportLine(generated)).toBe('import { testIcons } from "./icons.mjs";');
	});
	...
	it('encodes the declared specifier as a valid JavaScript string', async () => {
		...
		const result = await runCli(
			['theme', 'build', path.relative(project, themeFile), '--icons-specifier', "./icon's.mjs"],
			project
		);
		expect(result.code).toBe(0);

		const generated = fs.readFileSync(path.join(project, 'encoded.js'), 'utf8');
		expect(iconImportLine(generated)).toBe('import { testIcons } from "./icon\'s.mjs";');
	});
	...
	it('combines with the current --check flow', async () => {
		...
		const withoutSpecifier = await runCli(['theme', 'build', relativeTheme, '--check'], project);
		expect(withoutSpecifier.code).toBe(1);

		const withSpecifier = await runCli(
			['theme', 'build', relativeTheme, '--check', '--icons-specifier', './icons.mjs'],
			project
		);
	...
	it('leaves the scraped specifier untouched when the flag is absent', async () => {
		...
		const generated = fs.readFileSync(path.join(project, 'untouched.js'), 'utf8');
		expect(iconImportLine(generated)).toBe("import { testIcons } from './icons';");
	});
