 ...
 *
 * Auto-discovers .doc.mjs files from the assets/docs/ directory.
 * Supports --detail (full|compact|brief) and --lang (en|zh|dense).
 *
 * Usage:
 *   astryx-svelte docs                          List available topics
 *   astryx-svelte docs <topic>                  Print full doc
 *   astryx-svelte docs <topic> <section>        Print one section
 *
 * The renderer is upstream's verbatim: a `ReferenceDoc` is data, and the block
 * types it renders (prose, heading, code, table, list) carry no framework.
 */
...
/**
 * @param {import('../../../authoring/doctypes/types').ReferenceContentBlock} block
...
/**
 * @param {import('../../../authoring/doctypes/types').ReferenceSection} section
 * @param {'full' | 'compact' | 'brief'} detail
 * @returns {string}
 */
function formatSection(section, detail) {
	const blocks = section.content.map((b) => formatBlock(b, detail)).filter(Boolean);
...
/**
 * @param {import('../../../authoring/doctypes/types').ReferenceDoc} docs
 * @param {'full' | 'compact' | 'brief'} detail
 * @returns {string}
 */
				...
				switch (result.type) {
					case 'docs.list': {
						...
						emit(
							...
							text(
								[`Usage: ${run} docs <topic>`, `       ${run} docs <topic> <section>`].join('\n')
							)
						);
						break;
					}
