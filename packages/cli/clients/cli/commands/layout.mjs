/**
 * @file `layout` command — thin CLI wrapper around api/layout/layout.mjs.
 *
 * Subcommands:
 *   astryx-svelte layout expand "<expr>" [path]   compressed expression -> validated .svelte
 *   astryx-svelte layout check "<expr>"           validate + echo both canonical surfaces
 *   astryx-svelte layout grammar                  agent cheatsheet (alias table is branch-generated)
 *
...
export function registerLayout(program) {
	const layoutCmd = program
		.command('layout')
		.description('Generate Astryx layouts from compressed expressions (XLE/XLO)');

	layoutCmd
		.command('expand [expression] [path]')
		.description('Expand a layout expression into a validated Svelte component')
		...
		.action(
			async (
				...
				/** @type {LayoutExpandOptions} */ options
			) => {
				...
				let result;
				try {
					result = /** @type {LayoutExpandResponse} */ (
						await layoutExpand(source, {
							targetPath,
							form: options.form,
				...
				if (result.data.written) {
					...
					// Raw expanded component (no target path) — preformatted, emitted verbatim.
	...
	layoutCmd
		...
		.action(async (/** @type {string} */ expression, /** @type {LayoutCheckOptions} */ options) => {
			...
			let result;
			try {
				result = /** @type {LayoutCheckResponse} */ (
					await layoutCheck(source, {
						form: options.form,
						loose: options.loose || false,
						cwd: process.cwd()
					})
