/**
 * @file Regression test for `astryx-svelte theme build` install-instructions
 * import paths.
 *
...
// (there is no in-CLI fallback generator). Build core once if it isn't already
// present so the suite works in any CI job, regardless of job ordering.
beforeAll(() => {
	ensureCoreBuilt();
}, 200_000);
...
describe('theme build install-instructions import path', () => {
	...
	it('strips a leading src/ from the import path (relative to a file in src/)', async () => {
		const project = path.join(tmpDir, 'project');
		const themeFile = writeTheme(path.join(project, 'src', 'themes', 'ocean'), 'ocean');
