/**
 * @file Behavior coverage for `astryx-svelte init`. Ported case-for-case from
 * upstream's `clients/cli/commands/init.behavior.test.mjs` — 10 cases, 10 here.
 *
 ...
 *
 * These lock the non-interactive contract: `astryx-svelte init` never prompts,
 * is safe to re-run, writes the tool-agnostic AGENTS.md by default, and fails
...
afterEach(() => {
	...
	// The command enables the shared logger and never turns it back off; reset it
	// so a later suite in the same worker is not surprised by stray stdout.
...
describe('astryx-svelte init — default (no flags)', () => {
	...
	it('is non-interactive: exits cleanly with no TTY and no prompt text', async () => {
		...
		expect(stderr).not.toMatch(/prompt|inquirer|clack/i);
	});
...
describe('astryx-svelte init --features', () => {
...
describe('astryx-svelte init --agent <preset>', () => {
...
describe('astryx-svelte init --remove-agents', () => {
