 ...
 *
 * Ported case-for-case from upstream
 * `clients/cli/commands/layout.error-codes.test.mjs` at `v0.3.0`: 4 cases in a
 * two-subcommand loop, the same here. Two changes, both mechanical: the bin
 * lives at the package root (`bin/astryx-svelte.mjs`) rather than under
 * `clients/cli/bin/`, and the missing-file path is built from `os.tmpdir()` so
 * it is genuinely absent on Windows too — upstream's literal `/tmp/...`
 * resolves against the current drive there and would still be missing, but
 * only by accident.
 */
...
const MISSING_FILE = path.join(os.tmpdir(), 'definitely-missing-astryx-xyz.mjs');
...
describe('layout --json error codes', () => {
	for (const sub of ['expand', 'check']) {
		...
		it(`layout ${sub} --file <missing> → ERR_FILE_NOT_FOUND (not ENOENT)`, () => {
			const r = run(['--json', 'layout', sub, '--file', MISSING_FILE]);
			expect(r.status).toBe(1);
			const env = JSON.parse(r.stdout);
		...
		it(`layout ${sub} --file <missing> in human mode leaks no stack trace`, () => {
			const r = run(['layout', sub, '--file', MISSING_FILE]);
			expect(r.status).toBe(1);
