/**
 * @file Tests for `astryx-svelte theme build --watch` (upstream #3375).
 *
 ...
 *
 * This is the one build-theme suite that spawns the real binary rather than
 * using the in-process harness, because watch mode *is* a long-running process
 * with signal handling — upstream's suite does the same and defines its own
 * `runCli` for it.
 */
...
const CLI_BIN = path.resolve(__dirname, '../../../bin/astryx-svelte.mjs');
...
describe('theme build --watch', () => {
	...
	it('builds initially, rebuilds on change, and stops cleanly on SIGINT', async () => {
		...
		const child = spawn(process.execPath, [CLI_BIN, 'theme', 'build', 'wt.mjs', '--watch'], {
			cwd: tmpDir,
			env: { ...process.env, FORCE_COLOR: '0' }
		});
		let stdout = '';
		...
		// The graceful-shutdown half of this assertion is POSIX-only, and the
		// reason is Windows, not the port. Windows has no signals: `child.kill
		// ('SIGINT')` from a sibling process is mapped to TerminateProcess, so the
		// watcher's `process.once('SIGINT')` handler never runs and the farewell
		// line is never printed. Only a console CTRL_C_EVENT would reach it, and a
		// test runner cannot raise one for a detached child. The watcher stopping
		// (asserted above) holds on every platform; the message it prints on the
		// way out is asserted where it can be.
