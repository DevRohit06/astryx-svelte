/**
 * @file CLI-level tests for `astryx-svelte build` error-envelope faithfulness.
 *
...
describe('build --json error envelopes carry the API error code', () => {
	it(
		'bogus --type -> ERR_INVALID_ARGUMENT (not ERR_UNKNOWN)',
		async () => {
	...
	it(
		'non-positive --limit -> ERR_INVALID_ARGUMENT',
		async () => {
			const { status, stdout } = await runCli(
	...
	it(
		'non-integer --limit -> ERR_INVALID_ARGUMENT',
		async () => {
			const { status, stdout } = await runCli(
