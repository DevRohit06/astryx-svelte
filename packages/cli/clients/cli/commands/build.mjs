 ...
 *
 *   astryx-svelte build                  → the PLAYBOOK (how to build a page)
 *   astryx-svelte build "<what>"         → a COMPOSITION KIT (closest page
 *                                          template, blocks, components) with a
 *                                          recommended START.
 *
 ...
 *
 * The playbook's rules are the one place upstream's prose is genuinely
 * framework-specific and had to be rewritten rather than reused: `style={{}}`
 * is a React expression, and the reset stylesheet upstream tells you to import
 * (`reset.css` + `astryx.css`) is `base.css` here. Everything else — the four
 * numbered steps, the arrow structure, the closing tip — is upstream's.
 */
 ...
 * @param {string} run - The CLI invocation prefix (e.g. `npx @astryx-svelte/cli`).
 */
	...
	emit(
		...
		text(
			[
				...
				`   - No inline style= — use component props; design tokens via \`${run} docs tokens\`.`,
				`   - Wrap the app in <Theme theme={...}> and import ${CORE_PACKAGE}/base.css.`
			].join('\n')
		),
		text(
	...
	program
		...
		.action(
			async (
				...
				const limit = options.limit != null ? Number.parseInt(options.limit, 10) : 60;
				...
				out.push(
					...
					record({
						frame,
						foundation,
						setup: `import "${CORE_PACKAGE}/base.css"; no <div>/style for layout — use Stack/Grid + tokens`
					})
				);
