 ...
 *
 * Two layers:
 *   1. API-level (direct import of api/search/search.mjs) — ranking, cross-domain
 *      results, --type/--limit, typo tolerance, empty results, envelope shape.
 *   2. CLI-level (through the in-process harness) — exit codes and the --json
 *      contract, which only fire through a real Commander parse against argv.
 *
 * Ported case for case: 23/23. Three assertions carry this port's identity
 * rather than upstream's — the follow-up command is `astryx-svelte component
 * Button`, a doc hit's command starts `astryx-svelte docs `, and the text view
 * asserts the command as a SUBSTRING because `formatCliCommand` prefixes it
 * with the detected package manager (`pnpm exec …` in this repo).
 *
 * Upstream's CLI layer spawns a subprocess per case; this uses `runCli`, the
 * in-process harness that drives the same `createProgram()` the binary does.
 * That is slice 1's substitution, not this slice's, and it is why there is no
 * 30s-per-case subprocess budget here.
 */
...
// Run against the monorepo root so @astryx-svelte/core is discoverable.
...
describe(
	...
	() => {
		it('ranks an exact component name match first', async () => {
			...
			expect(top.command).toBe('astryx-svelte component Button');
		});

		it('returns components/utils tagged with a matching keyword', async () => {
			const { data } = await search('button', OPTS);
...
describe(
	...
	() => {
		it('returns results from more than one domain for a broad query', async () => {
			...
			// The doc topic should carry a follow-up `astryx-svelte docs` command.
			const docHit = data.results.find((r) => r.domain === 'doc');
			expect(docHit.command.startsWith('astryx-svelte docs ')).toBe(true);
		});
...
describe(
	'search() API — fuzzy / typo tolerance',
	() => {
...
describe(
	...
	() => {
		...
		it('renders each result as a greppable key: value record', async () => {
			const r = await runCli(['search', 'button'], REPO_ROOT);
			// A substring, not the whole line: `formatCliCommand` prefixes the command
			// with the detected package manager's runner.
			expect(r.stdout).toContain('astryx-svelte component Button');
