/**
 * @file template command — thin CLI wrapper around api/template/template.mjs.
 *
 * Registered with `.alias('add')`, upstream's own alias: `astryx-svelte add
 * hero ./src/routes/marketing` reads the way a scaffolding verb should.
 */
...
import { PAGE_SOURCE_FILE } from '../../../api/template/_adapter.mjs';
...
/**
 * A template entry as produced by the discover* helpers in
 * api/template/template.mjs. The api layer returns these as `object[]`; this
 * local shape captures the fields the command consumes.
 ...
 * declares `{type: string, data: unknown}`; narrow it here to the precise union
 * so the switch below type-checks.
 * @typedef {(
...
/**
 * The package name core (built-in) templates are reported under. Rendered only
 * when a row belongs to some *other* package, so the common listing stays
 * uncluttered.
 */
	...
	program
		...
		.action(
			...
			async (name, targetPath, options) => {
				...
				if (name && targetPath && !options.list && !options.skeleton) {
				...
				switch (result.type) {
					case 'template.list': {
						...
						const toRow = (t) => ({
							...
							package: t.package && t.package !== CORE_PACKAGE ? t.package : ''
						});
					...
					case 'template.copy': {
						emit(text(`Copied template to ${result.data.outputDir}/${result.data.fileName}`));
						break;
					}
				}
			}
...
/**
 * Mirror the destination-resolution logic in api/template/copy so we can detect
 * a clobber before invoking the API.
 *
...
async function detectTemplateCollision(name, targetPath) {
	...
	let templates;
	try {
		templates = /** @type {DiscoveredTemplate[]} */ (await discoverTemplates(process.cwd()));
	} catch {
		return null;
	}
	...
	// File-arg branch: targetPath looks like `./foo.svelte`.
	const { isFilePathArg } = await import('../../../foundation/fs/path-safety.mjs');
	let dest;
	if (isFilePathArg(targetPath)) {
		dest = resolved;
	} else {
		const fileName = match.type === 'block' ? path.basename(match.filePath) : PAGE_SOURCE_FILE;
