/**
 * @file Regression test for the `color-scheme` declaration
 * `astryx-svelte theme build` emits alongside `light-dark()` token output
 * (upstream #3658).
 *
 * `theme build` injects a `color-scheme` declaration into
 ...
 * base.css's own `html[data-theme]` -> `color-scheme` mapping (a bare
 ...
 *
 * `theme build` requires a compiled @astryx-svelte/core (there is no in-CLI
...
describe('theme build color-scheme output', () => {
	it('emits the mode-aware color-scheme rules in @layer astryx-theme for a light-dark() theme', async () => {
		...
		// injection in build.mjs (it checks the generated CSS for the literal
		// substring). Core's own define-theme suite covers the separate
		// [light, dark] tuple -> light-dark() conversion; this fixture writes the
		// string directly so this test stays focused on the CLI's own
