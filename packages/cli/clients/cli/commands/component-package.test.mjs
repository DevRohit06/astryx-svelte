/**
 * @file `component()` with `--package`: disambiguating a name that exists in
 * both core and a back-compat external package (`pkg.astryx.docs`).
 *
 * The fixture is a minimal consumer with `node_modules/@test/ext` contributing
 * a `Button` (different docs from core's) and a unique `ProfileCard`.
 *
 * ## Ported case count
 *
 * 7, matching upstream one for one, all live — the showcase case was `it.todo`
 * until slice 6 landed `findShowcase`. Two fixture adaptations:
 *
 *   - no `packages/core` symlink — a repo-local temp root resolves core through
 *     `findCoreDir`'s upward walk, and Windows needs no privilege for that;
 *   - the external showcase block is a `.svelte`, not a `.tsx`.
 *
 * The `astryx` package.json field the scanner reads **keeps upstream's
 * spelling**: it is a convention third-party packages author against, and
 * renaming it would fork that contract.
 */
...
function createFixture() {
	...
	fs.writeFileSync(
		path.join(blocksDir, 'ProfileCardShowcase.doc.mjs'),
		`
export const doc = {
  type: 'block',
...
`
	...
	fs.writeFileSync(
		path.join(blocksDir, 'ProfileCardShowcase.svelte'),
		'<div>ProfileCardShowcase</div>\n'
	);
	...
	fs.writeFileSync(
		path.join(extSrc, 'Button', 'Button.doc.mjs'),
		`
export const docs = {
  name: 'AcmeButton',
...
`
	...
	fs.writeFileSync(
		path.join(extSrc, 'ProfileCard', 'ProfileCard.doc.mjs'),
		`
export const docs = {
...
beforeEach(() => {
	tmpDir = fs.mkdtempSync(path.join(CLI_ROOT, '.astryx-pkg-test-'));
	createFixture();
});
...
describe('component() with --package option', () => {
	...
	it('throws for component not in the specified package', async () => {
		await expect(component('Avatar', { cwd: tmpDir, package: '@test/ext' })).rejects.toThrow(
			'No component "Avatar" in package'
		);
	});

	it('returns showcase from external package when --package + --showcase', async () => {
		const result = await component('ProfileCard', {
			cwd: tmpDir,
			package: '@test/ext',
			showcase: true
		});
