 ...
 * the colocated api/component/component.type.mjs to get narrowing + typed data.
 *
 * @typedef {(
...
export function registerComponent(program) {
	program
		...
		.option('--package <name>', 'Scope lookup to an external package (e.g. @acme/widgets)')
		.action(
			async (
				...
				// `resolveTheme` is async here where upstream's is sync — it loads a
				// theme package with `import()`, because `require()` resolves none of
				// this port's (see that file's header). This is its only caller.
				/** @type {any} */
				...
				const listFooter = text(
					[
						`Import from the path shown (e.g. import {Button} from '${CORE_PACKAGE}')`,
				...
				switch (result.type) {
					case 'component.list': {
						...
						const importCell = (item) => {
							...
							const qualify =
								item.package !== CORE_PACKAGE || (nameCounts.get(item.name)?.size ?? 0) > 1;
					...
					case 'component.detail.blocks': {
						...
						if (examples.length > 0) {
							out.push(section('Examples'), records(examples, { fields: ['name', 'description'] }));
						}
						if (related.length > 0) {
							out.push(
...
// (agent-docs, tests, and doc generation import from here)
export {
	discoverComponents,
