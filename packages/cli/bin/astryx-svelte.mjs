...
//
// Sibling package modules (e.g. `clients/`, `foundation/`) are resolved by this
// bin's REAL path. When the CLI is installed as a package, `npx astryx-svelte`
// runs this file through the `node_modules/.bin/astryx-svelte` symlink, and some
// Node versions resolve relative specifiers from the symlink's directory rather
// than the real package — breaking bare `../…` imports with a cryptic
...
//
// Upstream keeps its bin at `clients/cli/bin/astryx.mjs`, one level inside the
// client that owns it, so its `importSrc` paths carry a `../..`. Ours sits at
// the package root because that is the path `package.json#bin` has always
// declared, so `importSrc` is rooted at the package instead. Everything below
// the bin mirrors upstream's tree exactly.
import { fileURLToPath, pathToFileURL } from 'node:url';
import { realpathSync } from 'node:fs';
import { dirname, join } from 'node:path';

const binDir = dirname(realpathSync(fileURLToPath(import.meta.url)));
/** @param {string} rel - package-root-relative module path */
const importSrc = (rel) => import(pathToFileURL(join(binDir, '..', rel)).href);

const { isNodeVersionSupported, unsupportedNodeMessage } = await importSrc(
	'foundation/env/node-version.mjs'
);
...
const { program } = await importSrc('clients/cli/index.mjs');
const { isJsonMode, toErrorEnvelope } = await importSrc('foundation/response/json.mjs');
const { handleCommanderError } = await importSrc('clients/cli/lib/json-shim.mjs');
