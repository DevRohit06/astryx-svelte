 ...
 */
export function jsonError(message: string, suggestions?: Suggestion[], code?: ErrorCode): never;

/** Parse raw CLI output (string or object) into a typed result. */
export function parseResponse(raw: unknown): CLIResponse | CLIError | CLIUnsupportedError;

/** Type guard: returns true if result is an error. */
export function isError(result: unknown): result is CLIError | CLIUnsupportedError;
 ...
 */
export function assertResponse<T extends string>(raw: unknown, type: T): CLIResponse & { type: T };

declare global {
	namespace NodeJS {
		interface Process {
			/**
			 * Set once an envelope has reached stdout, so the bin error boundary and
			 * the `postAction` belt-and-suspenders never double-emit. Upstream's flag
			 * name is kept verbatim (`__xds` predates the Astryx rename) because it is
			 * process-internal plumbing, not observable output, and every later slice
			 * ports files that reference it by this name.
			 */
