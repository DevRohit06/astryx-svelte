 ...
 * contract.
 *
 * The contract has four guarantees, enforced here and in clients/cli/index.mjs:
 *
 ...
 *      effect (preAction gate in clients/cli/index.mjs).
 ...
 * live in api/json/parse.mjs and are exported via `@astryx-svelte/cli/json`.
 */
 ...
 *
 * Held at 1 to match `@astryxdesign/cli` — agent tooling written against
 * Astryx's envelopes reads the same `apiVersion` here.
 */
 ...
 * from the parsed --json option (see setJsonMode in clients/cli/index.mjs).
 * Read by humanLog() so human output is suppressed without every call site
 * having to thread a `json` boolean.
 *
 * @type {boolean}
 */
let _jsonMode = false;
...
/**
 * Enable/disable JSON mode globally. Called once from clients/cli/index.mjs as
 * soon as the root options are known (preAction), before any command body runs.
 * @param {boolean} on
 */
