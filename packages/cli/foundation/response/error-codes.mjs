 ...
 * `astryx-svelte --json` must NOT branch on prose — string-matching "No
 * component named X" is brittle and silently breaks the moment we reword it.
 *
 ...
 * ## Parity
 *
 * Transcribed from upstream `packages/cli/foundation/response/error-codes.mjs`
 * at `v0.3.0`: all **43** codes, in upstream's order, with upstream's grouping.
 * The list is a compatibility surface shared with `@astryxdesign/cli` — an agent
 * written against Astryx branches on the same strings here — so it is neither
 * renumbered, reordered, nor trimmed to the subset this port currently emits.
 ...
 *
 * Upstream's own count is 43, not the 53 quoted in `port/research/02` §1.3 and
 * `port/todo.md`; verified against the `Object.freeze` block at both `v0.1.7` and
...
export const ERROR_CODES = Object.freeze({
	...
	/** A top-level command name was not recognized (e.g. `astryx-svelte bogus`). */
	ERR_UNKNOWN_COMMAND: 'ERR_UNKNOWN_COMMAND',
	/** A subcommand under a command group was not recognized (e.g. `astryx-svelte theme bogus`). */
	...
	/** `@astryx-svelte/core` could not be located (not installed / not in a monorepo). */
	...
	// ── Upgrade ──────────────────────────────────────────────────────
	/** The current `@astryx-svelte/core` version could not be detected. */
