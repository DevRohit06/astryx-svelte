 ...
 *
 * Both surfaces produce the same AST and share the attribute-token grammar, so
 * the validator and expander never know which dialect the author wrote.
 * Whitespace is insignificant in compact form; `^` climb is rejected with a
 * correction (groups are the documented way).
 *
 * Ported unchanged from upstream: the grammar is a notation, not a framework
 * surface, so nothing in it is React-shaped.
 *
 * @input  expression source string
 * @output parse(source) -> {roots, overlays, form}
 * @position foundation/xle — pure (no I/O, no registry); binding happens in validate.mjs
 */
...
/** Fused shorthand letters (p6, g4, cp2 …) — the letter must be a KEY_ALIAS. */
...
function tokenize(str, line, startCol) {
	...
	while (i < str.length) {
		...
		if (depth > 0) {
			throw new XLEParseError('Unbalanced brackets in attribute list', line, startCol + start + 1);
		}
...
export function parseValue(raw, _depth = 0) {
	if (_depth > MAX_VALUE_DEPTH) {
		throw new XLEParseError(`Attribute value nested too deeply (limit ${MAX_VALUE_DEPTH})`, 0, 0);
	}
	const s = raw.trim();
...
function parseAttrToken(token, line) {
	...
	if (text.startsWith('@')) {
		...
		if (rest.startsWith('(') && rest.endsWith(')')) {
			value = {
				subexpr: parseCompactSiblings(new CompactStream(rest.slice(1, -1), line, col + eq + 2))
			};
		} else if (/^(['"]).*\1$/.test(rest)) {
	...
	for (let i = 0; i < text.length; i++) {
		...
		if (ch === '"' || ch === "'") quote = ch;
			...
			return {
				kind: 'kv',
				key: rawKey,
				value: parseValue(text.slice(i + 1)),
				raw: text,
				line,
				col
			};
...
function parseCompactNode(s) {
	...
	if (s.peek() !== '{') {
		if (!isIdentStart(s.peek())) {
			s.error(`Expected a component name, found '${s.peek() ?? 'end of input'}'`);
		}
...
function parseOutlineNodeSegment(segment, line) {
	...
	if (!m) {
		throw new XLEParseError(`Expected a component name, found '${first.text}'`, line, first.col);
	}
...
export function parseOutline(source) {
	...
	for (let li = 0; li < lines.length; li++) {
		...
		const indent = /** @type {RegExpMatchArray} */ (raw.match(/^[ \t]*/))[0].replace(
			/\t/g,
			'  '
		).length;
		const text = raw.trim();
		...
		if (slotMatch && !text.startsWith('http')) {
			if (!parent.node) {
				throw new XLEParseError(
					`Slot '${slotMatch[1]}:' has no parent component`,
					lineNo,
					indent + 1
				);
			}
