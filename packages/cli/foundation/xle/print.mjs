 ...
 *
 * Used by `astryx-svelte layout check` so either party (agent or human) can
 * request the other surface, and by round-trip conformance tests. Printing is
 * best-effort canonical: spelling variants the lexer normalized are emitted in
 * canonical form.
 *
 * Ported unchanged: this prints the notation, which is framework-neutral.
 *
 * @input  parse() AST (binding not required)
 * @output toCompact(doc) / toOutline(doc)
 * @position foundation/xle — inverse of parse.mjs
 */
...
function valueText(value) {
	if (typeof value === 'string') return /[\s,'"]/.test(value) ? quoteStr(value) : value;
...
function slotTokensCompact(node) {
	return node.slots.map((slot) => {
		...
		if (/** @type {any} */ (slot.value).idref) {
			return `@${slot.key}=#${/** @type {any} */ (slot.value).idref}`;
		}
...
function siblingsCompact(items) {
	return items
		.map((item, i) => {
			...
			const needsGroup = i < items.length - 1 && item.kind === 'node' && item.children.length > 0;
