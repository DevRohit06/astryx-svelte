 ...
 * synthetic registry so they don't depend on `@astryx-svelte/core` docs.
 *
 * Ported case-for-case from upstream `foundation/xle/xle.test.mjs` at
 * `v0.3.0`: 36 cases, 36 here. The parser, printer and detectForm halves are
 * verbatim — the notation is the notation. The synthetic registry the validator
 * half builds is retargeted three ways, each matching a divergence already
 * recorded in `registry-core.mjs`:
 *   - `importPath` is the root package (`@astryx-svelte/core` publishes one
 *     barrel where `@astryxdesign/core` publishes a subpath per component),
 *   - `exportName` is the bare name, not `XDS<Name>`,
 *   - renderable props are typed `Snippet`, not `ReactNode`, so `isNode` is
 *     computed the way `toComponentEntry` computes it here.
 * None of the three is visible in an assertion; they keep the fixture honest
 * about what a real registry entry looks like in this port. Input leniency for
 * the legacy `XDS` prefix is unchanged, so the `XDSAppShell` case is kept.
 */
...
function makeTestRegistry() {
	...
	const components = new Map([
		[
			'AppShell',
			make('AppShell', [
				prop('contentPadding', 'SpacingStep'),
		...
		[
			'Stack',
			make('Stack', [
				prop('gap', 'SpacingStep'),
				prop('hAlign', "'start' | 'center' | 'end' | 'between'"),
				prop('vAlign', "'start' | 'center' | 'end'")
			])
		],
		[
			'Chip',
			make('Chip', [
				prop('variant', "'neutral' | 'success' | 'error'"),
				prop('label', 'string', true)
			])
		],
		['Box', make('Box', [prop('isScrollable', 'boolean'), prop('children', 'Snippet')])]
	]);
...
describe('parseCompact depth guard', () => {
	...
	it('rejects deep nesting that crosses group boundaries', () => {
		expect(() => parseCompact('(' + 'V > '.repeat(2000) + 'C' + ')')).toThrow(XLEParseError);
	});
