/**
 * @file XLE binder/validator — resolves the parsed AST against the component
 * registry and annotates every node with its bound component and prop
 * assignments.
 *
 * Philosophy (from the XLE research): lenient lexer, strict validator. Spelling
 * variants normalize silently (p6 = pad=6 = padding=6), but semantic problems —
 * unknown components, props, enum values, blocks — are hard errors carrying
 * ranked suggestions, never guesses.
 *
 * Ported near-verbatim. The one adaptation is `CLICK_PROPS`: upstream hard-codes
 * the pair `onClick` / `clickAction`, and this port's core spells the DOM
 * handler `onclick` (Svelte's event-attribute convention). See the constant.
 *
 * @input  parse() AST + registry (+ block list for {hint} resolution)
 * @output validate() -> {errors, warnings}; nodes gain .bound
 * @position foundation/xle — between parse.mjs and expand.mjs
 */

import { levenshteinDistance } from '../text/levenshtein.mjs';
...
/**
 * Click-handler props an `opens=#id` trigger can bind to, most specific first.
 *
 * Upstream tests exactly two names, `onClick` and `clickAction`, in both the
 * warning below and the expander's handler choice. Two of this port's docs
 * still carry `onClick`, two carry `clickAction`, and **thirteen** carry
 * `onclick` — Svelte spells DOM event props lowercase, so the React name did
 * not survive the port for most components. Following upstream's literal pair
 * would emit `clickAction={…}` on components that declare neither, which is the
 * one thing the surrounding warning exists to prevent. Hence a list rather than
 * a pair; the first two entries and their order are upstream's.
 */
export const CLICK_PROPS = ['onClick', 'clickAction', 'onclick'];
...
export class Validation {
	...
	bindAttrs(node, component) {
		...
		if (component.undocumented) {
			...
			if (node.attrs.length > 0) {
				this.warn(
					node,
					`${component.name} has no documented props — attrs passed through unvalidated`
				);
			}
			return;
		}
		for (const attr of node.attrs) {
			if (attr.kind === 'kv') {
				...
				if (!hit) {
					...
					this.error(
						node,
						`'${attr.key}' is ambiguous on ${component.name} — it is a value of ` +
							`${hit.ambiguous.join(' and ')}; use key=value`
					);
				} else {
				...
				if (!hit || hit.ambiguous || typeof hit.value !== 'boolean') {
					this.error(
						...
						suggest(
							attr.key,
							[...component.props.keys()].filter((p) => component.props.get(p)?.isBoolean)
						)
					);
				} else {
	...
	bindKv(node, component, attr) {
		...
		if (key === 'opens') {
			// Overlay trigger binding — expander turns this into a click TODO.
			node.bound.props.set('__opens', attr.value);
			if (!CLICK_PROPS.some((p) => component.props.has(p))) {
				this.warn(
					node,
					`'opens=' on ${component.name}, which has no click handler — emitting a comment only`
				);
			}
			return;
		}
		...
		for (const slot of node.slots) {
			...
			if (!prop) {
				this.error(
					...
					suggest(
						slot.key,
						[...component.props.keys()].filter((k) => component.props.get(k)?.isNode)
					)
				);
				continue;
			}
			if (!prop.isNode && !prop.isFunction) {
				this.warn(
					node,
					`${component.name}.${slot.key} is typed '${prop.type}' — slot value may not fit`
				);
			}
			const sv = slot.value;
	...
	bindHintRef(node, hint) {
		...
		const nearest = suggest(normalizeBlockKey(hint.name), [...this.blockIndex.keys()])
			.map((k) => this.blockIndex.get(k)?.dirName)
			.filter(/** @returns {d is string} */ (d) => d != null);
		if (this.loose) {
			this.warn(
				node,
				`Unknown block '{${hint.name}}' — emitting a TODO placeholder (running with --loose)`
			);
		} else {
			this.error(
				node,
				`Unknown block '{${hint.name}}' — block hints must name an existing template block`,
				nearest.length > 0 ? nearest : ['run: astryx-svelte template --list --type block']
			);
		}
	}
