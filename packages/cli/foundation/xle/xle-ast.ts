...
export interface RegistryProp {
	...
	isFunction: boolean;
	/**
	 * Whether the prop takes renderable content. Upstream tests the type text for
	 * `ReactNode`/`ReactElement`; this port's core types the same props `Snippet`,
	 * so that is what the predicate looks for. Renderable props are the ones the
	 * expander fills with a `{#snippet}` block.
	 */
	isNode: boolean;
}
...
export interface RegistryComponent {
	name: string;
	/**
	 * The identifier the generated module imports and writes as a tag. Upstream
	 * stores `XDS${name}` here; this port stores the bare name, because that is
	 * what `@astryx-svelte/core` actually exports. See registry-core.mjs.
	 */
	exportName: string;
	dirName: string;
	importPath: string;
 ...
 *
 * Retained for shape parity with upstream, but nothing produces one here: a
 * `.svelte` file is exactly one component, so a block cannot be co-defined
 * inside the generated module the way upstream co-defines a block's TSX
 * function. See splice.mjs.
 */
...
/** Emitter emission context. */
export interface EmitContext {
	inStack?: boolean;
}

/**
 * A scaffolded reactive-state entry.
 *
 * Upstream scaffolds `const [name, setName] = useState(initial)`, so `setter`
 * holds an identifier. Svelte 5 has no setter binding — the rune declares a
 * plain `let`, and writing to it is the update — so `setter` holds the
 * assignment *expression* the change handler is bound to. The field name is
 * kept so the shape still reads as upstream's.
 */
