...
// `.svelte` is this port's addition to upstream's list and the one that carries
// the command it exists for: `template` writes a route file, which is
// `+page.svelte` here where it is `page.tsx` upstream. The rest of upstream's
// list is kept verbatim rather than pruned — the set only decides "file or
// directory?", so dropping `.tsx`/`.jsx` would not reject them, it would make
// `template blank ./foo.tsx` create a *directory* called `foo.tsx`, which is
// the exact bug the helper was written to close.
 ...
 *
 * Used by `astryx-svelte template <name> <path>` so that
 *   `astryx-svelte template blank ./foo.svelte`
 * writes to `./foo.svelte` as a file, not `./foo.svelte/+page.svelte` (the old
 * bug).
 *
