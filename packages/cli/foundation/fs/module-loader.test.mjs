...
import { z } from 'zod';
import { findPresentFiles, importUserModule, loadModuleWithParser } from './module-loader.mjs';
...
describe('importUserModule', () => {
	it('imports a .mjs file and returns its module namespace', async () => {
		...
		fs.writeFileSync(file, `export default {answer: 42};\nexport const named = 'hi';\n`);
...
describe('loadModuleWithParser', () => {
	...
	it('throws a readable error when there is no default export', async () => {
		...
		await expect(loadModuleWithParser(file, parse, { label: 'thing' })).rejects.toThrow(
			/thing is invalid/i
		);
	});
