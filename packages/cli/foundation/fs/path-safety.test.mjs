/**
 * @file Colocated tests for the path-safety guards.
 *
 * Ported case for case from upstream's suite, with `.svelte` folded into the
 * existing "known source extensions" case rather than added as a new one — the
 * extension set is the only thing this port changes here, and the case count is
 * the contract.
 */
...
import { assertWithin, sanitizeName, isFilePathArg, PathSafetyError } from './path-safety.mjs';
...
describe('assertWithin', () => {
	...
	it('rejects absolute paths outside root even when allowAbsolute', () => {
		expect(() => assertWithin('/etc/passwd', tmpDir, { allowAbsolute: true })).toThrow(
			PathSafetyError
		);
	});
...
describe('isFilePathArg', () => {
	it('returns true for known source extensions', () => {
		// `.svelte` is this port's addition and the one `template` actually
		// writes; upstream's `.tsx` head case is kept because the set only
		// decides file-vs-directory and pruning it would create a directory
		// called `foo.tsx` rather than reject the argument.
		expect(isFilePathArg('foo.svelte')).toBe(true);
		expect(isFilePathArg('./src/routes/+page.svelte')).toBe(true);
	...
	it('returns false for plain directory names', () => {
		...
		expect(isFilePathArg('src/routes')).toBe(false);
	});

	it('returns false for trailing slash even with extension-like name', () => {
		expect(isFilePathArg('foo.svelte/')).toBe(false);
	});
...
describe('assertWithin — symlink escape (realpath guard)', () => {
	...
	it('rejects a path that traverses a symlink pointing outside root', (ctx) => {
		// Creating a *directory* symlink on Windows needs either Developer Mode
		// or SeCreateSymbolicLinkPrivilege. Where neither is available there is
		// no way to build the fixture, so the case skips itself with a reason
		// rather than being dropped or rewritten into something weaker — the
		// guard it covers is real on every platform.
		try {
			fs.symlinkSync(outside, path.join(root, 'link'));
		} catch (err) {
			if (err.code === 'EPERM' || err.code === 'EACCES') {
				ctx.skip('directory symlinks are not permitted for this user');
				return;
			}
			throw err;
		}
