 ...
 *
 * The `.ts` branch is why `jiti` is a runtime dependency of this package. A
 * SvelteKit project is TypeScript by default, so `astryx-svelte.config.ts` is
 * the *likely* spelling rather than an exotic one, and Node still cannot import
 * a `.ts` file with the type stripping enabled by default in 22.x.
 */
