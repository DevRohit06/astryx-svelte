 ...
 * `foundation/agent-docs/agent-docs.test.mjs` — 76 cases, 76 here.
 *
 * Five cases assert generated *content* and so assert this port's content
 * instead of upstream's; each says so at its own site. The renamed exports
 * (`getXdsVersion` -> `getAstryxVersion`, `injectXdsBlock` ->
 * `injectAstryxBlock`, `removeXdsBlock` -> `removeAstryxBlock`) are the
 * pre-Astryx `XDS` residue corrected in the source, not new behaviour — the
 * markers themselves keep both spellings, which is what the legacy-marker cases
 * below exercise.
 */
...
describe('generateCompressedIndex', () => {
	...
	it('tailors the self-check to the styling system (xstyle for StyleX, not class for Tailwind)', () => {
		// Upstream's flagged attribute is `className=`; Svelte's is `class=`.
		// StyleX path: class/inline style are veers; the fix is the xstyle prop + a token
		...
		expect(stylexSelfCheck).toMatch(/class=/);
		// class IS the system in Tailwind — it must NOT be flagged
		...
		expect(tailwindSelfCheck).not.toMatch(/class=/);
	});

	it('defaults to the CSS-variable styling path (no compiler)', () => {
		const result = generateCompressedIndex('1.0.0');
		expect(result).toMatch(/style\/class with tokens/);
	...
	it('recommends xstyle when StyleX is configured', () => {
		// Upstream's line names `@astryxdesign/core/theme/tokens.stylex`, a deep
		// export this port does not publish. The Svelte rule that replaces it is the
		// one CLAUDE.md enforces: StyleX may never be imported from a `.svelte`
		// file, so the styles live in a sibling `.stylex.ts`.
		const result = generateCompressedIndex('1.0.0', { stylingSystem: 'stylex' });
		expect(result).toMatch(/xstyle prop with styles from a sibling \.stylex\.ts/);
	});

	it('recommends Tailwind utilities when Tailwind is configured', () => {
		// Upstream's second assertion names `tailwind-theme.css`, a file it ships
		// and this port does not; the token-backed utility names carry the rule.
		...
		expect(result).toMatch(/bg-surface, text-primary, rounded-lg/);
	});

	it('includes upgrade command and migration rule', () => {
		const result = generateCompressedIndex('1.0.0');
		expect(result).toContain('upgrade --apply');
		expect(result).toMatch(/after any @astryx-svelte\/core bump/);
	});

	it('states the invocation once in the CLI header (yarn)', () => {
		const result = generateCompressedIndex('1.0.0', { invocation: 'yarn astryx-svelte' });
		expect(result).toContain('yarn astryx-svelte <cmd>');
		expect(result).not.toContain('npx astryx-svelte');
	});

	it('uses the pnpm exec invocation', () => {
		const result = generateCompressedIndex('1.0.0', { invocation: 'pnpm exec astryx-svelte' });
		expect(result).toContain('pnpm exec astryx-svelte <cmd>');
		expect(result).not.toContain('npx astryx-svelte');
	});

	it('uses the scoped package for one-off (uninstalled) runs so agents never hit the bare name', () => {
		const result = generateCompressedIndex('1.0.0', { invocation: 'npx @astryx-svelte/cli' });
		expect(result).toContain('npx @astryx-svelte/cli <cmd>');
		// The header defines the mapping; the bare "run every command as
		// `npx astryx-svelte`" footgun must be absent.
		expect(result).not.toContain('npx astryx-svelte <cmd>');
	});
});
...
describe('detectStylingSystem', () => {
	...
	it('returns css for a plain project', () => {
		writePkg({ svelte: '5.0.0', vite: '6.0.0' });
		expect(detectStylingSystem(tmpDir)).toBe('css');
	});

	it('detects stylex when the compiler plugin is present', () => {
		// Upstream's fixture is `@stylexjs/babel-plugin`; the plugin a Svelte/Vite
		// project actually installs — and the one docs/vite.config.ts uses — is
		// `@stylexjs/unplugin`, so that is the one this case pins.
		writePkg({ '@stylexjs/unplugin': '0.0.1' });
	...
	it('prefers stylex over tailwind when both are configured', () => {
		writePkg({ '@stylexjs/unplugin': '0.0.1', tailwindcss: '4.0.0' });
		expect(detectStylingSystem(tmpDir)).toBe('stylex');
	});
});

describe('getAstryxVersion', () => {
	it('reads version from core package.json', () => {
		const coreDir = path.join(tmpDir, 'core');
		fs.mkdirSync(coreDir, { recursive: true });
		fs.writeFileSync(path.join(coreDir, 'package.json'), JSON.stringify({ version: '3.4.5' }));

		expect(getAstryxVersion(coreDir)).toBe('3.4.5');
	});
});

describe('injectAstryxBlock', () => {
	it('injects into an existing file without markers', () => {
		const filePath = path.join(tmpDir, 'test.md');
		fs.writeFileSync(filePath, '# Existing content\n');

		const result = injectAstryxBlock(filePath, '<!-- ASTRYX:START -->\nnew\n<!-- ASTRYX:END -->');
	...
	it('replaces existing markers', () => {
		...
		injectAstryxBlock(filePath, '<!-- ASTRYX:START -->\nnew\n<!-- ASTRYX:END -->');
	...
	it('returns false and does not create file when createIfMissing is false', () => {
		...
		const result = injectAstryxBlock(
			filePath,
			'<!-- ASTRYX:START -->\ncontent\n<!-- ASTRYX:END -->'
		);
...
describe('installAgentDocs', () => {
	...
	function setupCorePackage(dir, version = '1.0.0') {
		// Create a minimal @astryx-svelte/core so getAstryxVersion works
		const coreDir = path.join(dir, 'node_modules', '@astryx-svelte', 'core');
		fs.mkdirSync(coreDir, { recursive: true });
		fs.writeFileSync(path.join(coreDir, 'package.json'), JSON.stringify({ version }));
	}
...
describe('parseBlockVersion', () => {
	...
	it('reads prerelease versions', () => {
		expect(parseBlockVersion('Astryx v1.2.3-beta.4 · 10 components')).toBe('1.2.3-beta.4');
	});
...
describe('injectAstryxBlock / removeAstryxBlock — malformed managed blocks (no user-content loss)', () => {
	...
	it('refuses (throws) instead of orphaning a second block when duplicates exist', () => {
		...
		fs.writeFileSync(f, `# Doc\n${S}\nB1\n${E}\nMID\n${S}\nB2\n${E}\nTAIL\n`);
		expect(() => injectAstryxBlock(f, BLOCK)).toThrow(/multiple|malformed/i);
	...
	it('refuses to append a second block when a START has no matching END', () => {
		...
		fs.writeFileSync(f, `# Doc\nUSER1\n${S}\nOLD (no end)\nUSER2\n`);
		expect(() => injectAstryxBlock(f, BLOCK)).toThrow(/end marker|malformed|no matching/i);
...
describe('isAstryxInitialized — resilient to malformed markers (postinstall hot path)', () => {
	// isAstryxInitialized runs on every consumer install (via postinstall). It must
	// never throw — even on a half-written or garbage-marker file — so it uses a
	// plain `.includes(MARKER_START)`, never the strict findManagedBlock.
	const START = '<!-- ASTRYX:START -->';
	const END = '<!-- ASTRYX:END -->';

	it('returns true (never throws) on duplicate START markers', () => {
		fs.writeFileSync(path.join(tmpDir, 'AGENTS.md'), `${START}\na\n${START}\nb\n${END}\n`);
	...
	it('returns true on an END-before-START file without throwing', () => {
		fs.writeFileSync(path.join(tmpDir, 'AGENTS.md'), `${END}\nuser\n${START}\nblock\n`);
		expect(() => isAstryxInitialized(tmpDir)).not.toThrow();
		expect(isAstryxInitialized(tmpDir)).toBe(true);
	});
