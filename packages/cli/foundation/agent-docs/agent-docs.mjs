 ...
 *
 * Generates a CLI cheat sheet from live command metadata and injects it into
 * agent doc files between a pair of HTML-comment markers. Supports multiple
 ...
 *
 * The machinery — marker discovery, the malformed-block refusals, the presets,
 * the staleness inspection — is upstream's verbatim. The *content* of the
 * generated block is not, because every line of it is an instruction about this
 * package graph:
 *
 *   - **The markers keep upstream's spelling** (`ASTRYX:START`, and the legacy
 *     `XDS:START` pair read during migration). This reverses the call made for
 *     the config/manifest basenames and for the same reason pointed the other
 *     way: a manifest's payload names component sources a CLI will *read*, so a
 *     shared basename lets a React integration be loaded as Svelte. A marker's
 *     payload is a cheat sheet an agent *reads*, and one block that the last
 *     writer owns is strictly better for that agent than two contradictory ones.
 *   - **Setup is one stylesheet plus a StyleX compiler**, not upstream's
 *     `reset.css` + `astryx.css` pair. Core publishes `./base.css` (reset
 *     included) and ships its `.stylex.js` *uncompiled*, so a consumer's bundler
 *     runs the StyleX plugin — the same fact CLAUDE.md records about `docs/`.
 *   - **The topic scan accepts hyphens.** Upstream matches `^(\w+)\.doc\.mjs$`
 *     against `color.doc.mjs`-style names; six of this port's topics are
 *     hyphenated (`getting-started`, `working-with-ai`, …) and `\w+` drops them
 *     silently — a shorter list, no error.
 */
...
/**
 * @typedef {object} AgentDocsFileStatus
 * @property {string} path
 * @property {string|null} blockVersion
 ...
 * installed core version. This is the detection half of the `upgrade` agent-docs
 * refresh: it never writes, so `upgrade` can run it on EVERY path — including the
 * up-to-date / no-codemods short-circuits — and decide whether to rewrite a stale
 * block, nudge an uninitialized repo, or stay silent.
 *
 * A managed block is:
 ...
 *   files: AgentDocsFileStatus[],
 *   staleFiles: string[],
 *   blockVersions: string[]
 * }}
 */
export function inspectAgentDocs(targetDir, installedVersion) {
	const version = installedVersion ?? getAstryxVersion(findCoreDir(targetDir));
	/** @type {AgentDocsFileStatus[]} */
	const files = [];
	...
	const blockVersions = [
		...new Set(
			staleEntries.map((f) => f.blockVersion).filter(/** @returns {v is string} */ (v) => v != null)
		)
	];
 ...
 * Plain CSS variables (via `style`/`class`) always work, so they're the safe
 * default. Precedence: stylex (compiler wired) -> tailwind -> css.
 *
 * The compiler list is upstream's plus `@stylexjs/unplugin`, which is the one a
 * Svelte/Vite project actually installs and the one `docs/vite.config.ts` uses.
 * Upstream's Next.js/webpack entries are kept rather than pruned: the set only
 * decides which advice to print, so a stale entry costs nothing and removing one
 * would misclassify a project that happens to use it.
 *
 * @param {string} targetDir
...
/**
 * Doc topics bundled with this CLI, for the `docs <topic>` reference line.
 *
 * Upstream's pattern is `^(\w+)\.doc\.mjs$`. Hyphenated basenames are the
 * majority here (`getting-started`, `styling-libraries`, `working-with-ai`, …),
 * and `\w` excludes `-`, so upstream's spelling would silently list 14 of this
 * port's 20 topics. The `.dense`/`.zh` variants are still excluded, because the
 * anchored `\.doc\.mjs$` only matches the base form.
 * @returns {string[]}
 */
function bundledDocTopics() {
	const docsDir = path.join(CLI_ROOT, 'assets', 'docs');
	if (!fs.existsSync(docsDir)) return [];
	return fs
		.readdirSync(docsDir)
		.map((f) => /^([\w-]+)\.doc\.mjs$/.exec(f))
 ...
 *
 * Structured as: workflow (behavioral) -> rules (error prevention) -> CLI
 * reference. Templates are positioned first in the workflow to teach agents the
...
export function generateCompressedIndex(
	...
	// Required setup. Core publishes ONE stylesheet — `base.css` already carries
	// the reset — where upstream splits `reset.css` + `astryx.css`. And core ships
	// its `.stylex.js` uncompiled (svelte-package transpiles TypeScript and does
	// not run StyleX), so the consumer's bundler MUST run the StyleX plugin or
	// every component renders unstyled with no error.
	lines.push('SETUP (once) — without these, components render unstyled:');
	lines.push("  app.css:        @import '@astryx-svelte/core/base.css';");
	lines.push("                  @import '@astryx-svelte/theme-neutral/theme.css';");
	lines.push(
		'  vite.config.ts: run @stylexjs/unplugin over @astryx-svelte/core ' +
			'(add it to optimizeDeps.exclude + ssr.noExternal).'
	);
	lines.push('');
	...
	lines.push(
		'1. `astryx build "<idea>"` — START HERE: returns a kit (closest [page] + ' +
			'[block]s + [component]s). No args = full playbook.'
	);
	lines.push(
		'2. `astryx template <name> [--skeleton]` — scaffold the [page]/[block]s it ' +
			'named, or study their layout. Templates are reference code.'
	);
	lines.push('3. `astryx component <Name>` — props + examples for every component you use.');
	lines.push('');

	// Rules — the top error-preventers. Upstream's rules transfer intact except
	// for the element name: `<div>` is `<div>` in Svelte too, but `className` is
	// `class`, which is what the styling lines below say.
	lines.push('RULES:');
	lines.push(
		'- No <div> — components do all layout/spacing. Full page -> AppShell; ' +
			'sidebar nav -> SideNav.'
	);
	lines.push(
		'- Frame first: pick the shell (AppShell / Layout+LayoutPanel) and budget ' +
			'regions in px BEFORE writing content (`astryx docs layout`).'
	);
	lines.push(
		'- Dense data = rows (Table, List/Item) edge-to-edge — never Card-wrapped ' +
			'list items. Card = dashboard widgets, galleries, settings groups only.'
	);
	lines.push(
	...
	// recommend a path that isn't compiled here. The StyleX branch also names the
	// one constraint that has no upstream analogue: StyleX may not be imported
	// from a `.svelte` file, so `stylex.create` lives in a sibling `.stylex.ts`.
	if (stylingSystem === 'stylex') {
		lines.push(
			'- Custom styling: component props first; else the xstyle prop with styles ' +
				'from a sibling .stylex.ts (never import @stylexjs/stylex in a .svelte ' +
				'file). No raw hex/px.'
		);
	} else if (stylingSystem === 'tailwind') {
		lines.push(
			'- Custom styling: component props first; else Tailwind utilities backed by ' +
				'tokens (bg-surface, text-primary, rounded-lg). No raw hex/px.'
		);
	} else {
		lines.push(
			'- Custom styling: component props first; else style/class with tokens — ' +
				'var(--color-*|--spacing-*|--radius-*). No raw hex/px. (No StyleX/Tailwind ' +
				"compiler here — don't use xstyle/utility classes.)"
		);
	}
	lines.push(
		'- Tokens for every value (`astryx docs tokens`). Brand/accent via ' +
			'`astryx theme` — never override --color-* in :root.'
	);
	// Self-check — post-generation pass. The fix names the sanctioned escape hatch
	// for the configured system.
	const selfCheckFix = {
		stylex:
			'replace any class=, style=, raw <div>/<span> layout, imported .css/@apply, ' +
			'or hardcoded #hex/px with the component or the xstyle prop + a token',
		tailwind:
			'replace any style=, raw <div>/<span> layout, imported .css/@apply, or ' +
			'hardcoded/arbitrary value (e.g. bg-[#fff], p-[13px]) with the component or ' +
			'a token-backed utility',
		css:
			'replace any raw <div>/<span> layout, imported .css/@apply, or hardcoded ' +
			'value (#hex, 16px) with the component or a token (var(--color-*|--spacing-*|…))'
	};
	lines.push(
		`- SELF-CHECK before you finish: re-read the file and ${
			selfCheckFix[stylingSystem] ?? selfCheckFix.css
		}. If unsure a component/prop exists, run \`astryx component <Name>\` / ` +
	...
	lines.push('  upgrade --apply    run after any @astryx-svelte/core bump');
...
/**
 * Get the Astryx version from the core package, falling back to this CLI's own.
 *
 ...
 * @returns {string}
 */
export function getAstryxVersion(coreDir) {
	if (coreDir) {
...
/**
 * Inject or update the Astryx section in a file using the Astryx markers.
 ...
 * @param {boolean} [options.onlyReplace] Only write if markers already exist
 * @returns {boolean} Whether the file was written
 */
export function injectAstryxBlock(
	filePath,
	compressedIndex,
	...
	if (fs.existsSync(filePath)) {
		...
		if (block) {
			content = content.slice(0, block.start) + compressedIndex + content.slice(block.end);
...
/**
 * Inject or update the Astryx section in AGENTS.md.
...
export function injectAgentsMd(targetDir, version) {
	...
	injectAstryxBlock(agentsPath, compressedIndex, {
...
/**
 * Inject or update the Astryx section in CLAUDE.md.
...
export function injectClaudeMd(targetDir, version) {
	...
	return injectAstryxBlock(claudePath, compressedIndex);
}

/**
 * Remove the Astryx section from a file.
 * If the file becomes empty (only a boilerplate header remains), deletes it.
 *
 * @param {string} filePath
 * @param {{deleteIfEmpty?: boolean}} [options]
 * @returns {boolean} Whether the Astryx section was found and removed
 */
export function removeAstryxBlock(filePath, { deleteIfEmpty = false } = {}) {
...
/**
 * Remove the Astryx section from all known agent doc files.
 * @param {string} targetDir
 */
export function removeAgentDocs(targetDir) {
	...
	for (const p of allPaths) {
		...
		if (removeAstryxBlock(filePath, { deleteIfEmpty })) {
			if (!fs.existsSync(filePath)) {
 ...
 * @param {boolean} [options.onlyReplace] Only update files that already have markers
...
export function installAgentDocs(
	...
	const version = getAstryxVersion(coreDir);
	...
	if (paths && paths.length > 0) {
		...
		for (const p of paths) {
			...
			injectAstryxBlock(filePath, compressedIndex, {
				createIfMissing: true,
	...
	if (agent) {
		...
		for (const p of inject) {
			injectAstryxBlock(path.join(targetDir, p), compressedIndex);
			written.push(p);
		}
	...
	if (existing.length > 0) {
		for (const p of existing) {
			const didWrite = injectAstryxBlock(path.join(targetDir, p), compressedIndex, {
				onlyReplace
			});
	...
	injectAstryxBlock(path.join(targetDir, defaultPath), compressedIndex, {
		createIfMissing: true,
		header: `# AGENTS.md\n\nProject-specific guidance for AI coding agents.`
	});
