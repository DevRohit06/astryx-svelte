/**
 * @file Path-safety regression tests for `--agent-docs-path`. Ported
 * case-for-case from upstream's
 * `foundation/agent-docs/agent-docs.path-safety.test.mjs` — 3 cases, 3 here.
 *
...
describe('installAgentDocs path safety', () => {
	it('rejects --agent-docs-path that escapes the target directory', () => {
		...
		expect(() => installAgentDocs(target, { paths: ['../OUT.md'] })).toThrow(PathSafetyError);
	...
	it('rejects an absolute --agent-docs-path with a clear error', () => {
		...
		expect(() => installAgentDocs(target, { paths: [absPath] })).toThrow(
	...
	it('rejects deeper traversal segments', () => {
		...
		expect(() => installAgentDocs(target, { paths: ['../../../escaped.md'] })).toThrow(
			PathSafetyError
		);
