 ...
 *
 * The floor is upstream's 22.13.0, which is inside this repo's declared
 * `engines.node: ">=22"` rather than in conflict with it: `>=22` is the
 * dependency-resolution floor every package shares, and 22.13.0 is the point
 * within Node 22 where `styleText` landed. The `engines` field cannot express
 * the second without diverging from the rest of the monorepo, so the precise
 * check lives here and runs before anything can fail cryptically.
 */
...
export function parseNodeVersion(version) {
	...
	const [major = 0, minor = 0, patch = 0] = cleaned.split('.').map((seg) => parseInt(seg, 10) || 0);
...
export function unsupportedNodeMessage(version, minimum = MIN_NODE_VERSION) {
	...
	return (
		`astryx-svelte requires Node.js >=${minimum} (you have v${clean}). ` +
