 ...
 * `integrations.mjs`) with absolute contribution roots under the repo root
 * (process.cwd()), matching the repo-local temp-dir convention the rest of the
 * suites use. The helper is exercised directly; stderr is captured to assert
 * exactly one warning line per broken integration and zero output in --json
 * mode / for clean integrations.
 *
 * All six cases land in slice 3, which is not obvious given
 * `validateLoadedIntegration` ships here with only one of its three
 * contribution checks implemented. Every case exercises either `missing_root`
 * — the one check with no forward dependency — or the no-issue path, and the
 * "no issues" case passes for the right reason: an *existing but empty*
 * components root produces nothing from `checkRoots`, and the three deferred
 * checks would have produced nothing from an empty directory either.
 */
	...
	it('emits exactly one stderr line for an integration with issues', async () => {
		...
		expect(errLines[0]).toBe(
			'Warning: @acme/widgets has 1 integration issue(s). ' +
				'Run: astryx-svelte validate-integration @acme/widgets'
		);
	});
