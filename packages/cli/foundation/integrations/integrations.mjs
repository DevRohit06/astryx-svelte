 ...
 *
 * Integrations are PACKAGE NAMES listed in astryx-svelte.config.{ts,mjs,js}.
 * Each package declares a single conventional root manifest sibling to its
 * package.json — astryx-svelte.integration.{ts,mjs,js} — which contributes
 ...
 *
 * **The basenames carry the `-svelte` infix, and that is a deliberate reversal
 * of the call made for the `astryx` package.json field.** That field
 * (`discoverExternalPackages`, in `foundation/fs/paths.mjs`) kept upstream's
 * spelling because it is a third-party authoring contract whose payload —
 * a docs directory and a category string — is framework-neutral. A manifest's
 * payload is not: it points at component sources the CLI will read as Svelte.
 * Sharing upstream's basename would let a React Astryx integration installed
 * alongside this port be loaded and its `.tsx` read as `.svelte`. Same
 * reasoning, opposite conclusion, because what the file contains differs.
 */
...
export const MANIFEST_BASENAMES = [
	'astryx-svelte.integration.ts',
	'astryx-svelte.integration.mjs',
	'astryx-svelte.integration.js'
];
...
export async function loadIntegrations(specs = [], { cwd = process.cwd() } = {}) {
	...
	for (const spec of specs) {
		...
		let manifest;
		try {
			manifest = await loadModuleWithParser(manifestFile, parseIntegration, {
