...
import { findClosestComponents, searchComponents, levenshteinDistance } from './string-utils.mjs';
...
describe('findClosestComponents', () => {
	it('returns distance 0 for an exact (case-insensitive) match', () => {
		expect(findClosestComponents('button', COMPONENTS)).toEqual([{ name: 'Button', distance: 0 }]);
	});
	...
	it('does not throw on empty / whitespace / regex-special / unicode / long input', () => {
		...
		expect(findClosestComponents('.*', COMPONENTS)).toEqual([]);
		expect(() => findClosestComponents('(', COMPONENTS)).not.toThrow();
		expect(() => findClosestComponents('café', { g: ['café', 'cafe'] })).not.toThrow();
		expect(() => findClosestComponents('a'.repeat(100_000), COMPONENTS)).not.toThrow();
	});
...
describe('searchComponents', () => {
	...
	it('treats regex metacharacters as literal (no throw, no match-all)', async () => {
		...
		await expect(searchComponents('.*', NO_CORE, COMPONENTS)).resolves.toEqual([]);
		await expect(searchComponents('(', NO_CORE, COMPONENTS)).resolves.toEqual([]);
		await expect(searchComponents('[a-z]+', NO_CORE, COMPONENTS)).resolves.toEqual([]);
	});

	it('does not throw on empty / whitespace / unicode / very-long input', async () => {
		await expect(searchComponents('', NO_CORE, COMPONENTS)).resolves.toEqual([]);
		await expect(searchComponents('   ', NO_CORE, COMPONENTS)).resolves.not.toThrow();
		await expect(
