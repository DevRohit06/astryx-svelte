 ...
 *
 * Covers the util param/return description merge (HookTranslationDoc) added so
 * dense translations of params/returns are actually consumed, plus a regression
 * guard for the existing component prop-description merge.
 *
 * ## Ported case count
 *
 * 4, matching upstream one for one. The only edit is inside the fixture: the
 * `containerRef` return is typed `HTMLElement | null` rather than
 * `React.RefObject<HTMLElement | null>`. `mergeTranslation` treats the string
 * as opaque, so the assertion is unchanged; a React type in a Svelte port's
 * fixture would just be wrong.
 */
	...
	return {
		...
		params: [
			...
			{ name: 'options.isActive', type: 'boolean', description: 'Whether active.', required: true },
			{
				...
				type: '() => void',
				description: 'Escape callback.',
				required: false
			}
		],
		returns: [
			{ name: 'containerRef', type: 'HTMLElement | null', description: 'Ref to attach.' },
			{ name: 'focusFirst', type: '() => void', description: 'Focuses first element.' }
		]
	};
}

describe('mergeTranslation — util param/return descriptions', () => {
	it('overrides param and return descriptions by name, preserving other fields', () => {
		...
		expect(containerRef.type).toBe('HTMLElement | null');
