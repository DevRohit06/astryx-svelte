 ...
 *
 * A near-verbatim port: this file reads `.doc.{ts,mjs,js}` files, which are the
 * one artifact in the pipeline that is framework-neutral, so nothing about the
 * React → Svelte move touches it.
 */
 ...
 *     a stamped `type` and validate against the matching per-kind parser. Every
 *     doc `packages/core` ships is in this form.
 ...
 */
export async function loadComponentDoc(docPath, { zh = false, dense = false, lang } = {}) {
...
export function mergeTranslation(docs, translation) {
	...
	if (translation.components && merged.components) {
		merged.components = merged.components.map((/** @type {any} */ comp, /** @type {any} */ i) => {
			const trans =
				translation.components.find((/** @type {any} */ t) => t.name === comp.name) ||
 ...
 *
 * The `mod.default ?? mod.docs` read is this port's one change: upstream reads
 * only `mod.docs`, because at `v0.3.0` every doc it ships is still the legacy
 * loose form. Every doc `packages/core` ships is the *stamped default export*
 * form its own `parseDoc` prefers, so reading only `docs` here would return
 * `undefined` for all 209 of them.
