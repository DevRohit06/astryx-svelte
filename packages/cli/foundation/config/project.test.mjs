/**
 * @file Colocated tests for `Project`, the single project-read API.
 *
 * ## Ported case count
 *
 * Upstream has 17; **17 here, all live as of slice 9**. `Project` itself landed
 * whole in slice 3 — it was only the three discovery *bodies* that were stubbed
 * — so what was deferred here was ever only coverage of the discovery, never of
 * the class. The three slice-4 cases (`components()`, `memoizes components()`,
 * `collects issues for a broken integration`) landed with slice 4;
 * `templates() returns type-tagged templates` with slice 6; and the last two —
 * `codemods() returns core + integration groups` and `does not throw when a
 * codemod is broken` — with slice 9's codemod registry and integration codemod
 * discovery.
 *
 * **Two cases were refixtured while slice 4 was pending, and stay that way.**
 * Upstream's `issues() dedupes` and `issues() validates unvisited` both use
 * `scaffold({brokenComponent: true})`. They are re-pointed at a *missing
 * contribution root* instead, which `checkRoots` reports as `missing_root`. The
 * property under test is unchanged (issues dedupe; `issues()` completes on
 * demand for an integration no discovery call has visited); only the issue that
 * populates the set differs. Keeping the missing-root fixture means the two
 * cases exercise a different check from their three `invalid_component`
 * neighbours rather than a fourth copy of the same one.
 *
 * Temp dirs are repo-local rather than under the OS temp dir. Upstream's stated
 * reason — that Vite blocks a dynamic import from `/tmp` — **does not hold
 * here**; an absolute `file://` import from the OS temp dir works fine in this
 * setup. The convention is kept for parity with the other suites, not because
...
/**
 * Scaffold a consumer project (package.json + astryx-svelte.config.mjs) under a
 * repo-local temp dir, plus an installed integration package under
 * node_modules.
 *
 * `missingComponentsRoot` is this port's addition: it declares a components
 * root in the manifest without creating the directory, which is the shape
 * `checkRoots` reports. See the note at the top of the file.
 */
function scaffold({
	...
	const manifest = {};
	if (withComponents || missingComponentsRoot) manifest.components = './components';
	...
	if (withComponents && !missingComponentsRoot) {
		...
		if (!brokenComponent) {
			fs.writeFileSync(
				path.join(compDir, 'Widget.svelte'),
				`<script module>\n\texport interface WidgetProps {}\n</script>\n`
			);
		}
		// brokenComponent => doc with no sibling .svelte (invalid_component issue).
	}
	...
	if (withTemplates) {
		...
		fs.writeFileSync(path.join(tDir, 'hero.svelte'), `<h1>Hero</h1>\n`);
	}
	...
	if (withCodemods) {
		...
		if (brokenCodemod) {
			fs.writeFileSync(path.join(cmDir, 'bad.mjs'), `export default { not: 'a codemod' };\n`);
		} else {
			fs.writeFileSync(
...
describe('findConfigPath', () => {
	it('rejects multiple config files at the same root', () => {
		fs.writeFileSync(path.join(tmpDir, 'package.json'), JSON.stringify({ name: 'consumer' }));
		fs.writeFileSync(path.join(tmpDir, 'astryx-svelte.config.mjs'), `export default {};\n`);
		fs.writeFileSync(path.join(tmpDir, 'astryx-svelte.config.js'), `module.exports = {};\n`);
...
describe('Project discovery', () => {
	it('components() returns integration ownership records', async () => {
		...
		const widget = comps.find((c) => c.name === 'Widget' && c.package === '@acme/widgets');
		expect(widget).toBeTruthy();
		// Upstream's integration source is `Widget.tsx`; ours is `Widget.svelte`.
		expect(widget.sourcePath).toMatch(/Widget\.svelte$/);
	});
	...
	it('codemods() returns core registry groups + integration groups', async () => {
		...
		// `core` is `[]` — the registry walk is live, the registry is empty by
		// design (see assets/codemods/registry.mjs), and upstream's own assertion
		// here is only `Array.isArray`. The emptiness is pinned in that module's
		// own suite, so it cannot drift unnoticed from either side.
...
describe('Project issues (skip + warn)', () => {
	...
	it('issues() dedupes and is complete on demand', async () => {
		scaffold({ missingComponentsRoot: true });
		const project = await Project.load(tmpDir);
...
describe('Project.issuesUrl routing', () => {
	it('routes core to config.issuesUrl, falling back to the default', async () => {
		...
		expect(project.issuesUrl({ package: '@astryx-svelte/core' })).toBe('https://example.com/core');
	});
