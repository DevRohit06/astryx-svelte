<!--
	...
	`style` prop is a string. A scoped `<style>` block would not do — Svelte
	scopes the selector, not a class handed to a component — and this lands on
	`LayoutPanel`.
-->
<script lang="ts">
	import {
	...
	let activeId = $state<string | undefined>(OUTLINE_ITEMS[0]?.id);
...
{#snippet outlineEnd()}
	<LayoutPanel isScrollable={false} label="On this page" role="complementary" style={outlinePanel}>
		<Outline items={OUTLINE_ITEMS} onActiveIdChange={(id) => (activeId = id)} />
	</LayoutPanel>
{/snippet}
	...
	<LayoutContent isScrollable={false} padding={8}>
		<VStack gap={8}>
			<VStack gap={2}>
				...
				<Text type="supporting" color="secondary">Last updated March 30, 2026</Text>
				{#if isMobile.matches}
					<Selector
						label="On this page"
						isLabelHidden
			...
			<Card>
				<VStack gap={3}>
					...
					<Text type="body" color="secondary">
						Help me get set up with Product Name. Based on my project, do the following: 1.
						Install @astryxdesign/core and the StyleX compiler. 2. Wrap my app in
						ThemeProvider. 3. Replace one existing component with an Astryx equivalent.
					</Text>
				</VStack>
			</Card>

			<VStack gap={4}>
				<Heading id="prerequisites" level={2}>Prerequisites</Heading>
			...
			<VStack gap={4}>
				<Heading id="install-package" level={2}>Install the package</Heading>
				<Text type="body">
					Every project starts with installing the core package. This gives you access to all
					components, tokens, and utilities.
				</Text>
				<VStack gap={2}>
					<Text type="body" weight="bold">Step 1: Install the core package</Text>
					<CodeBlock code="npm install @astryxdesign/core" language="bash" width="100%" />
				</VStack>
				<VStack gap={2}>
					<Text type="body" weight="bold">Step 2: Import the precompiled styles</Text>
					<Text type="body" color="secondary">
...
import '@astryxdesign/core/astryx.css';`}
				...
				<VStack gap={2}>
					<Text type="body" weight="bold">Step 3: Import your first component</Text>
					<CodeBlock
...
export default function App() {
  return <Button label="Hello Astryx" variant="primary" />;
			...
			<VStack gap={4}>
				<Heading id="configure-theming" level={2}>Configure theming</Heading>
				<Text type="body">
					Astryx ships with a default theme that works out of the box. To customize colors,
					typography, and spacing, wrap your app in a theme provider.
				</Text>
				<CodeBlock
  ...
  return (
    <ThemeProvider theme="default">
      {children}
			...
			<VStack gap={4}>
				<Heading id="next-steps" level={2}>Next steps</Heading>
				<List density="compact" listStyle="disc">
					<ListItem label="Fundamental concepts — How theming, layout, and composition work" />
					<ListItem
						label="Component API reference — Props, variants, and examples for every component"
					/>
