<!--
	Ported from upstream's `assets/templates/pages/documentation/page.tsx`.
	Transcribed, not re-authored: the parity rule covers template content too.

	No Heroicons on this page, so no icon substitutions.

	Upstream's `previewCard` and `cardGrid` are `CSSProperties` consts applied
	through the `style` prop; here they are strings of the same declarations,
	because Svelte's `style` prop is a string. A scoped `<style>` block would not
	do — Svelte scopes the selector, not a class handed to a component — and both
	land on `Grid`/`Card`.

	`ClickableCard.onClick` is `onclick` here: the component extends the DOM
	attributes, so the handler keeps its DOM name.
-->
<script lang="ts">
	import {
		Button,
	...
	const previewCard = 'border-radius: var(--radius-element);';
...
{#snippet content()}
	<LayoutContent padding={8}>
		<VStack gap={10}>
					...
					<StackItem size="fill">
						<VStack gap={4}>
							...
							<Text type="large" weight="normal" color="secondary">
								An open-source UI library to help developers quickly build beautiful,
								accessible products.
							</Text>
							<HStack>
			...
			{#each COMPONENT_CATEGORIES as category (category.label)}
				<VStack gap={4}>
					...
					<Grid columns={{ minWidth: 260 }} gap={2} style={cardGrid}>
						{#each category.items as item (item.key)}
							...
							>
								<VStack gap={3}>
									<Card variant="muted" padding={0} minHeight={160} style={previewCard} />
									<VStack gap={0.5}>
										<Text type="body" weight="bold">{item.name}</Text>
										<Text type="body" color="secondary" maxLines={3}>{item.desc}</Text>
									</VStack>
								</VStack>
							</ClickableCard>
						{/each}
