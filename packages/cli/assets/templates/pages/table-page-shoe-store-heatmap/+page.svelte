<!--
	Ported from upstream's `assets/templates/pages/table-page-shoe-store-heatmap/page.tsx`.
	Transcribed, not re-authored: the parity rule covers template content too.

	Two upstream oddities are carried rather than corrected: the slug says
	"heatmap" but the page renders a revenue **area chart** (no heatmap anywhere),
	and its `template.doc.mjs` files it under `Table - Chart` accordingly.
	...
	**The revenue chart is deferred, not dropped.** Upstream renders `Chart` /
	...
	`Thumbnail`'s `style={{width: 36, height: 36, flexShrink: 0}}` is a
	`CSSProperties` object upstream and a `style` string here — same three
	declarations, same order.
-->
<script lang="ts">
	import {
		Badge,
	...
	type ProductCategory = 'Running' | 'Lifestyle' | 'Basketball' | 'Training' | 'Skateboarding';
	...
	const thumbnailStyle = 'width: 36px; height: 36px; flex-shrink: 0;';
	...
	const columns = $derived.by<TableColumn<OrderRow>[]>(() => [
		{
			key: 'id',
			header: 'Order',
			width: pixel(110),
...
{#snippet idCell(item: OrderRow)}
	<Link href="#" isStandalone>{item.id}</Link>
{/snippet}

{#snippet productCell(item: OrderRow)}
...
{#snippet amountCell(item: OrderRow)}
	<Text type="body">${item.amount}</Text>
{/snippet}

{#snippet customerCell(item: OrderRow)}
	<Text type="body">{item.customer}</Text>
{/snippet}

{#snippet emailCell(item: OrderRow)}
	<Text type="body">{item.email}</Text>
{/snippet}

{#snippet statusCell(item: OrderRow)}
	<Badge
...
{#snippet dateCell(item: OrderRow)}
...
{#snippet header()}
	<LayoutHeader hasDivider>
		<HStack gap={2} vAlign="center">
			...
			<Button label="New order" icon={plusIcon} />
		</HStack>
	</LayoutHeader>
{/snippet}
...
{#snippet content()}
	<LayoutContent padding={3}>
		<VStack gap={4}>
			<!--
				Upstream's revenue chart, transcribed and inert: `Chart`, `ChartGrid`,
				`ChartAxis`, `area` and `line` come from `@astryxdesign/charts`, which
				this port has not started. `grid` and `axes` are `ReactNode` props
				upstream and would be snippets here. Uncomment when
				`@astryx-svelte/charts` lands; nothing else on this page changes.

				{#snippet grid()}<ChartGrid horizontal />{/snippet}
				{#snippet axes()}
					<ChartAxis position="bottom" />
					<ChartAxis position="left" tickFormat={(v: unknown) => `$${Number(v) / 1000}k`} />
				{/snippet}
				<Chart
					data={revenueData}
					xKey="date"
