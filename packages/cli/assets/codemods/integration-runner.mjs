 ...
 * Config codemods target the consumer's astryx-svelte.config.* file; code
 * codemods are applied to source files discovered under `--path`, filtered by
 * each codemod's `fileExtensions`.
 *
 ...
 * implementation of "run a config codemod against astryx-svelte.config.*" and
 * "run a code codemod against source files".
 */

import * as fs from 'node:fs';
import * as path from 'node:path';
import { findSourceFiles, makeLog, runCodeCodemod, runConfigCodemod } from './run-codemod.mjs';

/**
 * Run file-based integration codemods, version-ordered. Config codemods run
 * first (targeting astryx-svelte.config.*), then code codemods (source
 * globbing).
 *
...
export function runIntegrationCodemods(
	...
	for (const entry of configEntries) {
		...
		const r = runConfigCodemod(entry, { apply, log, parse, walk });
	...
	if (codeEntries.length > 0) {
		...
		for (const entry of codeEntries) {
			log.info(`  ${entry.codemod.title} (v${entry.version}, ${entry.package})`);
			const r = runCodeCodemod(entry, files, { apply, log, parse, walk });
			totalFilesChanged += r.filesChanged;
			totalTransformsApplied += r.filesChanged;
			writtenFiles.push(...r.writtenFiles);
			errors.push(...r.errors);
