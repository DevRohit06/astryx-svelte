 ...
 * than silently skipping migrations. Callers that must survive one bad
 * integration (`Project.codemods`, `_adapter.selectIntegrationCodemodsFor`)
 * discover each integration in isolation and catch.
 */
 ...
 * @returns {Promise<Map<string, Array<import('../../authoring/codemod/type').CodemodEntry>>>}
	...
	for (const integration of loadedIntegrations ?? []) {
		...
		for (const version of versionFolders) {
			...
			for (const { id, file } of files) {
				...
				const codemod = await loadModuleWithParser(file, parseCodemod, { label });

				/** @type {import('../../authoring/codemod/type').CodemodEntry} */
				const entry = {
					id,
					type: codemod.type,
					codemod,
 ...
 *
 * @param {Map<string, Array<import('../../authoring/codemod/type').CodemodEntry>>} byVersion
 * @param {string} from
 * @param {string} to
 * @returns {Array<{version: string, codemods: Array<import('../../authoring/codemod/type').CodemodEntry>}>}
 */
export function selectIntegrationCodemods(byVersion, from, to) {
	const versions = [...byVersion.keys()].sort(semverCompare);
	/** @type {Array<{version: string, codemods: Array<import('../../authoring/codemod/type').CodemodEntry>}>} */
	const results = [];
