/**
 * @file Duration and easing tokens. Prose verbatim; the usage block is a
 * `.stylex.ts` module reading `var(--duration-*)` rather than importing
 * `durationVars`/`easeVars`, which this port publishes from no subpath.
 */
			...
			content: [
				{
					type: 'code',
					lang: 'ts',
					label: 'Applying motion tokens (page.stylex.ts)',
...
export const styles = stylex.create({
	...
	slideUp: {
		...
		transitionTimingFunction: 'var(--ease-standard)'
				...
				{
					type: 'prose',
					text: "Svelte's own `transition:`/`animate:` directives take a duration in milliseconds rather than a CSS value, so read the token off the element when you need one: `getComputedStyle(node).getPropertyValue('--duration-medium')`. Prefer a CSS transition where one will do — it costs no JavaScript and honours the cascade."
				}
			]
		},
		{
