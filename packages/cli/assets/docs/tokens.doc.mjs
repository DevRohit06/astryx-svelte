/**
 * @file Every Astryx design token, by category.
 *
 * Transcribed from upstream's generated `assets/docs/tokens.doc.mjs` at
 * v0.3.0. The token *names and values* are carried verbatim, and that is
 * checked rather than assumed: all 184 rows resolve to a declaration in this
 * port's own `packages/core/src/lib/styles/tokens.stylex.ts`, which is the same
 * property the class oracle proves for the compiled CSS.
 *
 * Two things are not verbatim. The source path in upstream's generator banner
 * points at `packages/core/src/theme/tokens.stylex.ts`; ours is
			...
			content: [
				{
					type: 'code',
					lang: 'ts',
					label: 'Tokens in a .stylex.ts module',
				...
				},
				{
					type: 'code',
					lang: 'svelte',
					label: 'Applying them to a component',
					code: `<script lang="ts">
...
<Card xstyle={styles.card}>Tokens applied via xstyle</Card>`
				...
				{
					type: 'prose',
					text: 'StyleX may only be imported from a `.ts` module, never from a `.svelte` file — the bundler plugin Babel-parses anything that imports `@stylexjs/stylex` and would read Svelte markup as JSX. Author styles in a sibling `.stylex.ts` and pass them in as `xstyle`.'
				},
				{
					type: 'prose',
					text: 'For a non-StyleX styling library, `tokenVar()` / `tokenVars` from `@astryx-svelte/core/theme` return the same `var(--token)` references as typed values. See `astryx-svelte docs styling` for how to apply tokens via xstyle, class, and compound component patterns, and `astryx-svelte docs theme` for overriding tokens with defineTheme.'
				}
			]
		}
	]
