/**
 * @file Frame-first layout guide. Prose verbatim; the three code blocks become
 * Svelte markup, which changes one structural thing worth naming: upstream's
 * `header={<LayoutHeader/>}` slot props are snippets here, written as
 * `{#snippet header()}…{/snippet}` inside the component.
 */
	...
	sections: [
		{
			...
			content: [
				...
				},
				{
					...
					code: `<script lang="ts">
	import { AppShell, Layout, LayoutContent, LayoutPanel, SideNav } from '@astryx-svelte/core';
</script>

<!-- Frame: nav 256 | content flex | inspector 380 (resizable) -->
<AppShell contentPadding={0}>
	{#snippet sideNav()}
		<SideNav><!-- nav items --></SideNav>
	{/snippet}
	...
	<Layout>
		...
		},
		{
			...
			content: [
				...
				{
					type: 'prose',
					text: 'Start from a template that matches the archetype (`astryx-svelte template --list`), then study its structure with `--skeleton` before customizing.'
				}
			]
		},
		{
