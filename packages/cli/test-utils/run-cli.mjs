 ...
 * spawning `node bin/astryx-svelte.mjs` per assertion. That makes CLI suites
 * fast and deterministic (no per-test Node cold-start to starve under parallel
 * load).
 *
 * Returns `{status, code, stdout, stderr}` (both `status` and `code` are the
 * exit code). Accepts the cwd either positionally (`runCli(args, dir)`) or as an
 * option (`runCli(args, {cwd})`). It IS async (command actions are async), so
 * call sites use `await runCli(...)`.
 *
 * Lives outside the published tree (`test-utils/` is not in `package.json#files`),
 * like upstream's.
 */
 ...
 *
 * @param {string[]} args argv after `astryx-svelte` (e.g. `['manifest', '--json']`)
...
export async function runCli(args, cwdOrOpts) {
	const cwd = typeof cwdOrOpts === 'string' ? cwdOrOpts : cwdOrOpts && cwdOrOpts.cwd;
	...
	process.argv = ['node', 'astryx-svelte', ...args];
	...
	try {
		...
		if (e instanceof ExitSignal) {
			...
			// Mirror bin/astryx-svelte.mjs's error boundary: route it through the JSON
			// shim so --json consumers get a valid envelope (with the mapped ERR_ code)
