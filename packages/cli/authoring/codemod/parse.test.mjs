 ...
 * against a codemod module's default export during `astryx-svelte upgrade`. Zod
 * is sealed inside the parser; authors write a plain object with a `type`
...
describe('parseCodemod (load boundary)', () => {
	...
	it('accepts a code codemod with description, fileExtensions, explicit isOptional', () => {
		...
		expect(parsed.fileExtensions).toEqual(['.svelte']);
	});
	...
	it('rejects a non-function transform', () => {
		expect(reason({ type: 'code', title: 'x', transform: 'nope' })).toMatch(/transform/i);
	});

	it('rejects a missing/invalid type discriminator', () => {
		expect(() => parseCodemod({ title: 'x', transform: () => null })).toThrow();
		expect(() => parseCodemod({ type: 'bogus', title: 'x', transform: () => null })).toThrow();
	});
