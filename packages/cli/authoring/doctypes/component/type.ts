...
export interface ComponentBaseDoc {
	...
	type?: 'component';
	/** The component's name, PascalCase.
	 *  e.g. `"Button"`, `"Table"`, `"TextInput"`, `"AppShell"`
	 *
	 *  Upstream words this as the doc's directory name. It is not that here:
	 ...
	 *  Lowercase only. Used by `astryx-svelte component <term>` for fuzzy matching.
	 ...
	 *  directory's doc covers a group but some `*.svelte` files shouldn't
	 ...
	 *  by this component: `astryx-*` classes plus data-attribute reflections that
 ...
 *
 *   /\*\* \@type \{import('@astryx-svelte/cli/authoring').ComponentDoc\} *\/
 *   export const docs = \{ ... \};
 *
...
export interface ComponentEntry {
	/** Full export name as published from `@astryx-svelte/core`. e.g. `"TableRow"`,
	 ...
	 *  (e.g. `"TableRow"` → `"Table Row"`). See `ComponentBaseDoc.displayName`. */
	displayName: string;
 ...
 * they cluster related components but have no corresponding `*.svelte` file.
...
export interface ComponentRef {
	/** Full export name as published from `@astryx-svelte/core`, e.g.
	 *  `"ChatComposer"`. Must match the `name` field of the referenced
	 *  sub-component's own doc. */
	name: string;
}
 ...
 *
 * Use this when the directory has multiple `*.svelte` files
 ...
 *
 * Use this when the directory has one main `*.svelte` file
