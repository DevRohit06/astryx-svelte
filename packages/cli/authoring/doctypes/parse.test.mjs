 ...
 *
 * 27 cases, matching upstream's suite one for one. The six `loadComponentDoc`
 * cases were `it.todo` until slice 4 landed
 * `foundation/discovery/component-loader.mjs`; they now run.
 */
...
describe('per-kind parsers (stamped format)', () => {
	...
	it('parseComponent rejects a missing name with a readable message', () => {
		expect(() => parseComponent({ type: 'component', name: '', props: [] })).toThrow(
			/name is required/
		);
	});
	...
	it('parseHook rejects a function doc missing returns', () => {
		expect(() => parseHook({ type: 'function', name: 'useThing', params: [] })).toThrow();
	});
...
describe('parseDoc (load boundary, both formats)', () => {
	...
	it('accepts the OLD loose single-component shape (no type)', () => {
		// Upstream writes `const {type, ...loose}`; this repo's eslint config sets
		// an explicit `varsIgnorePattern: '^_'`, which turns off the default
		// `ignoreRestSiblings`. Renaming the binding keeps the omitted key intact.
		const { type: _type, ...loose } = goodComponent;
	...
	it('passes through loose extras (usage, playground, theming, importPath, showcase)', () => {
		const { type: _type, ...base } = goodComponent;
		const parsed = parseDoc({
			...
			importPath: '@astryx-svelte/core/Widget',
			showcase: 'WidgetHero'
		});
		expect(parsed.importPath).toBe('@astryx-svelte/core/Widget');
	...
	it('REGRESSION: loads the OLD loose `export const docs = {}` format', async () => {
		...
		fs.writeFileSync(
			...
			[
				...
				"  importPath: '@astryx-svelte/core/Legacy',",
		...
		expect(docs.importPath).toBe('@astryx-svelte/core/Legacy');
	});
