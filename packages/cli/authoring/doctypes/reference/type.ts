 ...
 * { type: 'code', lang: 'svelte', code: 'padding: spacingVars[...]' }
 ...
 * in the docs/ directory and it shows up in `astryx-svelte docs`.
 *
 ...
 * and can be individually retrieved via `astryx-svelte docs <topic> <section>`.
 */
...
export interface ReferenceTranslationDoc {
	...
	sections: {
		 ...
		 *  change (code, table). */
		content: ({ type: 'prose'; text: string } | { type: 'list'; items: string[] } | null)[];
	}[];
}
