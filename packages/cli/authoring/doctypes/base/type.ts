 ...
 * - An **ComponentSlotElement** — a serialized element descriptor the playground
 *   resolves (see the TODO on {@link ComponentSlotElement})
 *
...
export interface ComponentPlaygroundConfig {
	 ...
	 */
	wrapper?: {
		/** Parent component name as exported from `@astryx-svelte/core`, e.g. `'TabList'`. */
		component: string;
 ...
 * should have an entry. Skip internal/styling props like `class` (this port's
 * counterpart to upstream's `xstyle`/`className`, applied through
 * `internal/sx.ts`), `style`, and `data-testid`.
 *
 * @example
 * ```
...
export interface ComponentPropDoc {
	/** Prop name exactly as it appears in the Svelte markup, camelCased.
	...
	required?: boolean;
	/** For `Snippet` props: the Astryx components this slot typically accepts.
	 ...
	 *  Omit for `Snippet` props that accept plain text (label, description).
	 *
	 *  @example
	 *  ```
...
// TODO(playground): this type serializes React's `createElement`. Svelte 5 has
// no synchronous element factory, so there is no direct counterpart, and the
// only consumer is the docs playground — a later phase. The shape is ported
// verbatim and the resolution mechanism is deliberately NOT invented here: how
// a descriptor becomes a rendered Svelte component is still open.
/**
 ...
 * via `@scope` selectors. Each component renders one or more stable `astryx-*`
...
export interface ComponentThemingVar {
	 ...
	 * `astryx-svelte theme build` errors if a theme sets a private var directly.
	 */
	private?: boolean;
}
...
export interface UsageDoc {
	...
	/** Component-specific accessibility requirements a consumer must preserve. */
