 ...
 * `astryx-svelte.config.*`. Zod is sealed inside the parser, so these exercise
 * the public contract only: a validated value comes back on success, a readable
...
describe('parseConfig (load boundary)', () => {
	...
	it('accepts a fully-populated valid config', () => {
		const cfg = {
			integrations: ['@astryx-svelte/core'],
	...
	it('rejects a postCodemod hook without buildCommand', () => {
		expect(() => parseConfig({ hooks: { postCodemod: [{ name: 'empty' }] } })).toThrow();
	});
	...
	it('rejects an xle component missing `from`', () => {
		expect(() => parseConfig({ experimental: { xle: { components: { Foo: {} } } } })).toThrow();
	});
});
