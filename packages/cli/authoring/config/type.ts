/**
 * Public type surface for an Astryx config file (`astryx-svelte.config.{ts,mjs,js}`).
 *
 * Authors write a plain object against {@link AstryxConfig} — in TS via
 * `satisfies AstryxConfig`, in JS via
 * `/** @type {import('@astryx-svelte/cli/authoring').AstryxConfig} *\/`.
...
export interface XleComponent {
	/** Import specifier the component is imported from, e.g. '$lib/components/KpiCard'. */
	from: string;
	/** Optional human description shown in tooling. */
	description?: string;
	/**
	 * Import as the module's default export instead of a named export. Defaults
	 * to false.
	 *
	 * Inert for a `.svelte` file — a Svelte component module has nothing *but* a
	 * default export, so there is no named-export branch for this flag to select
	 * between. The field is kept because it is part of upstream's config
	 * contract; the XLE consumer that would read it lands in slice 10.
	 */
	default?: boolean;
}

/** User config exported from astryx-svelte.config.{ts,mjs,js}. */
